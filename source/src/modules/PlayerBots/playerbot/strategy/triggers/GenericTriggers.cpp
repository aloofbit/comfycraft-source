...
// Named backwards: active when mana is BELOW high, not above it. It is the
// only gate on sitting down to drink -- nothing else uses this trigger -- and
// it was a hardcoded 65, which is not "top up between pulls", it is "wait until
// a third is gone". Drinking is out-of-combat only, so waiting buys nothing.
// NOBODY SITS DOWN WHILE THE TANK IS PULLING.
//
// Eating and drinking are out-of-combat behaviours, and the whole of a pull is
// out of combat for everyone except the tank - so the pull is precisely when a
// hungry bot decides to have lunch. It does not merely look bad: DrinkAction
// calls StopMoving and refuses while the bot is moving, so a mage sitting down
// is a mage that has stopped walking to the mob it is supposed to sheep.
//
// EngageOnOrder stands them up and strips the auras when the order lands, but
// that alone only bought one tick: on the next, this trigger saw a bot that was
// no longer drinking and still below its threshold, and sent it straight back
// down. The order has to suppress the appetite, not just interrupt the meal.
//
// Self-limiting, because IsGroupPullRunning reads the pull's own 15s clock -
// there is no way for this to leave a bot permanently unable to eat.
static bool GroupIsPulling(PlayerbotAI* ai)
{
    return PullStrategy::IsGroupPullRunning(ai);
}

bool HighManaTrigger::IsActive()
{
    if (GroupIsPulling(ai))
        return false;

    if (!AI_VALUE2(bool, "has mana", "self target"))
        return false;

    const uint8 mana = AI_VALUE2(uint8, "mana", "self target");

    // WHO IS DRINKING MATTERS MORE THAN THE NUMBER.
    //
    // DrinkAtMana is 95 and the config comment defends that for the case it was
    // written for: "a healer that starts a pull two thirds full runs dry before
    // it ends". True of healers, and only of healers. Applied to a mage it meant
    // sitting down after almost every sheep, which is not restoring mana so much
    // as being unavailable - and out-of-combat is exactly when a pull is being
    // set up, so the group waited on a mage drinking off the last 5%.
    if (PlayerbotAI::IsHeal(bot))
    {
        return mana < sPlayerbotAIConfig.drinkAtMana;
    }

    // TWO NUMBERS, BECAUSE THIS TRIGGER IS BOTH THE START AND THE STOP.
    //
    // It is asked every tick while the bot drinks, so whatever it returns false
    // at is where drinking ENDS. A single "drink below 50" therefore does not
    // mean "wait until 50 and then fill up", it means "drink from 49 to 50 and
    // stand". Shipped that way for one build on 2026-09-07 and it was worse
    // than the problem it fixed: the mage lived between 30 and 50, MediumMana
    // is 40, and below that casters drop their real rotation for the wand. The
    // report was "DPS is just trying to use wand", which was mana, not spec.
    //
    // So: do not sit down before half, and once sitting, finish the job. The
    // ceiling is short of full on purpose - the last few percent take as long
    // as the rest and nobody is waiting on them.
    const bool alreadyDrinking = ai->HasAura("drink", bot) || !bot->IsStandState();

    return alreadyDrinking ? (mana < 90) : (mana < 50);
}

// The eating counterpart. It exists because "low health" could not be used:
// that trigger drives healing decisions in three dozen class strategies, and it
// is a RANGE (criticalHealth..lowHealth) rather than a floor, so a bot below
// 20% health would not have eaten at all.
bool NeedsFoodTrigger::IsActive()
{
    // Same as drinking - see GroupIsPulling above.
    if (GroupIsPulling(ai))
        return false;

    return AI_VALUE2(uint8, "health", "self target") < sPlayerbotAIConfig.eatAtHealth;
}
...
bool HasCcTargetTrigger::IsActive()
{
    ...
    // marked mob could be controlled during a tank's pull - which is out of
    // combat for everyone except the tank, and is the one moment cc is really
    // worth anything. That worked, and it also made the mark on its own
    // sufficient: walk up to a camp with nothing happening, put moon on one of
    // them, and the mage sheeps it immediately. Which pulls the camp, on behalf
    // of a party that had not decided to fight yet.
    //
    // Marking says WHICH one when the fight starts. It does not say that it
    // has. Nothing changes in combat - this line is short-circuited before the
    // gate is ever asked - and nothing changes during a pull, which is what the
    // mirror was added for.
    if (!sServerFacade.IsInCombat(bot) && !CcFightIsUnderway(ai))
        return false;
