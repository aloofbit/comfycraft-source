...
namespace ai
{
    ...
    // Something is hitting a party member that is not me and not another tank.
    // Separate from ProtectPartyMemberTrigger, which waits for that member to
    // drop under 30% - by then the point of tanking has been missed.
    class PartyMemberAttackedTrigger : public Trigger
    {
    public:
        PartyMemberAttackedTrigger(PlayerbotAI* ai) : Trigger(ai, "party member attacked") {}

        virtual std::string GetTargetName() override { return "party member attacker"; }

        virtual bool IsActive() override
        {
            return ai->IsTank(bot) && AI_VALUE(Unit*, "party member attacker");
        }
    };
    ...
    // Sitting down to eat. See NeedsFoodTrigger::IsActive for why this is not
    // the "low health" trigger.
    class NeedsFoodTrigger : public Trigger
    {
    public:
        NeedsFoodTrigger(PlayerbotAI* ai) : Trigger(ai, "needs food") {}
