...
private:
    ...
    // Dire Bear is a level 40 spell and the bear-form trigger asks for it by
    // name, with nothing behind it - so a druid tank below 40 tried to cast a
    // spell it does not know and never shifted at all. In game that is a druid
    // "tank" standing in caster form taking the hits in cloth-grade armour.
    //
    // Fall back to plain Bear Form, which a druid has from level 10. Exactly
    // the pattern the warrior strategy already uses for
    // devastate -> sunder armor, and the reason one strategy file can serve
    // three expansions and every level in them.
...
void TankFeralDruidBuffStrategy::InitCombatTriggers(std::list<TriggerNode*>& triggers)
{
    ...
    // DEMORALIZING ROAR ON A SINGLE TARGET.
    //
    // Roar was wired ONLY to "melee light aoe" and "melee medium aoe", so
    // against one mob a bear never roared at all - and the roar is mitigation,
    // wanted on one target as much as on five.
    //
    // Driven by the debuff being MISSING rather than by a timer, so it is cast
    // once and refreshed rather than spammed.
    //
    // SWIPE IS DELIBERATELY LEFT AoE-ONLY. It was added here too and taken back
    // out: Maul is the single-target threat ability on this server, and
    // swiping one mob only burns the rage Maul wants. The owner's own
    // description of the rotation - "run in, demo roar, spam swipe" - is the
    // PACK rotation, which the AoE nodes above already implement.
    triggers.push_back(new TriggerNode(
        "demoralizing roar",
        NextAction::array(0, new NextAction("demoralizing roar", ACTION_NORMAL + 4), NULL)));
