...
void DruidCcStrategy::InitNonCombatTriggers(std::list<TriggerNode*>& triggers)
{
    ...
    // Out of combat too - see MageCcStrategy::InitNonCombatTriggers. Hibernate
    // only; entangling roots snares rather than removes, so casting it before
    // the fight buys nothing.
    triggers.push_back(new TriggerNode(
        "hibernate",
        NextAction::array(0, new NextAction("hibernate on cc", ACTION_INTERRUPT), NULL)));
}
