...
void RogueCcStrategy::InitNonCombatTriggers(std::list<TriggerNode*>& triggers)
{
    ...
    // Out of combat too - see MageCcStrategy::InitNonCombatTriggers. Sap is the
    // clearest case of the lot: it can ONLY be used out of combat, so a
    // combat-only trigger meant a rogue could never sap anything at all.
    triggers.push_back(new TriggerNode(
        "sap",
        NextAction::array(0, new NextAction("sap", ACTION_INTERRUPT + 1), NULL)));
}
