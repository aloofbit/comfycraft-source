...
namespace ai
{
    ...
    // CHARGE IS NOT USEFUL IN COMBAT, AND SAYING SO IS WHAT STOPS THE STANCE
    // DANCE.
    //
    // Charge cannot be used in combat on this core (SPELL_ATTR
    // 0x10000000 on 100/6178/11578; Intercept is the in-combat one). The
    // engine asks isUseful, then runs the action's PREREQUISITES, and only
    // then isPossible - so a charge that was "useful" (target 8+ yards out)
    // but impossible (in combat) still executed its prerequisite, Battle
    // Stance, every tick. The tank's Defensive Stance rule then flipped it
    // back, and with any target out of melee a protection warrior spent the
    // fight switching stances. Seen on a level 20 tank companion on
    // 2026-09-09 and traced through the engine's order of checks. Out of
    // combat, when the charge can land, nothing changes.
    class CastChargeAction : public CastReachTargetSpellAction
    {
    public:
        CastChargeAction(PlayerbotAI* ai) : CastReachTargetSpellAction(ai, "charge", 8.0f) {}
    ...
    // Taunt the mob beating on a party member, rather than whatever the bot
    // happens to be hitting. Overriding GetTargetName is the whole mechanism -
    // the framework resolves the named value to a Unit and casts at it, which
    // is how "taunt on snare target" already works.
    class CastTauntOnPartyAttackerAction : public CastMeleeSpellAction
    {
    public:
        CastTauntOnPartyAttackerAction(PlayerbotAI* ai) : CastMeleeSpellAction(ai, "taunt") {}

    protected:
        std::string GetTargetName() override { return "party member attacker"; }
        std::string getName() override { return "taunt on party attacker"; }
    };

    // The fallback when Taunt is on cooldown. Sunder Armor generates a large
    // flat amount of threat in 1.12 - far more than its damage - which is what
    // makes it the right filler rather than a plain melee swing.
    class CastSunderArmorOnPartyAttackerAction : public CastMeleeDebuffSpellAction
    {
    public:
        CastSunderArmorOnPartyAttackerAction(PlayerbotAI* ai)
            : CastMeleeDebuffSpellAction(ai, "sunder armor") {}

    protected:
        std::string GetTargetName() override { return "party member attacker"; }
        std::string getName() override { return "sunder armor on party attacker"; }
    };
