...
void ProtectionWarriorStrategy::InitCombatTriggers(std::list<TriggerNode*>& triggers)
{
    ...
    // Pick up whatever is hitting somebody else.
    //
    // `lose aggro` above does NOT cover this: it tests aggro on the tank's
    // CURRENT target, so it recovers a mob already being fought that turned on
    // the healer, and never notices an add that walked in and went straight
    // for them. Upstream has a "protect party member" node for that case, but
    // only inside the TBC and WotLK blocks - this build is MANGOSBOT_ZERO, so
    // a vanilla tank had no answer to a loose add at all.
    //
    // Taunt first, Sunder Armor when it is on cooldown: sunder's threat in
    // 1.12 is a large flat amount unrelated to its damage, which is what makes
    // it the right filler rather than just swinging. Both target the ATTACKER
    // through "party member attacker", not the party member being attacked -
    // upstream's version fires `intervene`, which leaps to the ally and
    // transfers damage without taking any threat, and so does not solve this.
    triggers.push_back(new TriggerNode(
        "party member attacked",
        NextAction::array(0,
            new NextAction("taunt on party attacker", ACTION_EMERGENCY + 1),
            new NextAction("sunder armor on party attacker", ACTION_EMERGENCY),
            NULL)));
...
{
    // NOT WarriorBuffStrategy's version. That one asks every warrior for
    // Battle Stance out of combat, so a tank left every fight in Defensive,
    // switched to Battle the moment it ended, and switched back at the next
    // pull - one more stance change per fight on top of the charge dance
    // (WarriorActions.h, CastChargeAction). A tank rests in Defensive.
    BuffStrategy::InitNonCombatTriggers(triggers);

    triggers.push_back(new TriggerNode(
        "defensive stance",
        NextAction::array(0, new NextAction("defensive stance", ACTION_NORMAL), NULL)));
