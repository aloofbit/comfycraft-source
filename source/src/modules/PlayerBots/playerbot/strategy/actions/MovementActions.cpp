...
{
    // While a pull is running, the pull owns where the bot goes.
    //
    // This has to be here rather than in PullMultiplier, because a multiplier
    // CANNOT be a gate. Engine::DoNextAction keeps a per-tick modifiedActions
    // list, and once an action has been multiplied to zero and re-pushed it
    // goes in that list - after which every later pop of it that tick SKIPS the
    // multipliers. Failed and impossible actions meanwhile re-push their
    // alternatives at relevance + 0.03. So a zeroed "reach melee" came back at
    // 0.03 with PullMultiplier never consulted again, and 0.03 is not zero, so
    // it ran. Measured 2026-09-07: "> reach melee @0.03" mid-pull, which is
    // what sent this to isUseful in the first place.
    //
    // isUseful is checked before multipliers and is not memoised, so a refusal
    // here is final. That is the difference between suppression and a gate.
    if (!IsPullMovement() && IsPullRunning())
        return false;

    // Passive is the same shape of problem and needs the same answer.
    //
    // It is enforced by PassiveMultiplier, so it fails identically: the action
    // is zeroed, and then Engine::DoNextAction only re-queues a zeroed action
    // when something still in the basket OUTRANKS it - so when it is the last
    // one left, nothing does and it executes at relevance zero anyway.
    //
    // Measured 2026-09-07: a tank told to follow mid-pull alternated "follow"
    // and "reach melee" indefinitely, running out to its firing position and
    // back again. The pull had ended cleanly; the follow strategy moved it
    // home, the zeroed reach action moved it back out, one per tick, and
    // neither ever won because they were never actually competing.
    //
    // PassiveMultiplier::Allows is the SAME list the multiplier scores with, so
    // the gate and the weight cannot disagree about what passive permits.
    if (ai->HasStrategy("passive", ai->GetState()) && !PassiveMultiplier::Allows(getName()))
        return false;

    // WALKING CANCELS CASTING, so a bot mid-cast does not walk.
    //
    // ReachTargetAction has always known this and checked it for itself, which
    // covered the reach actions and nothing else - follow, flee and the return
    // to a saved position could all still take a tick out from under a cast
    // that was nearly done. In a group that is precisely when it happens: the
    // mage starts a sheep, something asks it to reposition, and the sheep dies
    // a tenth of a second from landing, with no error anywhere because nothing
    // failed.
    //
    // skipAutorepeat is the third argument, and it is what makes this safe for
    // the pull: a gun or a wand is an auto-repeat spell, so a tank shooting is
    // not "casting" by this test and keeps its freedom to move. Only real cast
    // bars and channels stop the feet.
...
bool MovementAction::IsPullRunning()
{
    const PullStrategy* pull = PullStrategy::Get(ai);
    return pull && pull->GetPullState() != PullState::None;
}

bool MovementAction::IsPullMovement()
{
    // The pull's own movers, and only these, may move the bot during a pull.
    const std::string& name = getName();
    return (name == "pull approach") ||
...
{
    // The pull owns where the bot goes, and this is the choke point.
    //
    // The gate started in isUseful, but subclasses override that without
    // calling the base - ReachTargetAction does, and so "reach melee" walked a
    // tank out of position mid-pull despite the gate existing. Chasing every
    // override is a losing game; the two primitives that actually move a bot
    // are not.
    if (!IsPullMovement() && IsPullRunning())
        return false;

    // Mid-charge, the core is moving the bot. Anything started here would go
    // through MotionMaster::Mutate, which interrupts the charge first. See
    // PlayerbotAI::IsCharging.
    if (ai->IsCharging())
        return false;
...
bool MovementAction::ChaseTo(WorldObject* obj, float distance, float angle)
{
    ...
    // Charge. This guard existed but tested EFFECT_MOTION_TYPE, and this
    // core's charge generator reports CHARGE_MOTION_TYPE, so it never fired
    // and the chase below interrupted every charge a bot made.
    if (ai->IsCharging())
    {
        return false;
    }
...
{
    // Fleeing abandons a pull outright.
    //
    // Nothing on this path used to clear it, so the pull target and the saved
    // anchor both survived: PullMultiplier went on zeroing every ordinary
    // action, and "return to pull position" went on outranking them at 35, so
    // the bot fled and was then dragged back to the anchor it was fleeing from.
    // Only combat actually ending cleared it (PlayerbotAI::OnCombatEnded).
    //
    // Routed through "pull end" rather than OnPullEnded() so the pet's react
    // state is restored and the anchor erased - the same cleanup a pull that
    // finished normally gets.
