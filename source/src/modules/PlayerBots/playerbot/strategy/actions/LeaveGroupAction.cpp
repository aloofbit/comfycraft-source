...
namespace ai
{
	bool LeaveGroupAction::Leave(Player* player) 
    {
        ...
        // A COMPANION IS BEING DISMISSED, NOT RE-TASKED.
        //
        // ResetStrategies rebuilds the bot's engines - AddDefaultReactionStrategies
        // -> Engine::Init -> Engine::Reset -> Queue::Pop - and this runs from
        // INSIDE an action that the engine is still executing:
        //
        //   Engine::DoNextAction -> ListenAndExecute -> UninviteAction::Execute
        //     -> LeaveGroupAction::Leave -> ResetStrategies -> Engine::Reset
        //     -> Queue::Pop  -> SEH 0xc0000005
        //
        // That killed the server twice on 2026-09-06 when a DEAD companion was
        // uninvited. A companion has nothing to reset to: the group hook has
        // already queued it for deletion and it will be gone within a tick, so
        // rebuilding its brain first is work that can only go wrong.
        //
        // The re-entrancy is a general hazard rather than a companion one - any
        // bot uninvited at the wrong moment can hit it - but the safe general
        // fix is deferring the reset out of action execution, which is a bigger
        // change than this one and should be made deliberately.
