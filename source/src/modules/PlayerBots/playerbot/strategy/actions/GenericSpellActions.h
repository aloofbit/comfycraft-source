...
namespace ai
{
    ...
    {
    public:
        CastCrowdControlSpellAction(PlayerbotAI* ai, std::string spell) : CastRangedDebuffSpellAction(ai, spell), lastFailure(SPELL_CAST_OK), attempts(0) {}

        // A refused cc cast must not be retried on the very next tick.
        //
        // Engine::ListenAndExecute calls SetActionDuration ONLY when Execute
        // returned true, so a failing cast sets no delay of any kind and the
        // trigger re-pushes it immediately - a mage visibly stuttering on the
        // same spell several times a second without ever casting it. The
        // trigger cannot break the loop either: it stays active precisely
        // BECAUSE the aura never landed.
        //
        // isPossible() cannot prevent it. It ends in CanCastSpell with
        // ignoreRange = true, and that call forgives seven results the real
        // cast does not - MOVING, LINE_OF_SIGHT, OUT_OF_RANGE and NOT_INFRONT
        // among them. Same shape as the pull opener bug on 2026-09-07.
        bool Execute(Event& event) override;
    ...
    private:
        ...
        // Reported to the master only when it CHANGES, so a stuck cc says its
        // reason once instead of once a tick.
        SpellCastResult lastFailure;

        // Consecutive refusals on lastTarget. The reason decides what to do;
        // this decides when to stop believing the reason, because five failures
        // running on one mob is not a situation about to resolve itself.
        // Reset by a success, and by the target changing.
        uint32 attempts;
