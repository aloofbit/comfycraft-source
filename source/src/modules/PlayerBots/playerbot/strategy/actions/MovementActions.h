...
namespace ai
{
    ...
    protected:
        // True for the pull's own movers, which are the only actions allowed to
        // move the bot while a pull is running - see isUseful.
        bool IsPullMovement();

        // True while a pull is running at all. Checked in the movement
        // primitives, not only in isUseful, because subclasses override
        // isUseful without calling the base.
        bool IsPullRunning();
