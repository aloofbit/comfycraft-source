...
namespace ai
{
    ...
    public:
        ...
        virtual bool isUseful() override
        {
            ...
            // A companion never releases. CompanionReviveAction stands it back
            // up where its owner is, so releasing would only send it to a
            // graveyard first and then teleport it back from there - a corpse
            // run whose only effect is the loading screen at the end of it.
            // Suppressed here rather than at the trigger so the manual
            // "release" whisper still works when the owner wants it.
            if (sPlayerbotAIConfig.companionReviveDelay &&
                CompanionOwnership::IsCompanion(bot->GetGUIDLow()))
                return false;
    ...
    // Companions do not corpse run. Thirty seconds after the party stops
    // fighting (AiPlayerbot.CompanionReviveDelay), a dead companion gets back
    // up at its owner's feet with a quarter of its health and mana and no
    // resurrection sickness.
    //
    // At the owner rather than at the body on purpose: it died where the fight
    // was, and reviving there is reviving inside whatever killed it. Free
    // rather than on spirit-healer terms because a companion is hired help -
    // a durability bill on top of the hire is a tax that buys nothing.
    //
    // A healer companion still wins the race. "party member dead" fires the
    // moment combat ends, at ACTION_EMERGENCY, so a real resurrection lands
    // well inside the wait and this never runs.
    class CompanionReviveAction : public Action
    {
    public:
        CompanionReviveAction(PlayerbotAI* ai, std::string name = "companion revive") : Action(ai, name) {}
    ...
    public:
        virtual bool Execute(Event& event) override
        {
            ...
            // 0.25f sets health, mana and energy to a quarter of maximum in one
            // call; rage is zeroed for us. false is applySickness - see above.
            bot->ResurrectPlayer(0.25f, false);

            // A no-op when there is no corpse, which is the normal case since a
            // companion never releases. It matters only for the companion that
            // was already a ghost when this fired, e.g. one that released on the
            // manual whisper and is still walking back.
            bot->SpawnCorpseBones();
            ...
            sLog.outDetail("[BOT CORPSE] %s: companion revive - back up at owner %s", bot->GetName(), master->GetName());
            // ignoreSilent: this is companion state, like the pull call-outs,
            // and a muted companion still has to say when it is back up.
            ai->TellPlayerNoFacing(master, "Back on my feet.", PlayerbotSecurityLevel::PLAYERBOT_SECURITY_ALLOW_ALL, false, true, true);
