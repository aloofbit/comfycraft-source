        ...
        if (grindTarget)
        {
            ...
            if (!grindName.empty())
            {
                ...
                if (ai->HasStrategy("pull", BotState::BOT_STATE_COMBAT))
                {
                    ...
                    {
                        // Ammo, or a bot that does not need any. Every other
                        // ammo test in the module asks HasCheat first (see
                        // HunterTriggers, CastShootAction); this one counted
                        // real arrows and was the odd one out, so a bot with
                        // the item cheat -- which can shoot perfectly well
                        // without them -- was refused the pull it was capable
                        // of. Druids and paladins open with a spell instead.
                        const bool canOpenAtRange =
                            ai->GetBot()->getClass() == CLASS_DRUID ||
                            ai->GetBot()->getClass() == CLASS_PALADIN ||
                            ai->HasCheat(BotCheatMask::item) ||
                            AI_VALUE2(uint32, "item count", "ammo");

                        if (strategy->CanDoPullAction(grindTarget) && canOpenAtRange)
                        {
