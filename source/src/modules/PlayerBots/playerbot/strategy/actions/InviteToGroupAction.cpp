    ...
    bool InviteToGroupAction::Invite(Player* inviter, Player* player)
    {
        ...
        if (Group* group = inviter->GetGroup())
        {
            if(GetBotAI(player) && !GetBotAI(player)->IsRealPlayer())
                ...
                {
                    // A COMPANION never promotes a full party to a raid. The
                    // five-body party cap is what stops a group of five players
                    // becoming fifteen with companions, and silently converting
                    // to a 40-slot raid is exactly how that cap gets lost.
                    // Ordinary bots keep the old behaviour, because `.bot group
                    // size=10` deliberately wants a raid.
    ...
    bool JoinGroupAction::Execute(Event& event)
    {
        ...
        if (group)
        {
            if (group->IsFull())
            {
                ...
                {
                    // Same rule as in Invite() above: a companion does not turn
                    // a full party into a raid, it just does not join.
            ...
            // Already in that party is the job done, not a failure. A parked
            // companion coming back after a dropped connection or a restart
            // is still a member of its owner's party - the logout never
            // evicted it - and reporting "join: failed" for arriving where it
            // was asked to go was a real message a player asked about.
