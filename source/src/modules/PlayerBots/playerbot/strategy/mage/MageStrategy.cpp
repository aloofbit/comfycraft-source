...
void MageCcStrategy::InitNonCombatTriggers(std::list<TriggerNode*>& triggers)
{
    ...
    // CC BEFORE THE FIGHT, WHICH IS THE ONLY TIME IT IS REALLY WORTH ANYTHING.
    //
    // This trigger lived in the combat engine alone, so a marked moon was only
    // ever sheeped once the mage was ALREADY fighting - and during a tank's
    // pull it is not, because a party member does not enter combat just because
    // a groupmate did. The trigger was therefore not installed at the one
    // moment the mark exists for, which read from outside as "cc is blocked by
    // holding for the pull". The hold is innocent: CcTargetValue resolves the
    // mark directly and never consults it.
    //
    // Only crowd control that HOLDS a mob is mirrored out of combat. Fear,
    // scare beast and turn undead send it running instead, and something
    // running out of a pull arrives in the next one - a lesson this codebase
    // already learned once, in FindTargetForCcStrategy. Entangling roots is
    // left out for a duller reason: it snares rather than removes, so casting
