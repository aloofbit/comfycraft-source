...
namespace ai
{
    ...
    public:
        ...
        // THIS USED TO CAST BY ITSELF, AND THAT WAS THE WHOLE POLYMORPH BUG.
        //
        // The old body picked a sheep variant and then called ai->CastSpell
        // directly, returning its result. Doing so skipped every layer beneath
        // it - most importantly CastSpellAction::Execute, which is the ONLY
        // thing that calls SetDuration with the spell's cast time. It also used
        // the two-argument CastSpell, so no duration was even computed.
        //
        // The action therefore kept the Action constructor's default of 100ms,
        // ran again a tenth of a second later, and called CastSpell again -
        // building a NEW Spell that cancelled the one four fifths finished. A
        // mage visibly starting polymorph over and over, forever. The debug
        // trace said it outright and it took far too long to read: every line
        // was "polymorph 1 (40.01) (duration: 0.10s)", and 0.10s is that
        // default, which means nothing on the cast path had run at all.
        //
        // It also silently opted polymorph out of everything the base class
        // does for crowd control: the already-casting guard, the reason-driven
        // back-off, and the failure reporting. Three fixes aimed at those had
        // no effect on polymorph because none of that code was ever reached.
        //
        // Now it only CHOOSES, and lets the base cast. The variant is re-rolled
        // only when no cast is in flight, so a re-entry cannot swap the spell
        // out from under a cast that is already running.
        virtual bool Execute(Event& event) override
        {
            if (!bot->IsNonMeleeSpellCasted(true, false, true))
            {
                ...
                // Changes the spell, not the action's name - the queue and the
                // trigger both still know this action as "polymorph".
                SetSpellName(polySpells[urand(0, polySpells.size() - 1)]);
            }

            return CastCrowdControlSpellAction::Execute(event);
        }
    };
