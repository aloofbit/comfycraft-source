                ...
                if (isUseful)
                {
                    ...
                    if (isPossible && relevance)
                    {
                        ...
                        if (actionExecuted)
                        {
                            ...
                            // While a pull is running, name the action that just
                            // won the tick.
                            //
                            // This is the one question the pull's own step
                            // reports cannot answer. They say what the pull
                            // tried to do; they cannot say what outranked it.
                            // The tank running at its target after the opener
                            // is some other action taking movement, and without
                            // this the only way to see which is LogFileLevel 3
                            // and a log trawl.
                            // GetMaster() first, and only then the strategy
                            // lookup: this runs for every executed action of
                            // every bot, and the 300 random bots have no master
                            // to narrate to, so they must not pay for it.
                            if (ai->GetMaster())
                            {
                                if (PullStrategy* pullStrategy = PullStrategy::Get(ai))
                                {
                                    // "> " so a won tick reads differently from
                                    // the pull's own step text. Without it the
                                    // action name that follows a step message
                                    // looks like a second event when it is the
                                    // same tick reported twice.
                                    // Carry the relevance. An action winning a
                                    // tick during a pull is only surprising if
                                    // the multiplier should have zeroed it, and
                                    // the number says which way it got through:
                                    // ~0.01x its push value means the >= 100
                                    // emergency escape, anything else means the
                                    // multiplier never ran on it at all.
                                    if (pullStrategy->HasPullStarted())
                                    {
                                        std::ostringstream won;
                                        won << "> " << action->getName()
                                            << " @" << std::fixed << std::setprecision(2) << relevance;
                                        pullStrategy->TellStep(won.str());
                                    }
                                }
                            }
