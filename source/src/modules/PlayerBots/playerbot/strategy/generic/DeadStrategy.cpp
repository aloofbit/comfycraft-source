...
void DeadStrategy::InitDeadTriggers(std::list<TriggerNode*>& triggers)
{
    ...
    // Above repop (+4) and everything else here, because for a companion this
    // IS the exit - the rest of this strategy is corpse-run machinery that a
    // companion never reaches, having no corpse to run to. The trigger answers
    // false for every other bot, so nothing below it changes.
    triggers.push_back(new TriggerNode(
        "companion revive",
        NextAction::array(0, new NextAction("companion revive", relevance + 5.0f), NULL)));
