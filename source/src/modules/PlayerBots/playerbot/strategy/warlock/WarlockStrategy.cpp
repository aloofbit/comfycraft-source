...
void WarlockCcStrategy::InitNonCombatTriggers(std::list<TriggerNode*>& triggers)
{
    ...
    // Out of combat too - see MageCcStrategy::InitNonCombatTriggers. Banish
    // only; fear is deliberately NOT mirrored, because a feared mob runs into
    // the next pull rather than sitting out of this one.
    triggers.push_back(new TriggerNode(
        "banish",
        NextAction::array(0, new NextAction("banish on cc", ACTION_INTERRUPT + 2), NULL)));
}
