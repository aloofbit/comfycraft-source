...
void HunterCcStrategy::InitNonCombatTriggers(std::list<TriggerNode*>& triggers)
{
    ...
    // Out of combat too - see MageCcStrategy::InitNonCombatTriggers. Freezing
    // trap only; scare beast sends the mob running, which is the opposite of
    // keeping it out of the pull. Relevance matches the combat copy because
    // traps chain movement actions and need to outrank them.
    triggers.push_back(new TriggerNode(
        "freezing trap",
        NextAction::array(0, new NextAction("freezing trap on cc", ACTION_INTERRUPT + 1), NULL)));
}
