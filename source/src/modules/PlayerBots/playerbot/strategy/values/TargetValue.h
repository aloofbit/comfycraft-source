...
namespace ai
{
    ...
    public:
        virtual Unit* GetResult() { return result; }
    ...
    {
    public:
        FindNonCcTargetStrategy(PlayerbotAI* ai) : FindTargetStrategy(ai), ccFallback(nullptr) {}

        // SKIPPED, BUT REMEMBERED.
        //
        // Passing over the crowd-control mark is right while there is anything
        // else worth hitting, and wrong when there is not: the group then has no
        // target at all and stands waiting out a fifty second sheep on the last
        // mob alive. Crowd control is for reducing how many things are on you at
        // once, so when it IS the only thing on you the mark has already done
        // its job.
        //
        // Every subclass that filters through IsCcTarget gets this for free,
        // and none of them has to know about it.
        virtual Unit* GetResult() override { return result ? result : ccFallback; }

    protected:
        virtual bool IsCcTarget(Unit* attacker);

    private:
        bool IsCcTargetInternal(Unit* attacker);
