...
// Something is hitting one of my party and it is not me. Return the ATTACKER so
// a tank can taunt it.
//
// The gap this fills: `lose aggro` only recovers a mob the tank is ALREADY
// fighting - it tests aggro on the tank's current target. An add that walks in
// and goes straight for the healer is never the current target, so nothing
// fires, and in game that reads as the tank ignoring it.
Unit* PartyMemberAttacker::Calculate()
{
    Group* group = bot->GetGroup();
    if (!group)
        return nullptr;
    ...
    for (const ObjectGuid& guid : attackers)
    {
        ...
        // Already on me: that is the job working, not a problem to solve.
        if (victim == bot)
            continue;

        if (!bot->IsInGroup(victim))
            continue;

        // Leave another tank's mobs alone - two tanks taunting the same add off
        // each other is worse than either of them just holding it.
        if (ai->IsTank((Player*)victim))
            continue;

        // Only what we could actually reach and taunt.
        if (sServerFacade.GetDistance2d(bot, unit) > 30.0f)
            continue;

        // Prefer whatever is beating on the lowest-health member.
        if (!best || victim->GetHealthPercent() < best->GetVictim()->GetHealthPercent())
            best = unit;
    }
