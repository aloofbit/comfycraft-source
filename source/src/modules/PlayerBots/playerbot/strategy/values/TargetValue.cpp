...
// Answers "should this be passed over", and records the last one passed over so
// GetResult can fall back to it when nothing else turned up. See the class.
bool FindNonCcTargetStrategy::IsCcTarget(Unit* attacker)
{
    if (IsCcTargetInternal(attacker))
    {
        ccFallback = attacker;
        return true;
    }
...
bool FindNonCcTargetStrategy::IsCcTargetInternal(Unit* attacker)
{
