...
void World::InitPlayerbotsAtStartup()
{
    ...
    // Companions with a living owner inside AiPlayerbot.CompanionParkMinutes
    // are kept for that owner's next login; everything else on the COMPANION
    // pool is deleted, which is what stops a crash leaking characters into it
    // for ever. Needs the config above. Safe when the feature has never been
    // used - the table is simply empty.
    CompanionOwnership::PurgeAtStartup();
