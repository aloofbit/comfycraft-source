...
void AiFactory::AddDefaultCombatStrategies(Player* player, PlayerbotAI* const facade, Engine* combatEngine)
{
    ...
    // A MARK SHOULD MEAN SOMETHING WITHOUT BEING SWITCHED ON FIRST.
    //
    // "focus rti targets" makes the marked mob the only thing on the bot's
    // attacker list, which is exactly what a skull is for - but it was off
    // everywhere, so marking one changed nothing and every group needed a
    // strategy toggle nobody would guess at. It stayed off for a good reason:
    // with no mark set it used to return an EMPTY list, so switching it on gave
    // you bots that refused to attack anything. AttackersValue::Calculate now
    // falls through to normal targeting when nothing is marked, which is what
    // makes this safe to have on by default.
    combatEngine->addStrategy("focus rti targets");
    ...
    switch (player->getClass())
    {
        case CLASS_PRIEST:
        {
            if (tab == 0)
            {
                ...
                // Healing specs only. "healer caution" suppresses the
                // go-and-hit-something actions after area damage and lets a
                // heal beat a panic flee - both wrong for shadow, which is
                // supposed to keep nuking.
                combatEngine->addStrategy("healer caution");
        ...
        case CLASS_WARRIOR:
        {
            if (tab == 2)
            {
                combatEngine->addStrategies("protection", "tank assist", "pull", "close", NULL);
            }
        ...
        case CLASS_SHAMAN:
        {
            ...
            {
                combatEngine->addStrategies("restoration", "healer caution", "flee", "ranged", NULL);
                if (sPlayerbotAIConfig.enableOffSpecStrategies)
                    combatEngine->addStrategy("offdps");
            }
            else
        ...
        case CLASS_PALADIN:
        {
            ...
            {
                combatEngine->addStrategies("holy", "dps assist", "healer caution", "flee", "ranged", NULL);
                if (sPlayerbotAIConfig.enableOffSpecStrategies)
                    combatEngine->addStrategy("offdps");
            }
            else
        ...
        case CLASS_DRUID:
        {
            ...
            if (tab == 1)
            {
                ...
                if (tanking)
                {
                    combatEngine->addStrategies("tank feral", "tank assist", "pull", "close", NULL);
                }
                else
                {
            ...
            {
                combatEngine->addStrategies("restoration", "dps assist", "healer caution", "flee", "ranged", NULL);
                if (sPlayerbotAIConfig.enableOffSpecStrategies)
                    combatEngine->addStrategy("offdps");
            }
            else
        ...
        case CLASS_DEATH_KNIGHT:
        {
            if (tab == 0)
            {
                combatEngine->addStrategies("blood", "tank assist", "pull", NULL);
            }
            else if (tab == 1)
            {
    ...
    // A companion is a hire, not an alt and not population, so it gets its
    // own string on top of whichever of the two above applied. This is how
    // companions are made quiet without silencing the owner's real alts.
    if (CompanionOwnership::IsCompanion(player->GetGUIDLow()))
        combatEngine->ChangeStrategy(sPlayerbotAIConfig.companionCombatStrategies);
...
void AiFactory::AddDefaultNonCombatStrategies(Player* player, PlayerbotAI* const facade, Engine* nonCombatEngine)
{
    ...
    switch (player->getClass())
    {
        case CLASS_PRIEST:
        {
            ...
            // Shackle Undead out of combat - see the mage case for why.
            nonCombatEngine->addStrategies("dps assist", "cc", "cure", "buff", "boost", NULL);
            break;
        }
        ...
        case CLASS_ROGUE:
        {
            ...
            // Sap out of combat - see the mage case. This one is not an
            // improvement but a repair: sap can ONLY be cast out of combat, so
            // without this a rogue could never sap anything at all.
            nonCombatEngine->addStrategies("dps assist", "cc", "poisons", "stealth", "buff", "boost", NULL);
            break;
        }
        ...
        case CLASS_WARLOCK:
        {
            ...
            // Banish out of combat - see the mage case.
            nonCombatEngine->addStrategies("dps assist", "cc", "pet", "buff", "boost", NULL);
            break;
        }
        ...
        case CLASS_MAGE:
        {
            ...
            // "cc" OUT OF COMBAT TOO, and this is the line that makes the
            // pre-pull sheep possible at all.
            //
            // The crowd control triggers were mirrored into each class's
            // InitNonCombatTriggers so a marked mob could be controlled during
            // a pull - the only time it is really worth doing, since a party
            // member is not in combat merely because a groupmate is. That
            // achieved nothing on its own: a strategy's non-combat triggers are
            // only installed when the strategy is in the NON-COMBAT engine, and
            // "cc" was added to the combat one alone for mage, priest, rogue
            // and warlock. Hunter, shaman and druid already carried it, which
            // is exactly why those worked and the mage did not.
            nonCombatEngine->addStrategies("dps assist", "cc", "cure", "buff", "boost", NULL);
            break;
        }
    ...
    // Same as the combat engine: the companion string goes on last. `+silent`
    // here is what stops the hello, the goodbye and the chit-chat; the pull
    // call-outs speak straight to the party and are not affected by it.
    if (CompanionOwnership::IsCompanion(player->GetGUIDLow()))
        nonCombatEngine->ChangeStrategy(sPlayerbotAIConfig.companionNonCombatStrategies);
