...
void PlayerbotFactory::Init()
{
    ...
    // [taxi-level cache] getAreaLevel() -> GetArea() -> getAreaFlag(), and
    // getAreaFlag() calls loadVMap() - so every node below pulls a VMap tile off
    // disk. Measured at 22s on a warm file cache and about 45s cold, which made
    // this the single largest block in start-up.
    //
    // The level depends only on map data, never on world state, so it is computed
    // once and kept. After a VMap/DBC update, force a recompute with:
    //     DELETE FROM tw_char.ai_playerbot_taxinode_level;
    //
    // DirectPExecute, not PExecute: the SELECT below must not run before the
    // table exists, and in this core a query against a missing table is a hard
    // crash (MySQLConnection::HandleMySQLError asserts), not a warning.
    CharacterDatabase.DirectPExecute(
        "CREATE TABLE IF NOT EXISTS `ai_playerbot_taxinode_level` "
        "(`node` int(10) unsigned NOT NULL, `level` int(11) NOT NULL, PRIMARY KEY(`node`))");

    std::unordered_map<uint32, int32> cachedTaxiNodeLevels;
    if (auto result = CharacterDatabase.PQuery("SELECT `node`, `level` FROM `ai_playerbot_taxinode_level`"))
    {
        do
        {
            Field* fields = result->Fetch();
            cachedTaxiNodeLevels[fields[0].GetUInt32()] = fields[1].GetInt32();
        } while (result->NextRow());
    }

    uint32 taxiNodesComputed = 0;
    ...
    for (uint32 i = 1; i < sTaxiNodesStore.GetNumRows(); ++i)
    {
        ...
        // getAreaLevel() is int32 and returns -1/-2 for an unknown area, while
        // Level is uint32 and always has been. Cache the signed value and let it
        // convert on the way out exactly as the direct assignment did, so the
        // cached path and the computed path stay bit-for-bit identical.
        int32 areaLevel;
        auto cached = cachedTaxiNodeLevels.find(i);
        if (cached != cachedTaxiNodeLevels.end())
        {
            areaLevel = cached->second;
        }
        else
        {
            areaLevel = taxiPosition.getAreaLevel();
            CharacterDatabase.DirectPExecute(
                "INSERT INTO `ai_playerbot_taxinode_level` (`node`, `level`) VALUES (%u, %d)",
                i, areaLevel);
            ++taxiNodesComputed;
        }

        taxiNodeLevel.Level = areaLevel;
    ...
    if (taxiNodesComputed)
        sLog.outString("Computed %u taxi node area levels from VMaps and cached them; later starts reuse the cache.",
            taxiNodesComputed);
}
...
void PlayerbotFactory::Randomize(bool incremental, bool syncWithMaster)
{
    ...
    // PROVISIONED, not random.
    //
    // These four hand a bot the consumables it would otherwise have to shop
    // for, and they were gated on isRandomBot -- which tests the RNDBOT account
    // prefix. A companion is on a COMPANION account on purpose, so it got no
    // ammo, no food, no potions and no reagents, at any level. No ammo is why a
    // warrior companion would not pull: the gate in
    // AttackAnythingAction::Execute counts real arrows.
    //
    // The item cheat is the honest test for "the server equips this bot":
    // random bots have it from RndBotCheats, companions are given it at login,
    // and the owner's own alts deliberately do not - they are real characters
    // with a real inventory and should not be handed free ammo.
    const bool provisioned = isRandomBot || (ai && ai->HasCheat(BotCheatMask::item));
    ...
    for(uint8 slot = 0; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        ...
        do
        {
            ...
            else
            {
                ...
                // THE QUALITY SEARCH ONLY EVER WALKS UPWARD.
                //
                // `quality` is rolled per slot (uncommon-to-rare at level 20-39)
                // and the loop above collects everything at that quality OR
                // BETTER. For most slots that is fine, because greens and blues
                // exist at every level. For a slot where this class simply has
                // no good options it comes back empty and the slot is left
                // bare - and the bot is never told, because an empty candidate
                // list is indistinguishable from "nothing worth upgrading".
                //
                // Measured 2026-09-06: a level 20 protection warrior has 4 poor
                // and 19 normal ranged weapons cached and NOTHING above that,
                // so a roll of uncommon or rare found zero every time. Arms and
                // fury each have exactly one uncommon and one rare, which is
                // why they sometimes got a bow and prot never did. In game:
                // "the tank will not pull", because InitAmmo returns early
                // without a ranged weapon and the pull gate wants ammo.
                //
                // So when the upward search finds nothing, look downward before
                // giving up. Only ever a fallback: a bot that found something at
                // its rolled quality still equips that, and an explicit
                // `.bot gear <quality>` is still honoured exactly.
                if (ids.empty() && !setQuality && quality > ITEM_QUALITY_POOR)
                {
                    for (uint32 q = ITEM_QUALITY_POOR; q < quality && ids.empty(); ++q)
                    {
                        uint32 currSearchLevel = searchLevel;
                        while (ids.empty() && currSearchLevel > 0)
                        {
                            for (uint32 id : sRandomItemMgr.Query(currSearchLevel, bot->getClass(), uint8(specId), slot, q))
                            {
                                ItemPrototype const* proto = sObjectMgr.GetItemPrototype(id);
                                if (proto && proto->ItemLevel <= maxItemLevel)
                                    ids.push_back(id);
                            }
        ...
        if (!found)
        {
            // LAST RESORT FOR THE RANGED SLOT.
            //
            // Everything above this point is a question of WHICH ranged weapon
            // is best, and a preference is allowed to come back empty. Owning
            // one at all is not: a warrior, hunter or rogue with an empty
            // ranged slot cannot pull, because InitAmmo returns immediately
            // with no weapon to feed and the opener has nothing to shoot. The
            // whole selection can miss for reasons that have nothing to do with
            // this bot -- a stale equip cache, a quality roll with no matching
            // items, the +/-9 level window in the filter above -- and every one
            // of those failures is silent.
            //
            // Worn Shortbow (2504): required level 1, item level 2, no bind, no
            // required skill, no class or race restriction, sold by vendors.
            // Deliberately the worst bow in the game. It only ever appears when
            // the alternative is an empty slot, and any real candidate found
            // above outranks it. Bows cover all three classes in vanilla.
            if (slot == EQUIPMENT_SLOT_RANGED &&
                (bot->getClass() == CLASS_WARRIOR || bot->getClass() == CLASS_HUNTER || bot->getClass() == CLASS_ROGUE) &&
                !bot->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_RANGED))
            {
                const uint32 fallbackRanged = 2504;

                uint16 eDest;
                if (RandomPlayerbotMgr::CanEquipUnseenItem(bot, slot, eDest, fallbackRanged) == EQUIP_ERR_OK)
                {
                    if (Item* pItem = bot->EquipNewItem(eDest, fallbackRanged, true))
                    {
                        pItem->SetOwnerGuid(bot->GetObjectGuid());
                        sLog.outDetail("Bot #%d %s:%d <%s>: nothing found for slot %d, fell back to Worn Shortbow",
                            bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", bot->GetLevel(), bot->GetName(), slot);
                        continue;
                    }
                }
            }
...
// Riding skills requirements are different
    ...
    // A WEAPON SKILL IS NOT THE ABILITY TO USE THE WEAPON.
    //
    // SetRandomSkill above grants a skill VALUE, which governs hit chance. What
    // actually fires a ranged weapon is a spell, and each one is paired with a
    // proficiency that nothing in the factory ever learned:
    //
    //     Bows      264   -> Shoot Bow       2480
    //     Guns      266   -> Shoot Gun       7918
    //     Crossbows 5011  -> Shoot Crossbow  7919
    //     Thrown    2567  -> Throw           2764
    //     Wands     5009  -> Shoot           5019
    //
    // CastShootAction resolves the spell from the equipped weapon's subclass
    // ("shoot gun" for a gun on vanilla), CanCastSpell answered no, and the
    // whole pull came back IMPOSSIBLE -- with a gun on the bot's back and 200
    // rounds in its bags. Measured 2026-09-06: not one of the 681 warriors on
    // this server knew any of these, at any level.
    //
    // Keyed off the skill the class was just given, so it follows whatever the
    // switch above decided rather than repeating the class list.
    static const struct { uint16 skill; uint32 proficiency; uint32 ability; } rangedWeapons[] =
    {
        { SKILL_BOWS,      264,  2480 },
        { SKILL_GUNS,      266,  7918 },
        { SKILL_CROSSBOWS, 5011, 7919 },
        { SKILL_THROWN,    2567, 2764 },
        { SKILL_WANDS,     5009, 5019 },
    };

    // Measured 2026-09-07, with a probe in this loop: every bot reaching here
    // ALREADY holds both spells - a freshly created level 20 warrior companion
    // logged `prof 266 had=1, ability 7918 had=1` before this ran. So this block
    // is defensive rather than the fix it was written as on 2026-09-06, and the
    // claim that "not one of the 681 warriors knew any of them" was reading
    // character_spell, which never holds them: they arrive as DEPENDENT spells
    // and `_SaveSpells` skips those. Kept because it costs nothing and a bot
    // that somehow lacks them cannot fire at all.
    for (const auto& ranged : rangedWeapons)
    {
        if (!bot->GetSkillValue(ranged.skill))
            continue;

        if (!bot->HasSpell(ranged.proficiency))
            bot->learnSpell(ranged.proficiency, false);

        if (!bot->HasSpell(ranged.ability))
            bot->learnSpell(ranged.ability, false);
    }
