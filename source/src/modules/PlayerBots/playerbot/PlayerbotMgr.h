...
protected:
    ...
    // True for the holder whose creates are COMPANIONS - hired by a player,
    // put on the server-owned COMPANION pool, counted against that player's
    // limit, and deleted on dismiss. False for the random-bot holder, whose
    // creates are population and belong to nobody.
    virtual bool CreatesCompanions() const { return false; }
    void Cleanup();   
private:
...
protected:
    ...
    // `.bot create` from a player hires a COMPANION: it goes on the
    // COMPANION0..N pool rather than the player's own account, which caps at 9
    // characters shared with their real ones.
    ...
    virtual bool CreatesCompanions() const override { return true; }

    void CheckTellErrors(uint32 elapsed);

    // Companions parked by this master's last logout: bring back the ones
    // still inside AiPlayerbot.CompanionParkMinutes, delete the rest. Runs
    // once, on the first update with the master in the world.
    void RestoreParkedCompanions();

private:
    Player* const master;
    PlayerBotErrorMap errors;
    time_t lastErrorTell;
    bool restoredCompanions;
};
