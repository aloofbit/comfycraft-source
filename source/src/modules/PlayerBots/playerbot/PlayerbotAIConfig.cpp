...
bool PlayerbotAIConfig::Initialize()
{
    ...
    EatDrinkMinDistance = config.GetFloatDefault("AiPlayerbot.EatDrinkMinDistance", 20.0f);
    EatDrinkMaxDistance = config.GetFloatDefault("AiPlayerbot.EatDrinkMaxDistance", 1000.0f);
    ...
    drinkAtMana = config.GetIntDefault("AiPlayerbot.DrinkAtMana", 95);
    eatAtHealth = config.GetIntDefault("AiPlayerbot.EatAtHealth", 95);

    // Clamped, not trusted: 0 is a division by zero in the sit duration and
    // anything over 100 finishes inside one tick, which just means a sit of
    // nothing. 2 is the DBC's own value and the no-op setting.
    botEatDrinkPercentPerSecond = config.GetIntDefault("AiPlayerbot.BotEatDrinkPercentPerSecond", 2);
    if (botEatDrinkPercentPerSecond < 1)
        botEatDrinkPercentPerSecond = 1;
    else if (botEatDrinkPercentPerSecond > 100)
        botEatDrinkPercentPerSecond = 100;

    // Not clamped at the bottom: 0 is a meaningful setting, it turns companion
    // self-revive off. Capped at ten minutes because past that the owner has
    // given up on the companion and dismissing it is the shorter road.
    companionReviveDelay = config.GetIntDefault("AiPlayerbot.CompanionReviveDelay", 30);
    if (companionReviveDelay > 600)
        companionReviveDelay = 600;
    ...
    companionParkMinutes = config.GetIntDefault("AiPlayerbot.CompanionParkMinutes", 15);
    ...
    companionCombatStrategies = config.GetStringDefault("AiPlayerbot.CompanionCombatStrategies", "");
    companionNonCombatStrategies = config.GetStringDefault("AiPlayerbot.CompanionNonCombatStrategies", "");
    companionTakesLoot = config.GetBoolDefault("AiPlayerbot.CompanionTakesLoot", true);
    companionArrivalDistance = config.GetFloatDefault("AiPlayerbot.CompanionArrivalDistance", 5.0f);
    ...
    groupJoinHelpHint = config.GetBoolDefault("AiPlayerbot.GroupJoinHelpHint", true);
    ...
    deferItemCache = config.GetBoolDefault("AiPlayerbot.DeferItemCache", false);
    ...
    // [startup-profile] Temporary instrumentation. Everything from here down to
    // LoadTalentSpecs() used to log as the single line "Loading spellIds.", and
    // measured 45 of the 110 seconds this server takes to reach "World server is
    // up and running" - by far the largest block, and completely unattributed.
    // Each of these is its own pass over the spell or item stores.
    // Remove once the slow one has been found and dealt with.
#define BOT_TIME_STEP(label, call)                                                  \
    {                                                                               \
        uint32 _profStart = WorldTimer::getMSTime();                                \
        call;                                                                       \
        sLog.outString("[startup] %-44s %6u ms", label,                             \
            WorldTimer::getMSTimeDiff(_profStart, WorldTimer::getMSTime()));        \
    }

    sLog.outString("Loading spellIds.");
    BOT_TIME_STEP("ChatHelper::PopulateSpellNameList",      ChatHelper::PopulateSpellNameList())
    BOT_TIME_STEP("ItemUsage::PopulateProfessionReagentIds", ItemUsageValue::PopulateProfessionReagentIds())
    BOT_TIME_STEP("ItemUsage::PopulateSoldByVendorItemIds",  ItemUsageValue::PopulateSoldByVendorItemIds())
    BOT_TIME_STEP("ItemUsage::PopulateReagentIdsForCraft",   ItemUsageValue::PopulateReagentItemIdsForCraftableItemIds())

    BOT_TIME_STEP("PlayerbotFactory::Init",                  PlayerbotFactory::Init())
    // ~5s of the start, and nothing needs it until something asks for a cached
    // item. Deferred, the first caller pays instead - see RandomItemMgr::EnsureInit,
    // which every public accessor on that class now opens with. Off by default, so
    // a normal start behaves exactly as it did; Start-Server.ps1 -Fast turns it on.
    if (deferItemCache)
        sLog.outString("[startup] RandomItemMgr::Init                        deferred");
    else
        BOT_TIME_STEP("RandomItemMgr::Init",                 sRandomItemMgr.Init())
    BOT_TIME_STEP("PlayerbotTextMgr::LoadBotTexts",          sPlayerbotTextMgr.LoadBotTexts())
    BOT_TIME_STEP("PlayerbotTextMgr::LoadBotTextChance",     sPlayerbotTextMgr.LoadBotTextChance())
    BOT_TIME_STEP("PlayerbotHelpMgr::LoadBotHelpTexts",      sPlayerbotHelpMgr.LoadBotHelpTexts())

    BOT_TIME_STEP("PlayerbotAIConfig::LoadTalentSpecs",      LoadTalentSpecs())
...
/*
 * BOT NPCs GET THEMSELVES LOGGED IN, from their own table.
 *
 * The first version leant on the module's "always online" event instead:
 * `.npcbot place` wrote always=ACTIVE, loadFreeAltBotAccounts above picked it
 * up at the next startup, and LoginFreeBots did the rest. It never once
 * survived a restart, and no startup ever logged "Enabling always online" for
 * one.
 *
 * THE EVENT STORE IS NOT OURS AND HAS ITS OWN LIFECYCLE.
 * RandomPlayerbotMgr::Remove deletes EVERY ai_playerbot_random_bots row for a
 * bot on its way out -- which is correct for a random bot being recycled, and
 * fatal for a flag that is supposed to mean "come back next time". Fixing that
 * would mean changing what Remove means for every bot on the server to make one
 * feature work.
 *
 * So persistence comes from tw_char.bot_npc, which this feature owns and which
 * says exactly the right thing already: a design with a character_guid is an NPC
 * that should be standing. One source of truth, and no flag anybody else is
 * entitled to clear.
 *
 * Pushed onto freeAltBots rather than logged in here, because here is far too
 * early -- this runs while the config is still loading, before the world exists.
 * LoginFreeBots is the thing that logs them in, on the random manager's update,
 * and it already handles a guid that is missing from the world.
 */
void PlayerbotAIConfig::loadBotNpcs()
{
    uint32 added = 0;

    for (const auto& itr : sBotNpcMgr.All())
    {
        BotNpcEntry const& npc = itr.second;
        if (!npc.IsPlaced())
            continue;

        uint32 accountId = sObjectMgr.GetPlayerAccountIdByGUID(
            ObjectGuid(HIGHGUID_PLAYER, npc.characterGuid));

        if (!accountId)
        {
            sLog.outError("Bot NPC %s is placed on character %u, which has no account. "
                          "It will not stand up.", npc.name.c_str(), npc.characterGuid);
            continue;
        }

        // A design made from somebody's own alt is already in the list from
        // loadFreeAltBotAccounts, and twice would start two logins for one guid.
        uint32 guid = npc.characterGuid;
        bool already = std::find_if(freeAltBots.begin(), freeAltBots.end(),
                                    [guid](std::pair<uint32, uint32> i) { return i.second == guid; })
                       != freeAltBots.end();
    ...
    if (added)
        sLog.outString("Standing up %u bot NPC(s).", added);
}
