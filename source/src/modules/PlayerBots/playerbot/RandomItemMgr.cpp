...
void RandomItemMgr::Init()
{
    ...
    // Set first, not last. BuildEquipCache and BuildItemInfoCache call our own
    // public accessors (CalculateStatWeight, CanEquipArmor, GetPlayerSpecId...),
    // every one of which now opens with EnsureInit() - so a flag set at the end
    // would recurse straight back in here.
    initialized = true;
