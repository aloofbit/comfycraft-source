...
class RandomItemMgr
{
	...
	public:
        ...
        // Init() builds eight caches off item_template and takes ~5s, and until
        // AiPlayerbot.DeferItemCache existed every start paid it whether or not a
        // bot ever appeared. Deferred, the first caller pays instead - so EVERY
        // public accessor below opens with EnsureInit(), including the ones AhBot
        // reaches with no bot in existence (PricingStrategy -> GetItemRarity).
        //
        // Both are idempotent, and the flag is set BEFORE the builders run: several
        // of them call these same accessors, which would otherwise re-enter here.
        void EnsureInit() { if (!initialized) Init(); }
