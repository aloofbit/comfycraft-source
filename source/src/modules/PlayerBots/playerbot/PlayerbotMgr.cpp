...
void PlayerbotHolder::HandlePlayerBotLoginCallback(QueryResult* /*dummy*/, SqlQueryHolder* holder)
{
    ...
    if (!bot || !bot->IsInWorld())
    {
        ...
        // A failed login used to be retried for ever, once per LoginFreeBots pass,
        // leaking one WorldSession each time. LoginFreeBots only drops a queue entry
        // on the pass where the bot EXISTS and is in world, so a bot that can never
        // log in is a permanent spin: measured 2026-09-06 at 170 errors and 170
        // leaked sessions inside three minutes, and it only ends at restart.
        //
        // That matters more than it looks, because `.bot create` is the companion
        // hire path: one flaky create and the server spins and leaks until someone
        // notices the log.
        //
        // Give up on this bot instead. Removing the queue entry is the whole fix
        // for the loop - the guid stays a perfectly good character and `.bot add`
        // will still log it in on demand, this just stops the unattended retrying.
        sPlayerbotAIConfig.freeAltBots.remove_if(
            [&](const std::pair<uint32, uint32>& entry)
            {
                return entry.second == info.botGuid.GetCounter();
            });

        // And close the leak. Two cases, and they need different handling:
        //
        //   _player unset - HandlePlayerLogin never got as far as building one, so
        //                   the session owns nothing and deleting it is all there is.
        //
        //   _player set but not in world - a half-loaded Player. ~WorldSession would
        //                   call LogoutPlayer(true) for us, but that SAVES, and
        //                   writing a partially loaded character back over a good row
        //                   is a worse bug than the leak. LogoutPlayer(false) does the
        //                   same teardown - out of HashMapHolder, session detached -
        //                   without the save, and it clears _player, so the
        //                   destructor then has nothing left to do.
...
void PlayerbotHolder::LogoutPlayerBot(uint32 guid, bool allowInstant, bool forDelete)
{
    ...
    if (bot)
    {
        ...
        else if (bot && (logout || !botWorldSessionPtr->isLogingOut()))
        {
            ...
            playerBots[guid] = nullptr;    // deletes bot player ptr inside this WorldSession PlayerBotMap
            // LogoutPlayer(false) when we are about to delete the character.
            //
            // DeleteBot is logout-then-Player::DeleteFromDB, and both halves go
            // out as async PExecute on four unordered worker threads. The save
            // this call used to make is a REPLACE INTO characters, so whenever
            // it landed AFTER the delete it put the character straight back.
            // Measured 2026-09-06: three live bots deleted, two of them still
            // in `characters` minutes later, delete reporting "ok" every time.
            //
            // The `if (!forDelete)` guard above already suppressed the module's
            // OWN save for this reason; this is the same save arriving through
            // the core's logout path, which that guard never covered.
            botWorldSessionPtr->LogoutPlayer(!forDelete); // this will delete the bot Player object and PlayerbotAI object
...
void PlayerbotHolder::OnBotLogin(Player * const bot)
{
    ...
    // Companions feed themselves through the item cheat.
    //
    // A companion is generated on hire and deleted on dismiss: it never shops,
    // never loots reagents and never buys ammo. The factory would have handed
    // it food, potions and ammo, but InitFood/InitPotions/InitAmmo are all
    // gated on `isRandomBot`, which tests the RNDBOT account prefix - and a
    // COMPANION account is deliberately not in that list. So a companion came
    // out with the level 1 Tough Jerky from character creation and nothing
    // else, at any level.
    //
    // Rather than un-gate the factory, give companions the item cheat. Eating
    // and drinking then run through the generic Food (24005) and Drink (24355)
    // auras - 2% of max health/mana per second for 25s/30s - which is
    // level-independent, needs no bag space and no vendor. It also closes the
    // ammo gap that stopped a warrior companion auto-pulling at all
    // (ChooseTargetActions.cpp:68).
    //
    // AiPlayerbot.BotCheats is deliberately NOT the place for this: that
    // setting also covers the owner's own alts run with `.bot add`, and those
    // are real characters with a real inventory.
    if (CompanionOwnership::IsCompanion(bot->GetGUIDLow()))
        ai->SetCheat(BotCheatMask((uint32)ai->GetCheat() | (uint32)BotCheatMask::item));
...
std::string PlayerbotHolder::ProcessBotCommand(std::string cmd, ObjectGuid guid, ObjectGuid masterguid, bool admin, uint32 masterAccountId, uint32 masterGuildId, const std::string param)
{
    ...
    // Companions live on the server-owned COMPANION pool, so they are on
    // neither a random account nor yours, and every check below would refuse
    // them - including the one that made `.bot delete` on your own freshly
    // hired companion answer "Not in your guild or account".
    //
    // THIS GATE IS THE ONE THAT MATTERS: it runs before any handler, so an
    // ownership test inside HandleBotDelete alone never gets a turn. That was
    // the bug.
    bool isMyCompanion = masterguid
                      && CompanionOwnership::GetOwner(guid.GetCounter()) == masterguid.GetCounter();

    if (!isRandomAccount && !isMyCompanion && (!isMasterAccount && !admin && masterguid))
    {
...
PlayerbotMgr::PlayerbotMgr(Player* const master) : PlayerbotHolder(),  master(master), lastErrorTell(0), restoredCompanions(false)
{
}
...
void PlayerbotMgr::UpdateAIInternal(uint32 elapsed, bool minimal)
{
    ...
    // Companions removed from the party, dismissed here rather than in the
    // group hook that noticed. PlayerbotGroupScript queues them because it
    // fires from inside Group::RemoveMember, and deleting a Player from there
    // re-enters the very function that is still unwinding.
    if (master)
    {
        for (uint32 guid : CompanionOwnership::TakeQueuedDismissals(master->GetGUIDLow()))
            DeleteBot(ObjectGuid(HIGHGUID_PLAYER, guid));

        // After the dismissals, so a companion removed from the party and then
        // logged out on is deleted here rather than brought back below. Once
        // the master is in the world, not at login: the relog goes through
        // LoginFreeBots on the random manager, which needs the master in its
        // real-player map and a map to teleport to.
        if (!restoredCompanions && master->IsInWorld())
        {
            restoredCompanions = true;
            RestoreParkedCompanions();
        }
    }
...
// Companions go on a server-owned COMPANION0..N pool, walked and grown lazily,
// exactly the way RandomPlayerbotMgr walks RNDBOT0..N. NOT the player's own
// account: that caps at 9 characters shared with their real ones, so three
// companions would cost a third of somebody's roster.
//
// And NOT the RNDBOT pool either, tempting as reuse looks. That list is how
// RandomPlayerbotMgr decides what it owns and may recycle; a companion swept
// into it would be re-rolled or logged out from under its owner.
uint32 PlayerbotMgr::GetOrCreateAccount(Player* master, std::string& error)
{
    if (!master)
    {
        error = "Companions need a player to belong to.";
        return 0;
    }
    ...
    for (uint32 accountNumber = 0; accountNumber < 10000; ++accountNumber)
    {
        std::ostringstream accountNameStr;
        accountNameStr << CompanionOwnership::ACCOUNT_PREFIX << accountNumber;
        const std::string accountName = accountNameStr.str();
        ...
        if (!accountId)
        {
...
#ifndef MANGOSBOT_ZERO
            AccountOpResult result = sAccountMgr.CreateAccount(accountName, accountName, MAX_EXPANSION);
#else
            ...
            if (result != AOR_OK)
            {
                error = "Failed to create a companion account.";
                return 0;
            }
            ...
            sLog.outString("[Companions] Created pool account %s (id %u).", accountName.c_str(), accountId);
        }

        if (sAccountMgr.GetCharactersCount(accountId) < maxCharsPerAccount)
        {
            CompanionOwnership::NoteAccount(accountId);
            return accountId;
        }

        // Full: walk on to the next one.
    }

    error = "Failed to find a free companion account.";
    return 0;
}

// Dismissing a companion must forget it, or its owner keeps paying for it
// against their limit and the startup purge would chase a character that is
// already gone.
void PlayerbotMgr::OnBotDeleted(uint32 botGuid, uint32 accountId)
{
    // Drop it from the random manager's real-player map before the character
    // stops existing. MovePlayerBot no longer puts companions there, but an
    // entry from an older binary - or any future route in - would be a
    // use-after-free the next time HasPlayerRelation walks it.
    sRandomPlayerbotMgr.ForgetTrackedPlayer(botGuid);

    CompanionOwnership::Unregister(botGuid);
    PlayerbotHolder::OnBotDeleted(botGuid, accountId);
}

// The other half of CompanionOwnership::ParkAll. A logout parks the companions
// instead of dismissing them; this is what happens to them when their owner
// comes back.
//
// A companion inside the window is logged back in THE WAY A HIRE IS: pushed
// onto freeAltBots with a "create group" value naming the master, so
// RandomPlayerbotMgr::LoginFreeBots logs it in, has it join the master's
// party, teleports it to them and hands it to this manager. Reusing that path
// means a returning companion is not a second way of coming online that could
// drift from the first - and it already copes with the two states a parked
// companion can be in: still in the party (a dropped connection leaves it
// there, offline) or evicted from it (a clean logout).
//
// One outside the window is deleted here. DeleteBot handles an offline
// character - it skips the logout and goes straight to DeleteFromDB - and
// OnBotDeleted unregisters it.
void PlayerbotMgr::RestoreParkedCompanions()
{
    if (!master || !sPlayerbotAIConfig.enabled)
        return;

    const std::vector<CompanionOwnership::Parked> parked =
        CompanionOwnership::ParkedOf(master->GetGUIDLow());
    if (parked.empty())
        return;

    const time_t now    = time(nullptr);
    const time_t window = time_t(sPlayerbotAIConfig.companionParkMinutes) * MINUTE;
    ChatHandler chat(master);

    for (const CompanionOwnership::Parked& p : parked)
    {
        const ObjectGuid guid(HIGHGUID_PLAYER, p.companionGuid);

        std::string name;
        if (!sObjectMgr.GetPlayerNameByGUID(guid, name))
            name = "A companion";

        if (now - p.since > window)
        {
            DeleteBot(guid);
            chat.PSendSysMessage("%s went home while you were away.", name.c_str());
            continue;
        }

        // Still online: a logout that never got as far as logging the bot
        // out. Nothing to relog, so just take the stamp off; the AI adopts
        // its master from the party on its own.
        if (sObjectMgr.GetPlayer(guid))
        {
            CompanionOwnership::Unpark(p.companionGuid);
            continue;
        }

        if (!p.accountId)
        {
            sLog.outError("RestoreParkedCompanions: no account for companion %u of %s, deleting it.",
                          p.companionGuid, master->GetName());
            DeleteBot(guid);
            continue;
        }
        ...
        // Never queue the same guid twice: LoginFreeBots would start two
        // logins for it, and the second is the "already ingame" kick.
        const bool queued = std::any_of(
            sPlayerbotAIConfig.freeAltBots.begin(), sPlayerbotAIConfig.freeAltBots.end(),
            [&](const std::pair<uint32, uint32>& entry) { return entry.second == p.companionGuid; });

        if (!queued)
        {
            sRandomPlayerbotMgr.SetValue(p.companionGuid, "create group", 1, master->GetName());
            sPlayerbotAIConfig.freeAltBots.push_back(std::make_pair(p.accountId, p.companionGuid));
        }

        chat.PSendSysMessage("%s is on the way back.", name.c_str());
    }
}
...
{    
    ...
    // A player may only have so many companions out. The party cap of 5 bounds
    // a group on its own, so this exists to stop one person in a duo taking
    // three of the five seats.
    if (master && CreatesCompanions())
    {
        const uint32 have = CompanionOwnership::CountFor(master->GetGUIDLow());
        if (have >= CompanionOwnership::MAX_PER_PLAYER)
        {
            std::ostringstream out;
            out << "You already have " << have << " companion(s). Dismiss one first.";
            messages.push_back(out.str());
            return;
        }
    }
    ...
    WorldSession* botSession = new WorldSession(accountId, NULL, SEC_PLAYER,
        ...
        if (!groupWith.empty())
        {
            // SAY SO IF THAT NAME IS NOBODY.
            //
            // The invite is deferred: it fires when the bot logs in, from
            // RandomPlayerbotMgr, which resolves the name with
            // FindPlayerByName and simply does nothing when it fails. So
            // "group=" pointing at a player who is not online - or at a
            // placeholder somebody pasted out of a macro without editing it -
            // produced a bot that was created, logged in, and silently not in
            // anyone's group, with no way to tell that from a broken invite.
            //
            // groupWith defaults to the master's own name, so the common case
            // needs no argument at all; this only fires when one was given.
            if (!sObjectAccessor.FindPlayerByName(groupWith.c_str()))
            {
                messages.push_back("No player called '" + groupWith + "' is online - creating " + name + " ungrouped.");
            }
        ...
        // A COMPANION IS NOT CREATED AT ITS OWNER'S FEET.
        //
        // It used to be, and the arrival was three visible steps on one spot:
        // the bot logs in on top of the player in starter clothes, LoginFreeBots
        // re-rolls its talents and gear where everyone can see, then teleports
        // it - to the same place. Leaving the character where character
        // creation put it, the race's start zone, means the fix-ups happen out
        // of sight and the teleport in LoginFreeBots is the one arrival: geared,
        // and beside the owner rather than on them. `.bot create` for an alt
        ...
        // Ownership is recorded BEFORE the login is queued. A companion that
        // fails to come online still has a character row, and without a row
        // here nothing would know whose it was or that it should be cleaned up.
        if (master && CreatesCompanions())
            CompanionOwnership::Register(master->GetGUIDLow(), botGuid, accountId);
...
bool PlayerbotHolder::DeleteBot(ObjectGuid guid, bool allowInstant)
{
    ...
    {
        // A DEAD BOT IS BUSY. Put it back on its feet before deleting it.
        //
        // A dead bot runs the revive machinery - FindCorpseAction walks the
        // FleeManager to work out where its corpse is, which reaches deep into
        // the AI object context. Delete the bot from under that and the next
        // tick dereferences a freed context:
        //
        //   PlayerbotAI::UpdateAI -> Engine::ListenAndExecute
        //     -> FindCorpseAction::Execute -> FleeManager::CalculateDestination
        //     -> MoveStyleValue::HasValue -> NamedObjectContext::GetObjectA
        //     -> SEH 0xc0000005
        //
        // That killed the server on 2026-09-06 when a companion was uninvited
        // while dead - which is a perfectly ordinary thing to do after a wipe,
        // and the one state in which dismissing had never been tried.
        //
        // Resurrecting first is not cosmetic: it takes the bot out of the
        // corpse state so none of that machinery is live, and clears the
        // corpse, which would otherwise outlive the character we are about to
        // delete.
        if (!player->IsAlive())
        {
        ...
        // Kill the pet FIRST, and kill it as deleted rather than saved.
        //
        // A hunter or warlock companion writes character_pet and pet_spell the
        // moment it summons, and every later save - logout included - writes
        // them again. Those writes are async PExecute on four unordered
        // workers, so one landing after DeleteFromDB's transaction leaves
        // pet_spell rows behind with no character_pet to find them by. Measured
        // 2026-09-06: 43 orphan pet_spell rows from a handful of hires, even
        // with the delete-order fix already in.
        //
        // PET_SAVE_AS_DELETED is "not saved in fact" - it removes the rows and
        // guarantees nothing is written afterwards, which is the only version
        // of this that does not depend on winning a race. It matters because
        // pet guids are reused and pet_spell keys PRIMARY on guid, so an orphan
        // here is a future abort, not just clutter.
        if (player->GetPet())
        ...
        // LogoutPlayerBot is a no-op when THIS holder does not hold the bot -
        // it opens with `if (Player* bot = GetPlayerBot(guid))` and silently
        // falls through otherwise. Deleting the character regardless is what
        // leaves a bot standing in the world with no row behind it, so make
        // sure somebody actually logged it out before we delete.
        if (sObjectMgr.GetPlayer(guid, true) && this != &sRandomPlayerbotMgr)
            sRandomPlayerbotMgr.LogoutPlayerBot(guid.GetCounter(), allowInstant, true);
    }

    Player::DeleteFromDB(guid, botAccount, true, true);

    // The bot value store is keyed by character low guid and is NOT part of
    // DeleteFromDB, so it survives the character and is inherited by whoever
    // reuses the guid. See RandomPlayerbotMgr::ForgetBot for what that cost.
    sRandomPlayerbotMgr.ForgetBot(guid.GetCounter());
...
std::string PlayerbotHolder::HandleBotDelete(Player* bot, Player* master, const std::string param)
{
    ...
    // A companion is on neither a random-pool account nor the player's own, so
    // the two tests below would both refuse it. Ownership for those lives in
    // companion_owner - see CompanionOwnership.
    const uint32 companionOwner = CompanionOwnership::GetOwner(guid.GetCounter());
    if (companionOwner)
    {
        if (!master || master->GetGUIDLow() != companionOwner)
            return "Not your companion";
