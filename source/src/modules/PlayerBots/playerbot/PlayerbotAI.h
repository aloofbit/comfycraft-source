...
{
public:
    // When the MASTER last told this bot to go and hit something.
    //
    // A deliberate order has to outlast the tick it was typed on. FleeStrategy
    // fires at ACTION_EMERGENCY + 9, so a standing panic outranks a fresh
    // `attack` and the order appears to be ignored until it is typed a second
    // time. That breaks the ordinary dungeon body-pull - walk in, back off,
    // stop, fight where you chose - which depends on `attack` taking hold the
    // first time.
    void NoteMasterAttackOrder() { m_masterAttackOrder = time(nullptr); }
    bool HasRecentMasterAttackOrder(uint32 seconds) const
    { return m_masterAttackOrder && (time(nullptr) - m_masterAttackOrder) < (time_t)seconds; }
...
public:    
    ...
    // False for a bot NPC, which never volunteers chat by any route. Asked at
    // the top of every emitter below; PlayerbotAI.cpp has the reasoning and the
...
public:
    ...
    // `follow` holds fire for a moment and then lets go; see FollowChatShortcutAction.
    void FadePassiveAfter(uint32 ms);
    void CancelPassiveFade() { passiveFadeStartMs = 0; }
    void SetFallAfterJump() { fallAfterJump = true; }
    void SetJumpTime(uint32 time) { jumpTime = time; }
    bool CanMove();
    // True while the core is carrying the bot along a charge spline. The
    // movement actions must not start anything of their own until it lands,
    // because MotionMaster::Mutate interrupts whatever is on top.
    bool IsCharging();
    void StopMoving();
    bool IsInRealGuild();
    ...
    // Think on the next tick instead of waiting out whatever delay the last
    // action left behind.
    //
    // Actions set an AI update delay when they finish - a global cooldown, an
    // rpg or travel wait, up to several seconds - and the bot does not evaluate
    // anything until it expires. For an order the player has just typed that
    // reads as the bot ignoring them: OnCombatStarted only clears the delay if
    // the bot happens to be sitting, so a pull given to a bot mid-anything sat
    // there until the previous action's timer ran out.
    void ReactImmediately() { ResetAIInternalUpdateDelay(); }
    void OnCombatEnded();
    void OnDeath();
    void OnResurrected();
...
protected:
    ...
    uint32 jumpTime;
    bool fallAfterJump;
    uint32 passiveFadeStartMs = 0; // when a follow set its passive, 0 = no fade pending
    uint32 passiveFadeDurationMs = 0;
    uint32 lootingSinceMs = 0; // when the loot watchdog first saw UNIT_FLAG_LOOTING, 0 = not looting
