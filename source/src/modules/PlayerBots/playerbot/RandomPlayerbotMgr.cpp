...
void RandomPlayerbotMgr::LoginFreeBots()
{
    if (!sPlayerbotAIConfig.freeAltBots.empty() && sPlayerbotAIConfig.botAutologin != BotAutoLogin::LOGIN_ONLY_ALWAYS_ACTIVE)
    {
        ...
        for (auto [accountId, botGuid] : sPlayerbotAIConfig.freeAltBots)
        {
            ...
            {
                /*
                 * A BOT NPC IS NEVER RE-GEARED, RE-LEVELLED OR DRAGGED TO A
                 * MASTER. Its outfit is the entire reason it exists -- the whole
                 * point of an NPC that is a Player is that it wears what it was
                 * dressed in, cape and all -- so a factory pass over it is
                 * destructive, not helpful.
                 *
                 * Skipping early rather than guarding each block: everything
                 * below is a "make this bot into a fresh adventurer" step, and
                 * an NPC wants none of them.
                 */
                if (bot->IsBotNpc())
                {
                    /*
                     * IT PUTS ITS CLOTHES ON HERE, and this is the right place
                     * for it precisely because it is the branch for a bot that
                     * is ALREADY IN WORLD -- fully loaded, inventory and all.
                     *
                     * OnBotLogin was the obvious home and was wrong: the
                     * inventory is not loaded there, so every slot looked empty,
                     * every piece was re-equipped from scratch, and each login
                     * left the previous item_instance orphaned. Thirty-two of
                     * them had piled up on one NPC before anybody counted.
                     *
                     * Safe to run on every pass now that dressing skips a slot
                     * whose DISPLAY already matches, before it queries anything.
                     * So the steady state is a handful of pointer compares, and
                     * a look changed on the website while the NPC was logged out
                     * is picked up the moment it is back.
                     */
                    /*
                     * IsInWorld, and it is load-bearing. A bot is reachable
                     * through sObjectMgr::GetPlayer BEFORE its inventory has
                     * loaded, so the first pass after a login saw every slot
                     * empty, re-equipped the whole outfit from scratch, and
                     * left the previous eight items orphaned in item_instance.
                     * Forty had piled up on one NPC that way.
                     *
                     * "All slots empty" cannot be told apart from "not loaded
                     * yet" by looking at the slots, so the question has to be
                     * asked of the login instead.
                     */
                    if (BotNpcEntry const* npc = bot->IsInWorld() ? sBotNpcMgr.Get(botGuid) : nullptr)
                    {
                        /*
                         * The bracket line needs no help on an ordinary login --
                         * it is a guild membership, `guild_member` persists it,
                         * and Player::_LoadGuild restores it before anything
                         * here runs. This covers the one case that is left: a
                         * subname edited on the website while the server was
                         * down, where the character comes back wearing the old
                         * one and nothing would otherwise notice.
                         *
                         * Free when unchanged -- ApplySubname compares the guild
                         * it is already in and returns.
                         */
                        BotNpc_ApplySubname(bot, npc->subname);

                        // Re-asserted here, not merely set at login:
                        // InitStatsForLevel wipes these flags and a bot NPC has
                        // no aura to put them back.
                        BotNpc_ApplyInvulnerability(bot, true);

                        if (!npc->look.empty() && npc->look != npc->lookApplied)
                        {
                            std::string error;
                            std::string want = npc->look;

                            if (BotNpc_Dress(bot, want, error))
                            {
                                bot->SaveToDB();
                                sBotNpcMgr.MarkLookApplied(npc->id, want);
                            }
                            else
                            {
                                sLog.outError("Bot NPC %s could not be dressed: %s",
                                              bot->GetName(), error.c_str());
                            }
                        }
                    }
                    continue;
                ...
                if (sRandomPlayerbotMgr.GetValue(botGuid, "create gear"))
                {
                    ...
                    // AMMO WAS BOUGHT FOR THE WEAPON THE PREVIOUS PASS PICKED,
                    // AND THIS BLOCK JUST PICKED A DIFFERENT ONE.
                    //
                    // `create levelup` runs Randomize a few lines above, which
                    // equips a ranged weapon and then calls InitAmmo for it.
                    // Every branch here re-rolls the equipment afterwards -
                    // including the `else`, which is what a companion hire takes,
                    // since CreateBot defaults gear to "default" and nothing
                    // matches it. Nothing revisited the ammo, so a bot handed a
                    // bow in the first pass and a gun in the second kept 200
                    // arrows it cannot load.
                    //
                    // That is not a soft failure. Spell::CheckCast rejects a
                    // ranged attack whose ammo subclass does not match the weapon
                    // with SPELL_FAILED_NO_AMMO, so CanCastSpell says no,
                    // PullAction::isPossible says no, and the pull silently
                    // becomes a run-in - measured on a level 20 protection
                    // warrior companion carrying a Slag Slugger and Sharp Arrows.
                    //
                    // Gated the way PlayerbotFactory::Randomize gates the same
                    // call: the owner's own alts are real characters and do not
                    // get free ammo.
                    PlayerbotAI* botAi = GetBotAI(bot);
                    if (IsRandomBot(bot) || (botAi && botAi->HasCheat(BotCheatMask::item)))
                    {
                        PlayerbotFactory factory(bot, bot->GetLevel());
                        factory.InitAmmo();
                    }

                    // AND CLEAR IT, which it never did. `create levelup` above,
                    // `create group` below and `test` below that all zero their
                    // value once acted on; this one did not, so every pass of
                    // LoginFreeBots re-rolled the equipment of every free alt
                    // bot in the world, for as long as it stayed logged in.
                    //
                    // Invisible until now only because nothing stayed logged in
                    // long enough to notice: a companion is dismissed, an alt
                    // logs out. The first bot NPC stood still and visibly
                    // changed clothes on a loop, which is what this looks like
                    // from the outside.
                    //
                    // Not a bot-NPC fix -- they skip this block entirely a few
                    // lines up. This is the underlying bug, and companions were
                    // paying it too.
                ...
                if (!IsRandomBot(bot) && GetPlayerBot(guid)) //Place bot in player manager.
                {
                    for (auto& [mGuid, master] : players)
                    {
                        ...
                        // Account equality was the only test here, which stopped
                        // working the moment companions moved to the server-owned
                        // COMPANION pool: a companion's account is never its
                        // owner's, so it was never handed to their PlayerbotMgr
                        // and stayed in the random manager's map. `.bot delete`
                        // then ran LogoutPlayerBot on a holder that did not have
                        // the bot, skipped the logout in silence, and deleted the
                        // character anyway - leaving it standing in the world with
                        // no row behind it.
                        const bool ownsIt = (accountId == sObjectMgr.GetPlayerAccountIdByGUID(masterGuid))
                                         || (CompanionOwnership::GetOwner(botGuid) == mGuid);
                ...
                if (master)
                {
                    // A companion lands BESIDE its owner, not on them: a random
                    // bearing at five yards, through the same close-point search
                    // a summoned hunter pet uses, so it comes down on walkable
                    // ground and not inside a wall. This is the one arrival a
                    // companion makes - it was created in its start zone and
                    // fixed up there (CreateBot) - and the same landing serves
                    // a parked companion coming back after its owner's relog.
                    if (CompanionOwnership::IsCompanion(botGuid))
                    {
                        const float angle = frand(0.0f, 2.0f * M_PI_F);
                        float x, y, z;
                        master->GetClosePoint(x, y, z, bot->GetObjectBoundingRadius(), sPlayerbotAIConfig.companionArrivalDistance, angle);
                        bot->TeleportTo(master->GetMapId(), x, y, z, master->GetAngle(x, y) + M_PI_F);
                    }
                    else
                ...
                /*
                 * A BOT NPC IS NEVER DROPPED FROM THE LIST. It is kept online by
                 * tw_char.bot_npc rather than by the "always" event -- the event
                 * store is recycled by RandomPlayerbotMgr::Remove, which is why
                 * leaning on it never survived a restart -- so the test below
                 * would throw every bot NPC out on its first pass. They would
                 * stay standing, having already logged in, and then never come
                 * back the first time one logged out.
                 */
                if (!bot->IsBotNpc())
                {
...
void RandomPlayerbotMgr::MovePlayerBot(uint32 guid, PlayerbotHolder* newHolder)
{
    ...
    // `players` is the REAL PLAYERS the random bot system tracks, and
    // HasPlayerRelation walks it calling GetSocial() - which asserts rather
    // than returning null. A companion must never go in:
    //
    //  - it is not a real player, and
    //  - it is DELETED on dismiss, which leaves a dangling pointer here that
    //    the next HasPlayerRelation dereferences.
    //
    // This only ever fired for alt-bots on the master's own account before
    // companions existed, which is why it survived this long. Handing every
    // companion to its owner's manager made it reach this line every time, and
    // crashed the server on the create after a dismiss.
    if (!CompanionOwnership::IsCompanion(guid))
...
bool RandomPlayerbotMgr::ProcessBot(Player* player)
{
    ...
    // The scheduled pass re-rolls AND teleports idle random bots, and a bot NPC
    // on an RNDBOT account counts as one. Its "randomize" event is set on first
    // login, so without this it would change clothes and wander off hours later.
    if (player && player->IsBotNpc())
        return false;
...
{
    /*
     * A BOT NPC IS NOT A FRESH RANDOM BOT, even though it looks exactly like one
     * to OnBotLogin: `.npcbot place` makes its body on an RNDBOT account, and a
     * body that has never logged in has zero played time. So its very first
     * login rolled a full random loadout and a random teleport, and the NPC
     * stood up in somebody else's clothes. A second place did not, because by
     * then the played time was no longer zero -- which is what made it look
     * like placing only worked the second time.
     */
    if (bot && bot->IsBotNpc())
        return;
...
{
    // Every re-roll comes through here or UpdateGearSpells, and an NPC's outfit
    // is the one thing about it that must never be re-rolled. InstaRandomize.
    if (bot && bot->IsBotNpc())
        return;
...
{
    // See InstaRandomize: an NPC's outfit is never re-rolled.
    if (bot && bot->IsBotNpc())
        return;
...
// Forget a guid that must never be dereferenced again. Called when a companion
// is deleted: the character is about to stop existing, and a stale entry in
// `players` is a use-after-free the next time HasPlayerRelation runs.
void RandomPlayerbotMgr::ForgetTrackedPlayer(uint32 guid)
{
    players.erase(guid);
}
...
void RandomPlayerbotMgr::ForgetBot(uint32 guid)
{
    // CHARACTER LOW GUIDS ARE REUSED, AND THIS TABLE OUTLIVES THE CHARACTER.
    //
    // The only sweep for ai_playerbot_random_bots is the `bot NOT IN (SELECT
    // guid FROM characters)` delete in RandomPlayerbotFactory, and that runs at
    // STARTUP. A companion dismissed mid-session leaves its rows behind, and the
    // next hire handed the same guid inherits them.
    //
    // `specNo` is the one that bites: SetEventValue exempts it and `specLink`
    // from expiry on purpose, and ChangeTalentsAction::AutoSelectTalents opens
    // with `if (specNo > 0)` and continues that spec WITHOUT looking at the
    // requested role. So a companion hired as a tank came back fury because the
    // guid's previous occupant was fury - measured 2026-09-07 on guid 4645,
    // which still carried rows stamped seven hours before the character existed.
    // 233 rows across 59 dead guids had accumulated, 36 of them specNo.
    //
    // Not owner-scoped, unlike Remove() above: a deleted character owns nothing
    // any more, and half a purge is what left this behind.
    CharacterDatabase.PExecute("DELETE FROM ai_playerbot_random_bots WHERE bot = '%u'", guid);
