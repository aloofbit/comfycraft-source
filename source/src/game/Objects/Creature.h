...
/*
 * WHAT A VENDOR ROW COSTS, when it is not simply the item's own buy_price.
 *
 * 1.12 HAS NO ExtendedCost -- that is TBC -- so this is ours. A cost is the
 * WHOLE price: money in copper plus up to three item requirements, and
 * `costId` 0 on a VendorItem means "whatever the item is worth", which is every
 * row that existed before sql/custom/094.
 *
 * money 0 with no items is free, and is now a thing a shop can say on purpose
 * rather than a thing that happens to an item priced at 0.
 *
 * THE CLIENT CANNOT BE TOLD ABOUT THE ITEMS. SMSG_LIST_INVENTORY carries seven
 * uint32 per row and none of them is an item cost, so the vendor window shows
 * the MONEY and nothing else. The server refuses the purchase and says what is
 * missing; drawing it properly needs an addon, the way addon/ComfyNPC already
 * repaints a tooltip the server cannot reach.
 */
#define MAX_VENDOR_COST_ITEMS 3
...
struct VendorCost
{
    ...
    uint32 money;
    uint32 item[MAX_VENDOR_COST_ITEMS];
    uint32 count[MAX_VENDOR_COST_ITEMS];

    bool WantsItems() const
    {
        for (int i = 0; i < MAX_VENDOR_COST_ITEMS; ++i)
            if (item[i] && count[i])
                return true;
...
struct VendorItem
{
    VendorItem(uint32 _item, uint32 _maxcount, uint32 _incrtime, uint32 _itemflags, uint32 _conditionId, uint32 _costId)
        : item(_item), maxcount(_maxcount), incrtime(_incrtime), itemflags(_itemflags),
          conditionId(_conditionId), costId(_costId) {}
    ...
    uint32 costId;                                          // vendor_cost.id, 0 = the item's own buy_price
};
...
struct VendorItemData
{
    ...
    void AddItem(uint32 item, uint32 maxcount, uint32 ptime, uint32 itemflags, uint32 conditonId, uint32 costId)
    {
        m_items.push_back(new VendorItem(item, maxcount, ptime, itemflags, conditonId, costId));
    }
