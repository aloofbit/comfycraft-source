...
/*
 * HOW MUCH OF A LIMITED-STOCK ITEM A VENDOR HAS LEFT, and when it last
 * restocked. One entry per item that has actually been bought; an item at full
 * stock has no entry at all, which is why both methods below treat "not found"
 * as maxcount rather than as zero.
 *
 * IT LIVES ON Unit, NOT ON Creature, and it used to live on Creature. That was
 * never a fact about the world -- it was where the vector happened to be put
 * back when only creatures could sell things. A BOT NPC IS A PLAYER THAT SELLS
 * THINGS (BotNpc.h), and moving these three declarations down one class is the
 * whole of what it took to let one count stock the way a creature does. Neither
 * method body changed: they touch this vector, sObjectMgr, sWorld and urand,
 * and nothing else.
 *
 * NOT SAVED ANYWHERE. Stock is in memory and a restart refills every vendor in
 * the world, which is upstream's behaviour and not something this change
 * touched.
 */
...
// Defined in Creature.h, which includes this file. Only ever used through a
// pointer here, so the declaration is enough and the include stays one-way.
struct VendorItem;
...
class Unit : public WorldObject
{
    ...
    public:
        ...
        // A machine-driven player (a bot) that a spline just left in the air
        // is stepped down to the ground. Runs whenever a spline ends or is
        // stopped; a no-op for anything else. See the definition for why.
        void SettleDrivenPlayerOnGround();
    ...
    public:
        /*
         * LIMITED STOCK. Moved down from Creature so a bot NPC -- a player that
         * sells things -- gets the same restocking a creature gets. See the note
         * on VendorItemCount above this class.
         *
         * A vendor's stock LIST is still resolved by the caller, because that is
         * the part that genuinely differs: a creature's comes from its entry, a
         * bot NPC's from the shop id on its design row. Only the COUNTING is
         * shared, and counting never asked what kind of thing it was on.
         */
    ...
    protected:
        ...
        // One entry per limited-stock item that has been bought from this unit.
        // Empty for the overwhelming majority of units, which sell nothing.
