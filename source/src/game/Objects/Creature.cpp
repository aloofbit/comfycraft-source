...
void Creature::SelectLevel(const CreatureInfo *cinfo, float percentHealth, float percentMana)
{
    ...
    // NEVER ZERO. A rate below 1 truncates a 1 HP critter to 0 max health, and a
    // creature respawned like that is alive at 0 HP: the client draws a corpse
    // that wanders, and Unit::Kill refuses a victim already at 0, so it can
    // never die and never respawn again. Rate.Creature.Normal.HP = 0.75 did
    // this to 1,744 spawns.
    health = std::max<uint32>(health, 1);
...
bool Creature::LoadFromDB(uint32 guidlow, Map *map, bool force)
{
    ...
    // A HOUSE CRITTER IS MADE HARMLESS HERE, and here rather than where it is
    // placed because there are two spawn routes and this is the one funnel both
    // share: HouseMgr::SpawnCritterNow when somebody places one, and
    // ObjectGridLoader when somebody walks into a house that already has one.
    // Dressing at the placement site gives you a rabbit that is safe until the
    // first relog. Guarded on the guid block first, so this costs one integer
    // compare for every other creature in the world.
    if (guidlow >= HOUSE_CRITTER_GUID_MIN && guidlow <= HOUSE_CRITTER_GUID_MAX)
        HouseDressCritter(this);
