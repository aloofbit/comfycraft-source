    ...
    m_honorMgr(this), m_bNextRelocationsIgnored(0), m_standStateTimer(0), m_newStandState(MAX_UNIT_STAND_STATE), m_foodEmoteTimer(0), _transmogMgr(new TransmogMgr(this))
{
    ...
    m_forcedInstanceMapId = 0;
    m_forcedInstanceId = 0;
...
void Player::GiveLevel(uint32 level)
{
    ...
    // Going down has to take the character's power with it, or a level 10 keeps a level
    // 60's spec and spellbook. InitTalentForLevel above only zeroes the free points for
    // an administrator rather than respeccing them, which is every use of .levelup here.
    if (level < oldLevel && m_usedTalentCount > CalculateTalentsPoints())
        ResetTalents(true);

    UpdateSpellsForLevel(oldLevel);
...
void Player::_SetCreateBits(UpdateMask *updateMask, Player *target) const
{
    ...
    else
    {
        ...
        // ComfyCraft: BuildValuesUpdate injects the FFA bit into PLAYER_FLAGS for a
        // flagged player, and a bit can only be injected into a field that is IN the
        // mask -- the loop above puts one there only when it is already non-zero.
        // A player who has not gone contested and is not standing in a town has
        // PLAYER_FLAGS == 0, so anyone meeting them for the first time was never told
        // they were attackable: summoned to them, teleported, or just walking into
        // range. Unit::SetPvP covers the player already in view; this covers the one
        // who was not. PLAYER_FLAGS is a visual bit (see below), so forcing it here
        // sends nothing a viewer would not otherwise be allowed.
        //
        // Same trap as the housing critter flags at Object.cpp:612, and worth
        // remembering as one rule: an injected value needs its field in the mask, and
        // both masks decide that by asking whether the field is already non-zero.
    }

    // And the same for your own view of yourself, which takes the branch above and so
    // was missed the first time. It matters after a relog: the login path sets the PvP
    // flag from the saved PLAYER_FLAGS_PVP_DESIRED bit and then strips that bit
    // (Player.cpp:17088), leaving a flagged player whose PLAYER_FLAGS is exactly 0. So
    // the field is in neither create block, and the player came back seeing their own
    // ordinary faction PvP badge rather than the FFA one.
    if (IsPvP() && sWorld.getConfig(CONFIG_BOOL_FACTIONLESS_ENABLE))
...
void Player::InitVisibleBits()
{
    ...
    // PROBE, 2026-09-13: does the 1.12 client offer an NPC interaction on a
    // PLAYER unit carrying UNIT_NPC_FLAGS? Nothing could answer that before
    // this line, because the field was never sent: a player's fields are
    // filtered through updateVisualBits for every viewer but themselves
    // (Player::_SetCreateBits, and _SetUpdateBits ANDs against it), and stock
    // MaNGOS leaves UNIT_NPC_FLAGS out -- reasonably, since no player has ever
    // had one. So the flag was set server-side, read back correctly by .modify
    // npcflag, and silently dropped on its way to every other client.
    //
    // Same family of trap as PLAYER_FLAGS above and the housing critter flags
    // at Object.cpp:612: a value only reaches a viewer if its field is in the
    // mask. Object::BuildValuesUpdate already writes UNIT_NPC_FLAGS verbatim
    // for a player -- its masking rules are gated on TYPEID_UNIT and simply
    // fall through -- so this bit is the whole of what was missing.
...
void Player::DeleteFromDB(ObjectGuid playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    switch (charDelete_method)
    {
        ...
        case 0:
        {
            ...
            // The pet's OWN tables, before character_pet goes - they are keyed
            // by the pet's id, so once the character_pet row is gone there is
            // nothing left to find them by.
            //
            // Pet guids are reused exactly like character guids, and all three
            // of these key PRIMARY on guid, so leaving them behind is the same
            // crash one level down. It is what killed the server the second
            // time on 2026-09-06, minutes after the character-table fix:
            //
            //     Duplicate entry '174-46023' for key 'PRIMARY'   pet_spell
            //
            // Ordering is safe because the whole block runs inside the
            // BeginTransaction above, which executes in order on commit -
            // bare PExecute would NOT be ordered against the delete below.
            CharacterDatabase.PExecute("DELETE FROM pet_aura WHERE guid IN (SELECT id FROM character_pet WHERE owner = '%u')", lowguid);
            CharacterDatabase.PExecute("DELETE FROM pet_spell WHERE guid IN (SELECT id FROM character_pet WHERE owner = '%u')", lowguid);
            CharacterDatabase.PExecute("DELETE FROM pet_spell_cooldown WHERE guid IN (SELECT id FROM character_pet WHERE owner = '%u')", lowguid);
            ...
            // Turtle's own per-character tables, which upstream's delete never
            // knew about. Added 2026-09-06 after a crash.
            //
            // CHARACTER GUIDS ARE REUSED. Delete a character and the next one
            // created can be handed the same low guid, so any table still
            // holding rows for the dead one collides with the new one - and on
            // this core a failing query is not a warning, it aborts the process
            // (MySQLConnection::HandleMySQLError). The crash was:
            //
            //     Duplicate entry '4642-53' for key 'PRIMARY'   character_transmogs
            //     [CRASH] signal-SIGABRT
            //
            // Nine of the tables below key PRIMARY on guid, so each was its own
            // waiting abort. It went unnoticed for as long as it did because
            // deleting characters was rare; companions create and delete one per
            // hire, which turned a latent bug into a reliable one.
            CharacterDatabase.PExecute("DELETE FROM character_transmogs WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM character_account_data WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM character_armory_stats WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM character_aura_suspended WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM character_pvp_currency WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM character_spell_dual_spec WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM character_stats WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM character_titles WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM character_xp_from_log WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM character_honor_cp WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM character_inventory_copy WHERE guid = '%u'", lowguid);
            CharacterDatabase.PExecute("DELETE FROM ai_playerbot_db_store WHERE guid = '%u'", lowguid);
            // sql/custom/083, reshaped by 086. UNPLACED, NOT DELETED, and the
            // difference is the point: since 086 a row is a DESIGN -- a name, a
            // race, a look, some words -- and the character is only the body it
            // was last wearing. Deleting the character should cost you the body
            // and not the design, so the next `.npcbot place` makes a new one
            // and everything about it is as it was.
            //
            // It still has to be cleared rather than left, for the reason the
            // nine above exist: CHARACTER GUIDS ARE REUSED, so a row still
            // naming a dead one would claim whoever is handed that guid next,
            // and the core would quietly treat somebody's new alt as an NPC.
            CharacterDatabase.PExecute("UPDATE bot_npc SET character_guid = 0 WHERE character_guid = '%u'", lowguid);
...
void Player::UpdateSkillsForLevel()
{
    ...
    for (SkillStatusMap::iterator itr = mSkillStatus.begin(); itr != mSkillStatus.end(); ++itr)
    {
        ...
        if (max != 1)
        {
            ...
            else if (max != maxSkill || val > maxSkill)     /// pull the cap, and the value with it, to the new level
            {
                uint32 newVal = val;

                if (val > maxSkill)
                {
                    // The level went down (.levelup with a negative amount, .character
                    // level). The cap follows it and so must the value: a level 10 has
                    // no business sitting at Defense 300. Park what was there in the
                    // same high-water map a talent respec uses, so climbing back hands
                    // it over instead of leaving a re-grind behind.
                    uint16& remembered = m_mForgottenSkills[uint16(pskill)];
                    if (val > remembered)
                        remembered = uint16(val);

                    newVal = maxSkill;
                }
                else
                {
                    // Climbing again: give back whatever an earlier drop took away,
                    // as far as this level allows.
                    auto const saved = m_mForgottenSkills.find(uint16(pskill));
                    if (saved != m_mForgottenSkills.end() && saved->second > newVal)
                        newVal = std::min<uint32>(saved->second, maxSkill);
                }

                SetUInt32Value(valueIndex, MAKE_SKILL_VALUE(newVal, maxSkill));
                if (itr->second.uState != SKILL_NEW)
                    itr->second.uState = SKILL_CHANGED;
            }
        }
...
// A spell the level decides: a class spell with a level of its own that was not bought
// with talent points. Recipes, proficiencies, racials, mounts and toys all fall outside
// it (spellLevel 0, or a family that is not the class's), and so does every talent and
// every rank that grows out of one - ResetTalents owns those.
bool Player::IsLevelBoundClassSpell(SpellEntry const* spellInfo) const
{
    if (!spellInfo || !spellInfo->spellLevel)
        return false;

    ChrClassesEntry const* clsEntry = sChrClassesStore.LookupEntry(GetClass());
    if (!clsEntry || spellInfo->SpellFamilyName != clsEntry->spellfamily)
        return false;

    if (GetTalentSpellPos(spellInfo->Id))
        return false;

    // a chain whose first rank is a talent is a talent all the way up
    return GetTalentSpellCost(sSpellMgr.GetFirstSpellInChain(spellInfo->Id)) == 0;
}

// Keeps the spellbook honest when the level drops. What the new level does not allow is
// unlearned outright, not hidden: only the top rank of a chain is ever saved, and
// _LoadSpells rebuilds the lower ranks from that row with its disabled flag - so a hidden
// rank 9 came back at the next login as nine hidden ranks, and a level 20 warrior had no
// Heroic Strike at all. Getting the spells back is LearnClassSpellsForLevel's job.
void Player::UpdateSpellsForLevel(uint32 oldLevel)
{
    if (GetLevel() < oldLevel)
        UnlearnSpellsAboveLevel();
}

// Unlearns every level-bound class spell the current level does not allow, dependent
// ranks included: once the top rank goes, the one under it is the top. Anything still
// carrying the old disabled flag goes too, whatever its level, so that a row left by
// the hide-not-unlearn days cannot shadow a fresh learn.
void Player::UnlearnSpellsAboveLevel()
{
    uint32 const level = GetLevel();

    std::vector<std::pair<uint32, uint32>> drop;            // spellLevel, spell id

    // Collect before acting: RemoveSpell walks the rank chain and mutates m_spells.
    for (const auto& itr : m_spells)
    {
        if (itr.second.state == PLAYERSPELL_REMOVED)
            continue;
        ...
        if (!IsLevelBoundClassSpell(spellInfo))
            continue;

        if (spellInfo->spellLevel > level || itr.second.disabled)
            drop.emplace_back(spellInfo->spellLevel, itr.first);
    }

    // Highest rank first. RemoveSpell only hands the spellbook slot down to the previous
    // rank when the one it removes is the active one, so working upwards would leave the
    // whole chain gone and nothing on the bar.
    std::sort(drop.begin(), drop.end(), [](std::pair<uint32, uint32> const& a, std::pair<uint32, uint32> const& b)
    {
        return a.first > b.first;
    });

    for (const auto& entry : drop)
        RemoveSpell(entry.second, false, true);
}

// Every class spell the character's level allows, whether a trainer or a class quest is
// where it normally comes from - both live in SkillLineAbility, so Summon Felhunter and
// Defensive Stance arrive with the rest. Talents stay the player's to spend, and so does
// anything without a level of its own: proficiencies, recipes and racials all read 0 and
// do not follow the level in either direction.
void Player::LearnClassSpellsForLevel()
{
    // Start from a clean book, so that nothing above the level and nothing stale can
    // shadow what is about to be learned.
    UnlearnSpellsAboveLevel();
    ...
    for (uint32 i = 0; i < sObjectMgr.GetMaxSkillLineAbilityId(); ++i)
    {
        ...
        if (!IsLevelBoundClassSpell(spellInfo) || spellInfo->spellLevel > level)
            continue;
    ...
    // LearnSpell re-enables any disabled higher rank it can reach. Nothing should be
    // disabled by now, but the sweep is cheap and keeps the promise.
    UnlearnSpellsAboveLevel();
}

// Weapon skills and Defense at the cap this level allows. Professions, secondary skills
// and languages are deliberately untouched: they do not track the level, and raising
// them is not what asking for a level is asking for.
void Player::MaxLevelDependentSkills()
{
    uint32 const maxSkill = GetSkillMaxForLevel();

    for (auto& itr : mSkillStatus)
    {
        if (itr.second.uState == SKILL_DELETED)
            continue;

        SkillLineEntry const* pSkill = sSkillLineStore.LookupEntry(itr.first);
        if (!pSkill)
            continue;

        SkillRaceClassInfoEntry const* rcEntry = GetSkillRaceClassInfo(itr.first, GetRace(), GetClass());
        if (!rcEntry)
            continue;
        ...
        uint32 const valueIndex = PLAYER_SKILL_VALUE_INDEX(itr.second.pos);
        if (SKILL_MAX(GetUInt32Value(valueIndex)) <= 1)
            continue;
...
void Player::UpdateArea(uint32 newArea)
{
    ...
    // ComfyCraft: no fighting in a town. PLAYER_FLAGS_SANCTUARY makes two players
    // friendly to each other in GetReactionTo, above both the FFA hackfix and our own
    // flag rule, so this prevents the fight rather than punishing it afterwards --
    // which the guards can no longer do, being friendly to everyone now.
    //
    // Set here rather than in UpdateZone because a town is an AREA inside a zone,
    // Goldshire inside Elwynn Forest, and only this runs on an area change. The
    // arena FFA code above is the precedent and says so in its own comment. Capitals
    // are read off the ZONE, which is safe here: UpdateZone assigns m_zoneUpdateId
    // before it calls us, so it is current either way.
    //
    // Duels are unaffected: the duel check in GetReactionTo returns hostile before
    // sanctuary is ever consulted, so Goldshire stays the duelling spot it has always
    // been. See docs/features/factionless.md piece 8.
    bool sanctuary = false;
    if (sWorld.getConfig(CONFIG_BOOL_FACTIONLESS_ENABLE))
    {
        sanctuary = sWorld.IsFactionlessSanctuaryArea(newArea);

        if (!sanctuary && sWorld.getConfig(CONFIG_BOOL_FACTIONLESS_SANCTUARY_CAPITALS))
        {
            if (areaEntry && (areaEntry->Flags & AREA_FLAG_CAPITAL))
                sanctuary = true;
            else if (AreaEntry const* zoneEntry = AreaEntry::GetById(m_zoneUpdateId))
                sanctuary = (zoneEntry->Flags & AREA_FLAG_CAPITAL) != 0;
        }
    }

    if (HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_SANCTUARY) != sanctuary)
    {
        if (sanctuary)
...
void Player::UpdateZone(uint32 newZone, uint32 newArea)
{
    ...
    // ComfyCraft: on a factionless world the other side's capital is not enemy ground,
    // so walking into Orgrimmar must not flag you for PvP against your will. Only the
    // capital half of the condition is dropped; a PvP realm still enforces everywhere.
    // Side effect, and the wanted one: a visitor now counts as resting in that city.
    // See docs/features/factionless.md piece 7.
    bool const capitalEnforcesPvP = !sWorld.getConfig(CONFIG_BOOL_FACTIONLESS_ENABLE)
                                    && (zoneEntry->Flags & AREA_FLAG_CAPITAL);

    switch (zoneEntry->Team)
    {
        case AREATEAM_ALLY:
            pvpInfo.inPvPEnforcedArea = GetTeam() != ALLIANCE && (sWorld.IsPvPRealm() || capitalEnforcesPvP);
            break;
        case AREATEAM_HORDE:
            pvpInfo.inPvPEnforcedArea = GetTeam() != HORDE && (sWorld.IsPvPRealm() || capitalEnforcesPvP);
            break;
        case AREATEAM_NONE:
...
void Player::SendBuyError(BuyResult msg, Unit* pVendor, uint32 item, uint32 /*param*/) const
{
    DEBUG_LOG("WORLD: Sent SMSG_BUY_FAILED");
    WorldPacket data(SMSG_BUY_FAILED, (8 + 4 + 1));
    data << (pVendor ? pVendor->GetObjectGuid() : ObjectGuid());
    data << uint32(item);
    //if (param > 0)
...
bool Player::IsBotNpc() const
{
    return sBotNpcMgr.IsBotNpc(GetGUIDLow());
}

uint32 Player::GetBotNpcQuestGiverId() const
{
    BotNpcEntry const* npc = sBotNpcMgr.Get(GetGUIDLow());
    return npc ? npc->questGiverId : 0;
}

/*
 * THE TWO HALVES OF "DOES THIS THING DEAL IN THAT QUEST", which Creature and
 * GameObject have always answered and Object answered false for.
 *
 * Same bodies as Creature::HasQuest and Creature::HasInvolvedQuest, reading
 * the same two maps -- only the key differs, and the key is the whole point:
 * a bot NPC has no creature entry, so it brings its own id. BotNpc.h has the
 * reasoning for reusing the creature tables at all.
 */
bool Player::HasQuest(uint32 quest_id) const
{
    uint32 const giver = GetBotNpcQuestGiverId();
    if (!giver)
        return false;

    QuestRelationsMapBounds bounds = sObjectMgr.GetCreatureQuestRelationsMapBounds(giver);
...
bool Player::HasInvolvedQuest(uint32 quest_id) const
{
    uint32 const giver = GetBotNpcQuestGiverId();
    if (!giver)
        return false;

    QuestRelationsMapBounds bounds = sObjectMgr.GetCreatureQuestInvolvedRelationsMapBounds(giver);
    for (QuestRelationsMap::const_iterator itr = bounds.first; itr != bounds.second; ++itr)
    {
        if (itr->second == quest_id)
            return true;
...
/*
 * THE SHOP A GOSSIP OPTION NAMES, or 0 for one that does not.
 *
 * `gossip_menu_option.action_menu_id` carries two different things on a vendor
 * option, told apart by size: 1, 2 and 3 are VENDOR_MENU_NORMAL, _TEMPLATE and
 * _ALL, which is what the field has always meant here. Anything larger is a
 * npc_vendor_template.entry, written by the dialog editor when a button names a
 * shop, and that is what lets one NPC offer several.
 *
 * The values cannot collide: no vendor template in this database is numbered 1
 * to 3, and the editor allocates from 200000 (sql/custom/092). Both callers --
 * drawing the menu and clicking it -- have to agree, so the rule lives in one
 * place rather than in two matching comparisons.
 */
static uint32 ShopNamedBy(int32 actionMenuId)
{
    return actionMenuId > int32(VENDOR_MENU_ALL) ? uint32(actionMenuId) : 0;
}
...
void Player::PrepareGossipMenu(WorldObject *pSource, uint32 menuId)
{
    ...
    /*
     * A BOT NPC HANDS OUT QUESTS SINCE 2026-09-15, and this arm has to be said
     * explicitly rather than left to the test above -- neither of its arms
     * names a player, so a player source sails through with canSeeQuests still
     * true whatever it is carrying.
     *
     * THE RULE IS THE CREATURE RULE: the questgiver bit, and a design that
     * actually points somewhere. Without the second check a bot with the bit
     * and no quests would still be asked, which costs nothing but says the
     * wrong thing in a debugger.
     *
     * This used to be a flat `canSeeQuests = false` with a note saying handing
     * a quest from a bot needed the QuestHandler typemasks widened. It did, and
     * they now are. docs/ai/content/playerbot-npcs.md.
     */
    if (pSource->GetTypeId() == TYPEID_PLAYER)
    {
        if (!pSource->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER) ||
            !static_cast<Player*>(pSource)->GetBotNpcQuestGiverId())
            canSeeQuests = false;
    }
    ...
    for (GossipMenuItemsMap::const_iterator itr = pMenuItemBounds.first; itr != pMenuItemBounds.second; ++itr)
    {
        ...
        if (pSource->GetTypeId() == TYPEID_UNIT)
        {
            ...
            switch (itr->second.option_id)
            {
                ...
                {
                    // A NAMED SHOP IS CHECKED INSTEAD OF THE NPC'S OWN STOCK.
                    // action_menu_id above VENDOR_MENU_ALL is a shop id -- see
                    // the matching note in OnGossipSelect -- and an NPC offering
                    // several shops need not carry any list of its own.
                    if (uint32 shop = ShopNamedBy(itr->second.action_menu_id))
                    {
                        // A shop is paid in GOLD or in GOODS, and which one it is
                        // is simply which table its rows are in. Checking only
                        // the gold table hid every item-trade button.
                        VendorItemData const* named = sObjectMgr.GetNpcVendorTemplateItemList(shop);
                        if ((!named || named->Empty()) && !sCustomMerchantMgr.HasShop(shop))
                        {
                            sLog.outErrorDb("Creature %u (Entry: %u) has a gossip option naming shop %u, which is empty or does not exist.",
                                            pCreature->GetGUIDLow(), pCreature->GetEntry(), shop);
                            hasMenuItem = false;
                        }
                        break;
                    }
        ...
        {
            /*
             * A BOT NPC. The npcflag gate is the same rule creatures get and is
             * worth keeping honest: npc_option_npcflag is how a menu row says
             * which kind of NPC it belongs on, and dropping the check here
             * would make a bot the one NPC in the world that shows every
             * option of every menu it is pointed at.
             *
             * Everything past plain gossip is REFUSED rather than attempted.
             * The creature arm above reaches GetVendorItems, IsTrainerOf,
             * CanInteractWithBattleMaster and friends, all of which are Creature
             * members with no Player equivalent -- so a vendor option on a bot
             * is not a thing that half-works, it is a thing that has not been
             * built. Showing it would be a menu entry that does nothing.
             */
            // pSource, NOT this: the flags that decide the menu are the NPC's,
            // and the viewer is a player whose own npcflags are 0.
            if (!(itr->second.npc_option_npcflag & pSource->GetUInt32Value(UNIT_NPC_FLAGS)))
                continue;
            ...
            switch (itr->second.option_id)
            {
                case GOSSIP_OPTION_GOSSIP:
                    break;
                /*
                 * SELLING, which a bot NPC can do since the vendor path stopped
                 * being Creature-typed. The check mirrors the creature arm
                 * exactly, including complaining about a flagged NPC with an
                 * empty list: the option would open a window with nothing in
                 * it, and there is nothing in game to say why.
                 *
                 * Only the TEMPLATE half exists here, because npc_vendor is
                 * keyed by creature entry and a bot NPC has none. BotNpc.h.
                 */
                case GOSSIP_OPTION_VENDOR:
                {
                    // The shop this button names, or the one the design points
                    // at. Same rule as the creature arm above.
                    uint32 const shop = ShopNamedBy(itr->second.action_menu_id);
                    VendorItemData const* items = shop
                        ? sObjectMgr.GetNpcVendorTemplateItemList(shop)
                        : BotNpc_VendorItems((Unit*)pSource);

                    // Gold or goods, the same as the creature arm above. A bot
                    // NPC reaches an item-trade shop by exactly this route.
                    if ((!items || items->Empty()) && !(shop && sCustomMerchantMgr.HasShop(shop)))
                    {
                        sLog.outErrorDb("Bot NPC %u has UNIT_NPC_FLAG_VENDOR but no shop, or a shop with nothing in it.",
                                        pSource->GetGUIDLow());
                        hasMenuItem = false;
                    }
                    break;
                ...
                /*
                 * EVERYTHING ELSE IS REFUSED RATHER THAN ATTEMPTED, and the list
                 * of what that covers is the list of Creature members with no
                 * Player equivalent: IsTrainerOf, CanInteractWithBattleMaster,
                 * CanTrainAndResetTalentsOf, the taxi node. Showing one would be
...
void Player::OnGossipSelect(WorldObject* pSource, uint32 gossipListId)
{
    ...
    switch (gossipOptionId)
    {
        ...
        {
            /*
             * action_menu_id CARRIES TWO DIFFERENT THINGS, told apart by size.
             *
             * 1, 2 and 3 are VENDOR_MENU_NORMAL, _TEMPLATE and _ALL -- which
             * HALF of the NPC's stock to show, which is what this field has
             * always meant here.
             *
             * ANYTHING LARGER IS A SHOP ID (npc_vendor_template.entry), written
             * by the dialog editor when a button names a shop. That is what lets
             * one NPC offer several: "Reagents" and "Weapons" as two buttons on
             * one menu. The values cannot collide -- no vendor template in this
             * database is numbered 1 to 3, and the editor allocates from 200000.
             */
            uint32 const action = uint32(pMenuData.m_gAction_menu > 0 ? pMenuData.m_gAction_menu : 0);
            uint32 const shop = ShopNamedBy(pMenuData.m_gAction_menu);

            /*
             * GOODS, NOT GOLD -- a different window entirely.
             *
             * This client can draw a coin price or a pile of item costs and
             * never both, so a shop is one or the other and the editor makes
             * you choose. Which it is needs no flag: it is whichever table the
             * rows were written to.
             *
             * Nothing here is creature-only, which is the point. A bot NPC
             * arrives at this same line, and the frame opens for it -- the
             * client never has to ask, because the server is telling.
             */
            if (shop && sCustomMerchantMgr.HasShop(shop))
            {
                if (Unit* pVendor = pSource->ToUnit())
                {
                    sCustomMerchantMgr.OpenShop(this, pVendor, shop);
                    break;
                }
            }

            uint8 const menuType = shop ? VENDOR_MENU_ALL : uint8(action ? action : VENDOR_MENU_ALL);

            GetSession()->SendListInventory(guid, menuType, nullptr, shop);
            break;
        }
        case GOSSIP_OPTION_STABLEPET:
    ...
    if (pMenuData.m_gAction_script)
    {
        ...
        /*
         * A BOT NPC. Without this arm a bot's buttons could navigate and close
         * and nothing else: the two tests above are exhaustive, so a player
         * source fell out of the bottom and action_script_id was read, stored,
         * and quietly never run. That made every gossip_scripts command --
         * CREATE_ITEM, TALK, CAST_SPELL, our own RECRUIT_BOT -- unreachable
         * from a bot NPC, with no error anywhere.
         *
         * THE ARGUMENT ORDER IS THE CREATURE ONE, AND IT IS LOAD-BEARING RATHER
         * THAN COSMETIC. Note the two arms above already disagree: a gameobject
         * passes (player, object) and a creature passes (creature, player).
         * Every command in ScriptCommands.cpp that wants a player resolves it
         * target-first --
         *
         *     if (!((pPlayer = ToPlayer(target)) || (pPlayer = ToPlayer(source))))
         *
         * -- which for a creature finds the viewer in `target` and is done.
         * A bot NPC is a Player too, so passing the pair the other way round
         * would satisfy that test with THE NPC ITSELF and hand it the item,
         * the quest credit or the spell that was meant for whoever clicked.
         * It would not error; it would work, on the wrong person. Source is the
         * one being talked to, target is the one doing the talking, exactly as
         * for a creature.
         *
         * What still does not work is the other half: a command that reaches
         * for Creature members off the source gets nullptr from ToCreature.
         * Player::SendBuyError is null-safe about it; not all of them have been
         * read. Try a command before promising it -- 35 of the 98 opcodes are
         * in use in gossip_scripts here and none of the rest has been measured.
         */
        else if (pSource->GetTypeId() == TYPEID_PLAYER)
            GetMap()->ScriptsStart(sGossipScripts, pMenuData.m_gAction_script, pSource->GetObjectGuid(), GetObjectGuid());
    }
...
void Player::PrepareQuestMenu(ObjectGuid guid, uint32 exceptQuestId)
{
    ...
    // A BOT NPC, which is a Player and so fell off the end of this and got no
    // quest menu at all until 2026-09-15. Same two maps, different key: it has
    // no creature entry, so its design carries an id of its own. BotNpc.h.
    else if (Player* pBot = ObjectAccessor::FindPlayer(guid))
    {
        uint32 const giver = pBot->GetBotNpcQuestGiverId();
        if (!giver)
            return;

        rbounds = sObjectMgr.GetCreatureQuestRelationsMapBounds(giver);
        irbounds = sObjectMgr.GetCreatureQuestInvolvedRelationsMapBounds(giver);
    }
    else
    {
...
Quest const* Player::GetNextQuest(ObjectGuid guid, Quest const *pQuest)
{
    ...
    if (Creature *pCreature = GetMap()->GetAnyTypeCreature(guid))
        rbounds = sObjectMgr.GetCreatureQuestRelationsMapBounds(pCreature->GetEntry());
    // A bot NPC. Without this the chain simply stops at a bot: the next quest
    // is not offered on the spot after handing one in, which is a quiet
    // degradation rather than a failure and would have taken a while to notice.
    else if (Player* pBot = ObjectAccessor::FindPlayer(guid))
    {
        if (uint32 giver = pBot->GetBotNpcQuestGiverId())
            rbounds = sObjectMgr.GetCreatureQuestRelationsMapBounds(giver);
        else
            return nullptr;
    }
...
void Player::AddQuest(Quest const *pQuest, Object *questGiver)
{
    ...
    if (pQuest->HasSpecialFlag(QUEST_SPECIAL_FLAG_TIMED))
    {
        ...
        /*
         * A SHARED TIMED QUEST INHERITS WHAT IS LEFT OF THE SHARER'S CLOCK,
         * which is right for a party member and wrong for a BOT NPC.
         *
         * A bot NPC is a player that has never been on the quest, so
         * getQuestStatusMap() has no entry -- and operator[] would build one
         * with m_timer 0 rather than say so, handing out a timed quest with no
         * time on it at all. Gated on the offerer actually being on the quest,
         * which is the thing "shared" was standing in for.
         *
         * 33 quests in this world DB are timed and none of them are ours, so
         * this has never fired. It is fixed here because it was found here.
         */
        if (questGiver && questGiver->GetTypeId() == TYPEID_PLAYER &&
            static_cast<Player*>(questGiver)->IsCurrentQuest(quest_id))
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder)
{
    ...
    /*
     * A BOT NPC'S NAME IS NOT HELD TO THE PLAYER NAME RULES.
     *
     * CheckPlayerName ends in isBasicLatinString with numericOrSpace = false,
     * so every character must be a basic Latin LETTER -- no space, no
     * underscore, no digit. That is the right rule for somebody typing a name
     * at character creation and the wrong one for scenery: real NPCs are called
     * "Innkeeper Allison", and a bot NPC that cannot be is a bot NPC that
     * always reads as a player.
     *
     * THIS IS NOT A WARNING SITE. Failing here returns false and flags
     * AT_LOGIN_RENAME, so the character never enters the world at all -- which
     * is why a spaced name looked like the CLIENT rejecting it ("Unknown" over
     * a unit with no name behind it) when the server had refused the login
     * before the client was ever asked.
     *
     * Narrow on purpose: a real player with a bad name still gets the rename
     * flag, which is what that flag is for. Only a row in tw_char.bot_npc buys
     * the exemption, and those are made by an admin on the website.
     *
     * Whether the CLIENT then draws it is a separate question and the one
     * actually worth measuring; this only stops the server answering it first.
     */
    bool const isBotNpcName = sBotNpcMgr.IsBotNpc(guid.GetCounter());
    ...
    /*
     * A BOT NPC STANDS WHERE IT WAS PUT, not where it logged out.
     *
     * An NPC being in a fixed place is most of what makes it an NPC rather than
     * a character, and a character conscripted into the job logs out wherever it
     * happened to be. Before this, `.npcbot add` looked broken: you ran it, the
     * bot logged in somewhere across the map, and the only way to find it was
     * `.summon`.
     *
     * HERE, RATHER THAN A TELEPORT ONCE IT IS IN WORLD. This runs before the map
     * and instance are resolved below, so the bot arrives on its mark directly.
     * Teleporting afterwards would also work and would be worse: the bot would
     * exist at the wrong place first, visibly, on every single restart.
     *
     * So characters.position_* is IGNORED for a bot NPC, deliberately -- it can
     * be shoved about, log out, and still come back on its mark. `.npcbot move`
     * is how the mark itself changes. sql/custom/085.
     */
    if (BotNpcEntry const* npc = sBotNpcMgr.Get(GetGUIDLow()))
    {
        if (MaNGOS::IsValidMapCoord(npc->x, npc->y, npc->z, npc->o))
        {
            Relocate(npc->x, npc->y, npc->z, npc->o);
            SetLocationMapId(npc->mapId);
        }
        else
        {
            // A row with no position -- added before sql/custom/085, or written
            // by hand. Left where it was rather than dropped at 0,0,0, which on
            // map 0 is the sea off Westfall.
            sLog.outError("Bot NPC %u has invalid stored coordinates (map %u, %f %f %f); leaving it where it logged out.",
                          GetGUIDLow(), npc->mapId, npc->x, npc->y, npc->z);
        }
    }
    ...
    // LOGGING IN INSIDE A HOUSE puts you at the way out, a little above it,
    // rather than exactly where you stood when you logged off.
    //
    // The saved position is a point in a world that has not been built yet.
    // Furniture and any building the owner placed are grid spawns that arrive
    // after the player does, so logging out on an inn floor and back in again
    // drops you straight through it -- the same reason arriving by teleport
    // needed HOUSE_ARRIVE_LIFT. And the way out is the one spot in a house that
    // is always worth standing on.
    //
    // GetArrivalPosition is the same answer .house go and the entrance portals
    // use, so every route into a house agrees on where you end up.
    //
    // By INSTANCE first, not by account: a guest logging in inside somebody
    // else's house would otherwise be sent to the coordinates of their OWN way
    // out, which in the host's instance is an arbitrary spot on the plain.
    if (GetMapId() == HOUSE_MAP_ID)
    {
        House const* house = state ? sHouseMgr.GetHouseByInstance(state->GetInstanceId()) : nullptr;
        if (!house)
            house = sHouseMgr.GetHouseByAccount(GetSession()->GetAccountId());

        if (house)
        {
            float ax, ay, az, ao;
            sHouseMgr.GetArrivalPosition(*house, ax, ay, az, ao);
            Relocate(ax, ay, az, ao);
        }
    }
    ...
    /*
     * BOT NPCs -- a character that acts as an NPC, bound in tw_char.bot_npc.
     *
     * Applied here rather than in BotNpcMgr::Load, which runs during
     * World::SetInitialWorldSettings when nobody is logged in at all. Coming
     * through LoadFromDB is the only way a bot NPC can arrive in world, so this
     * is the one place that has to be right.
     *
     * UNIT_NPC_FLAGS on a PLAYER reaches other clients only because
     * InitVisibleBits carries the bit -- see the note there. Setting it without
     * that is a silent no-op for every viewer but this character itself, which
     * is exactly how the question looked before it was measured.
     */
    if (BotNpcEntry const* npc = sBotNpcMgr.Get(GetGUIDLow()))
    {
        SetUInt32Value(UNIT_NPC_FLAGS, npc->npcFlags);
        m_botNpcGossipMenuId = npc->gossipMenuId;

        // And what is in its hands, off the same look that dressed it.
        // Nothing else sets this on a player: the client owns its own sheath
        // state and a bot NPC has not got one. BotNpc.h.
        SetSheath(BotNpc_SheathFor(npc->look));
    }
        ...
        do
        {
            ...
            if (!bag_guid)
            {
                ...
                else if (IsEquipmentPos(INVENTORY_SLOT_BAG_0, slot))
                {
                    ...
                    /*
                     * A BOT NPC KEEPS ITS COSTUME ACROSS A LOAD.
                     *
                     * This is the other half of forcing an NPC's outfit on, and
                     * without it the first half is pointless: `.npcbot dress`
                     * puts a paladin's tier set on a warrior body quite happily,
                     * the character saves wearing it, and then THIS line strips
                     * every piece on the way back in -- CanEquipItem ends in
                     * CanUseItem, which enforces AllowableClass. The row in
                     * `character_inventory` is deleted, the `item_instance`
                     * behind it is orphaned, and the NPC stands back up naked.
                     *
                     * It took a while to find because it looks like the dressing
                     * failing. It is not: the dressing works, saves, and is
                     * undone somewhere else entirely, one restart later.
                     *
                     * The slot is already known to be an equipment slot, and the
                     * item was in it when the character was saved, so the
                     * destination needs no deriving -- only the permission
                     * question is being skipped, and a bot NPC has no wearer to
                     * ask it of. See docs/ai/content/playerbot-npcs.md.
                     */
                    else if (IsBotNpc())
...
DungeonPersistentState* Player::GetBoundInstanceSaveForSelfOrGroup(uint32 mapid)
{
    ...
    // A forced instance beats everything -- see SetForcedInstance. The state is
    // always already loaded when this fires, because whatever set the override
    // had to prepare the instance before teleporting us into it.
    if (uint32 forcedId = GetForcedInstanceId(mapid))
        if (MapPersistentState* forced = sMapPersistentStateMgr.GetPersistentState(mapid, forcedId))
            return static_cast<DungeonPersistentState*>(forced);
...
bool Player::BuyItemFromVendor(ObjectGuid vendorGuid, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    /*
     * A BOT NPC. Same branch, same checks and the same reason as
     * WorldSession::SendListInventory: GetNPCIfCanInteractWith ends in
     * GetAnyTypeCreature, which is nullptr for a player guid.
     *
     * The two have to agree about who may buy, or an item is listed in a window
     * that then refuses to sell it.
     */
    Unit* pVendor = nullptr;
    VendorItemData const* botItems = nullptr;

    if (vendorGuid.IsPlayer())
    {
        Player* pNpc = GetMap()->GetPlayer(vendorGuid);

        if (!pNpc || pNpc == this || !pNpc->IsAlive() ||
            !pNpc->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_VENDOR) ||
            !IsWithinDistInMap(pNpc, INTERACTION_DISTANCE) ||
            pNpc->IsHostileTo(this))
        {
            DEBUG_LOG("WORLD: BuyItemFromVendor - %s is not a reachable bot NPC vendor.", vendorGuid.GetString().c_str());
            SendBuyError(BUY_ERR_DISTANCE_TOO_FAR, nullptr, item, 0);
            return false;
        }

        botItems = BotNpc_VendorItems(pNpc);
        pVendor = pNpc;
    }
    else
        pCreature = GetNPCIfCanInteractWith(vendorGuid, UNIT_NPC_FLAG_VENDOR);

    if (!pCreature && !pVendor)
    {
    ...
    if (pCreature && sWorld.IsAprilFools())
    {
    ...
    /*
     * THE SHOP THIS PLAYER ACTUALLY HAS OPEN, which is not always the one the
     * NPC is linked to: a gossip option can name a shop, and one NPC can offer
     * several. CMSG_BUY_ITEM carries only the vendor guid and the item entry, so
     * the session is the only place that knows which window this came from.
     *
     * GetOpenVendorTemplate is guarded on the guid, so a shop opened at one NPC
     * cannot be bought from while standing at another.
     *
     * The list checked here MUST be the list that was displayed. Anything else
     * refuses an item the player was just shown, which reads as the shop being
     * broken rather than as the mismatch it is.
     */
    uint32 const openShop = GetSession()->GetOpenVendorTemplate(vendorGuid);
    ...
    {
        vItems = nullptr;
        tItems = sObjectMgr.GetNpcVendorTemplateItemList(openShop);
    }
    else
    {
        // A bot NPC has the template half only -- npc_vendor is keyed by
        // creature entry, so there is no own list it could have.
        vItems = pCreature ? pCreature->GetVendorItems() : nullptr;
        tItems = pCreature ? pCreature->GetVendorTemplateItems() : botItems;
    }
    ...
    if (!reqFaction && pProto->RequiredReputationRank > 0)
        reqFaction = pVendor->GetFactionId();
    ...
    {
        SendBuyError(BUY_ERR_RANK_REQUIRE, pVendor, item, 0);
        return false;
    }

    if (crItem->conditionId && !IsGameMaster() && !sObjectMgr.IsConditionSatisfied(crItem->conditionId, this, pVendor->GetMap(), pVendor, CONDITION_FROM_VENDOR))
    {
        SendBuyError(BUY_ERR_CANT_FIND_ITEM, pVendor, item, 0);
        return false;
    }
    ...
    /*
     * WHAT THIS ROW COSTS. A cost row replaces the item's own price outright and
     * is not discounted -- see the matching note in SendListInventory.
     */
    VendorCost const* cost = sObjectMgr.GetVendorCost(crItem->costId);

    uint32 price = cost ? cost->money * count
                        : uint32(floor((pProto->BuyPrice * count) * GetReputationPriceDiscount(pVendor)));
    ...
    /*
     * THE GOODS HALF, CHECKED BEFORE ANYTHING IS TAKEN.
     *
     * Every requirement is verified first and only then are any consumed --
     * otherwise a player short on the third thing would have already handed over
     * the first two for an item they do not get.
     *
     * AND THE REFUSAL HAS TO SAY WHAT IS MISSING. SMSG_BUY_FAILED has no code
     * for "you also need three planks", and the window never showed the
     * requirement in the first place (1.12 has no ExtendedCost), so without this
     * line the button simply does nothing and the player has no way to find out
     * why. A notification is the only channel that can carry it.
     */
    if (cost && cost->WantsItems())
    {
        for (int ci = 0; ci < MAX_VENDOR_COST_ITEMS; ++ci)
        {
            if (!cost->item[ci] || !cost->count[ci])
                continue;

            uint32 const want = cost->count[ci] * count;
            if (HasItemCount(cost->item[ci], want))
                continue;

            ItemPrototype const* pNeed = sObjectMgr.GetItemPrototype(cost->item[ci]);
            std::ostringstream why;
            why << "You also need " << want << " x "
                << (pNeed ? pNeed->Name1 : "something") << ".";
            GetSession()->SendNotification("%s", why.str().c_str());
    ...
    /*
     * THE GOODS ARE TAKEN LAST, after the item is in the bag.
     *
     * Everything above can still refuse -- a full bag, a slot that will not take
     * it -- and each of those returns without touching the player's money
     * either. Consuming the requirement before the item existed would be the one
     * failure that costs somebody something for nothing.
     */
    if (cost && cost->WantsItems())
    {
        for (int ci = 0; ci < MAX_VENDOR_COST_ITEMS; ++ci)
        {
            if (cost->item[ci] && cost->count[ci])
                DestroyItemCount(cost->item[ci], cost->count[ci] * count, true);
        }
    }
...
// The cmangos-named shims the playerbot module calls. Both were empty; see the
// declaration in Player.h for what that cost.
...
// Every trainable class ability this character is high enough for.
//
// Modelled on ChatHandler::HandleLearnAllMySpellsCommand, which is the core's
// own "learn my class" and walks SkillLineAbility for the class spell family.
// The one deliberate difference is the LEVEL CAP: that command is a GM tool and
// grants every rank regardless, which on a level 20 companion would hand out
// rank 6 abilities it has no business having.
//
// Talent-learned spells are skipped the same way the command skips them -
// InitTalentForLevel and AutoSelectTalents own those, and learning the first
// rank here would fight them.
void Player::learnClassLevelSpells(bool /*includeHighLevelQuestRewards*/)
{
    ...
    for (uint32 i = 0; i < sObjectMgr.GetMaxSkillLineAbilityId(); ++i)
    {
        ...
        // Server-side / triggered spells carry no level and are not abilities.
        if (spellInfo->spellLevel == 0)
            continue;

        // The cap. Without it a level 20 tank gets rank 6 Sunder Armor.
        ...
        // Talents belong to the talent system, not here.
        const uint32 firstRank = sSpellMgr.GetFirstSpellInChain(spellInfo->Id);
...
float Player::GetReputationPriceDiscount(Unit const* pVendor) const
{
    FactionTemplateEntry const* vendor_faction = pVendor->GetFactionTemplateEntry();
...
void Player::UpdateForQuestWorldObjects()
{
    ...
    /*
     * BOT NPCs IN VIEW, whose quest mark has to be PUSHED rather than asked
     * for. Collected under the lock and sent after it, the same discipline the
     * gameobject half keeps: these are packets and the lock is over the
     * visibility set.
     */
    std::vector<ObjectGuid> botNpcs;
    ...
    /*
     * THE ! AND ? OVER A BOT NPC'S HEAD, 2026-09-15.
     *
     * The 1.12 client asks for a questgiver's mark with
     * CMSG_QUESTGIVER_STATUS_QUERY and, for a CREATURE, asks again whenever
     * the quest log changes. It does not do the second for a player, so a
     * bot NPC's mark is whatever it was the first time the client asked --
     * for ever.
     *
     * PUSHING IT ON THE NPC YOU ARE TALKING TO IS NOT ENOUGH, which is how
     * this was first written and what it got wrong: a quest given by one NPC
     * and handed in to another leaves the second one's mark untouched, so
     * "talk to Allison" showed complete inside her gossip window and nothing
     * at all over her head. Reported from a quest written on the website,
     * which is exactly the shape the editor makes easy.
     *
     * So every bot NPC the client can see is told, and this is the right
     * function to do it from: it already exists to mean "the quest log
     * changed, refresh what they can see", it is already called from accept,
     * from every status change and from both item checks, and m_visibleGUIDs
     * is already the set of things the client knows about. A mark for
     * something off screen would be a packet nobody could act on.
     *
     * Ordinary players are in that set too and answer 0 from
     * GetBotNpcQuestGiverId, so the cost on a normal server is one map lookup
     * per visible player per quest event.
     */
    for (const auto& guid : botNpcs)
    {
        Player* pBot = ObjectAccessor::FindPlayer(guid);
        if (!pBot || pBot == this || !pBot->GetBotNpcQuestGiverId())
            continue;

        GetSession()->SendQuestGiverMarkIfPlayer(pBot);
    }
}
...
void Player::RewardHonorOnDeath()
{
    ...
    for (const auto& itr : damagePerGroup)
    {
        ...
        // Honour's own copy of the old curve, NOT the XP one. They were the same
        // function until 2026-09-06, when XP was retuned to make grouping cheaper;
        // honour deliberately did not follow. See MaNGOS::Honor::honor_in_group_rate.
        honorRate *= MaNGOS::Honor::honor_in_group_rate(totalRewarded);
