...
void Unit::Kill(Unit* pVictim, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    // Invasions end the moment one side falls, and the loser is stood straight
    // back up rather than left to run for a corpse.
    //
    // BEHIND THE pPlayerVictim CHECK, NOT IN FRONT OF IT. Kill runs on every
    // creature death in the world -- a manager lookup ahead of this test would
    // be paid by every kobold anybody ever swings at.
    //
    // AND LAST, so the resurrect inside it lands after the death has finished
    // being processed. It has to happen here rather than on the manager's own
    // tick: sMapMgr.Update runs BEFORE sInvasionMgr.Update in World::Update, so
    // Player::Update would see JUST_DIED and call KillPlayer first, and the
    // loser would watch a corpse and a release prompt appear for the half
    // second before being resurrected out of them.
    if (pPlayerVictim)
        sInvasionMgr.OnPlayerDeath(pPlayerVictim);
}
...
void Unit::SetPvP(bool state)
{
    ...
    // ComfyCraft: the FFA bit that advertises a flagged player to everyone is injected
    // into PLAYER_FLAGS in BuildValuesUpdate, but flagging writes UNIT_FIELD_FLAGS and
    // nothing here dirties PLAYER_FLAGS, so the injection had nothing to ride on. The
    // bit only reached viewers when something else dirtied that field - going contested
    // on the first swing, or the sanctuary flag clearing on leaving a town - which made
    // the opening attack one-sided: whoever's field had been dirtied last could swing,
    // and the other could not until they were hit. Force the field out.
    //
    // Only on a real transition, which is all UpdatePvP calls us for, so this is one
    // extra field per flag and unflag rather than per tick.
    if (IsPlayer() && sWorld.getConfig(CONFIG_BOOL_FACTIONLESS_ENABLE))
        ForceValuesUpdateAtIndex(PLAYER_FLAGS);
...
void Unit::UpdateSplineMovement(uint32 t_diff)
{
    ...
    if (IsPlayer())
    {
        ...
        // The spline has put the bot at its final point; now bring it down.
        // DisableSpline ran above, before this position was applied, so its
        // own call found the previous point - this one is the one that holds.
        if (arrived)
            SettleDrivenPlayerOnGround();
    }
    else
        GetMap()->CreatureRelocation((Creature*)this, loc.x, loc.y, loc.z, loc.orientation);
}

// A BOT COMES DOWN TO THE GROUND WHEN A SERVER-DRIVEN MOVE ENDS OR STOPS.
//
// A spline's heights are the navigation mesh's, and the mesh sits above the
// terrain in places - a yard, in the screenshot that found this. A real
// player's client puts the character on the ground the moment it is in
// charge again; a bot has no client, so wherever the spline left it was
// where it stayed, hovering, until its next move. That is the last point of
// a move that arrived, and it is also the current point of a move that was
// STOPPED - which is what a companion does the instant it decides to cast,
// and why the first version of this, arrival only, still left casters in
// the air. The module's own MovePath grounds each point it hands the core;
// the chase, follow and point generators build their paths in the core and
// did not, and no stop ever did.
//
// Down only, and only a little: a bot on a bridge is on a WMO floor that
// GetHeight reports first, and a drop of more than a few yards is a real
// ledge, not a mesh offset. Not in water, where the floor is the lake bed
// and a bot put there would be underwater rather than swimming. Not on a
// taxi. The heartbeat is what makes the correction visible: the position is
// server-side until a movement packet says so.
void Unit::SettleDrivenPlayerOnGround()
{
    if (!IsPlayer() || !IsInWorld())
        return;

    Player* me = (Player*)this;
    if (me->IsTaxiFlying() || IsInWater() || !Script_IsMachineDriven(me))
        return;

    const float x = GetPositionX();
    const float y = GetPositionY();
    const float z = GetPositionZ();

    const float ground = GetMap()->GetHeight(x, y, z, true);
    const float above  = z - ground;
    if (ground <= INVALID_HEIGHT || above <= 0.1f || above >= 3.0f)
        return;

    me->SetPosition(x, y, ground + 0.05f, GetOrientation());
    SendHeartBeat();
}
...
void Unit::DisableSpline()
{
    ...
    // Every stop of a spline comes through here - StopMoving, and so the
    // bot module's own StopMoving before a cast - as does arrival. A bot
    // stopped mid-path is at mesh height like one that arrived.
    SettleDrivenPlayerOnGround();
}
...
/*
 * LIMITED VENDOR STOCK. Moved here from Creature.cpp unchanged, so that a bot
 * NPC -- a player carrying UNIT_NPC_FLAGS and a shop id, see BotNpc.h -- counts
 * stock the way a creature does. Nothing in either body was Creature's: they
 * touch m_vendorItemCounts, sObjectMgr, sWorld and urand.
 *
 * WHAT STAYS WITH THE CALLER is resolving the stock LIST, which is the part that
 * really differs -- a creature's comes from its entry, a bot NPC's from its
 * design row. Counting never asked what it was counting for.
 */
