...
class Player final: public Unit
{
    ...
    private:
        ...
        uint32 m_botNpcGossipMenuId;                        // tw_char.bot_npc; 0 for an ordinary character
    ...
    public:
        ...
        // Unit, not Creature: a bot NPC is a Player that sells things, and the
        // only thing this reads off the vendor is its guid.
        void SendBuyError(BuyResult msg, Unit* pVendor, uint32 item, uint32 param) const;
        ...
        // Unit, not Creature, for the same reason as SendBuyError: the body
        // calls GetFactionTemplateEntry, which every Unit has.
        float GetReputationPriceDiscount(Unit const* pVendor) const;
    ...
    public:
        ...
        /*
         * BOT NPCs. Creature and GameObject already override this (Creature.h:968,
         * GameObject.h:838) and Object's base returns 0, which is what made the
         * whole gossip pipeline reusable without touching it: PrepareGossipMenu,
         * SendPreparedGossip and OnGossipSelect all take a WorldObject* and ask
         * the source for its default menu. A player that is a bot NPC answers
         * with its bound menu; every other player answers 0 and behaves exactly
         * as it always has.
         *
         * Bound in tw_char.bot_npc and applied in LoadFromDB. See
         * docs/ai/content/playerbot-npcs.md.
         */
        uint32 GetDefaultGossipMenuId() const override { return m_botNpcGossipMenuId; }
        // Asks the binding table, NOT the npcflag. Those differ on purpose:
        // `.modify npcflag 1` on an ordinary bot is a probe and should not
        // silently switch its AI off, and a bot NPC whose menu is still 0 is
        // already an NPC and should already be standing still.
        bool IsBotNpc() const;
        void SetBotNpcGossipMenuId(uint32 menuId) { m_botNpcGossipMenuId = menuId; }

        /*
         * THE ID ITS QUESTS HANG OFF in `creature_questrelation` and
         * `creature_involvedrelation`, or 0 for every ordinary player.
         *
         * Asked of the design table on demand rather than cached beside
         * m_botNpcGossipMenuId, and the difference is warranted: the menu id is
         * read by a virtual on every gossip build, while these are read on a
         * click and on a status query. A cached copy would be one more thing
         * `reload bot_npc` and `.npcbot` have to remember to move.
         */
        uint32 GetBotNpcQuestGiverId() const;

        // A bot NPC answers for the quests its design points at. Every other
        // player answers false, which is what Object's default already did.
    ...
    public:
        ...
        void UpdateSpellsForLevel(uint32 oldLevel);
        bool IsLevelBoundClassSpell(SpellEntry const* spellInfo) const;
        void UnlearnSpellsAboveLevel();
        void LearnClassSpellsForLevel();
        void MaxLevelDependentSkills();
    private:
    ...
    public:
        ...
        // learnClassLevelSpells / learnDefaultSpells: cmangos training helpers.
        //
        // THESE WERE EMPTY STUBS until 2026-09-06, left over from the port with
        // a comment saying "Penqle has equivalents" - which was true, and
        // nobody ever called them. PlayerbotFactory::InitAvailableSpells calls
        // both, so every bot created by this server learned NO CLASS ABILITIES
        // AT ALL: a level 20 warrior companion had 47 spells, every one of them
        // a language, weapon proficiency or cooking recipe, and not Battle
        // Stance, Heroic Strike, Defensive Stance, Sunder Armor, Revenge or
        // Taunt. In game it read as "the tank does not cast anything".
...
// Instance system:
    ...
    public:
        ...
        // Put this player into a specific instance of a map, ahead of whatever
        // they are bound to. Housing uses it so a guest reaches somebody else's
        // house: a permanent bind wins over a group bind, so without an override
        // a visitor with a house of their own is routed straight back to it.
        // Consulted at the top of GetBoundInstanceSaveForSelfOrGroup, which is
        // the one place the core decides which instance you enter. Not saved --
        // it lasts for the visit and no longer.
        void SetForcedInstance(uint32 mapId, uint32 instanceId) { m_forcedInstanceMapId = mapId; m_forcedInstanceId = instanceId; }
        void ClearForcedInstance() { m_forcedInstanceMapId = 0; m_forcedInstanceId = 0; }
        uint32 GetForcedInstanceId(uint32 mapId) const { return (m_forcedInstanceMapId && m_forcedInstanceMapId == mapId) ? m_forcedInstanceId : 0; }
        uint32 m_forcedInstanceMapId;
        uint32 m_forcedInstanceId;
