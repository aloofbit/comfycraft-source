...
void Object::BuildValuesUpdate(uint8 updatetype, ByteBuffer * data, UpdateMask *updateMask, Player *target) const
{
    ...
    // HOUSING TRIED TO HANG A SELECTION HIGHLIGHT HERE. The server side is
    // sound and worth knowing about: GAMEOBJECT_DYN_FLAGS is built PER VIEWER a
    // few lines down, so GO_DYNFLAG_LO_ACTIVATE -- the yellow glimmer a quest
    // chest wears -- would have highlighted one player's chosen furniture for
    // that player alone, with no second object, no spell and no client patch.
    // Nothing appeared in game, with a fresh creation block or without one.
    //
    // BUT THAT TEST ONLY EVER PROVED IT FOR ONE OF THE THREE TYPES, and the
    // reason is the writer below rather than the client. Housing's types are
    // GENERIC (5), CHAIR (7) and MAP_OBJECT (14); the switch at the
    // GAMEOBJECT_DYN_FLAGS case emits the flag for QUESTGIVER, CHEST, GENERIC,
    // SPELL_FOCUS and GOOBER only, and everything else falls to `default:`,
    // which writes uint32(0) whatever the object's field says. So for CHAIR and
    // MAP_OBJECT the flag was ZEROED ON THE WIRE and the experiment could not
    // have shown anything. Only the GENERIC result stands.
    //
    // Note also what the flag's own definition says it is
    // (SharedDefines.h:329): "enables interaction with GO" -- an interaction
    // enable, not a highlight. The quest sparkle is a separate client
    // behaviour, which is the likelier reason GENERIC showed nothing too.
    // Re-testing means widening that switch, and see the warning in CLAUDE.md
    // about doing it through IsActivateToQuest.
    //
    // Removed rather than left switched off: it cost a map-id compare on every
    // gameobject in every update block, and a reader finding it here would
    // reasonably conclude a highlight exists. What housing uses instead is a
    // summoned glow standing on the object -- HouseMgr::RefreshSelectionMarker
    // -- plus the blink that re-creating it produces. Recoverable from git if
    // this is ever worth another look on a different object type.
    ...
    *data << (uint8)updateMask->GetBlockCount();
        ...
        for (uint16 index = 0; index < m_valuesCount; ++index)
        {
            if (updateMask->GetBit(index))
            {
                if (index == UNIT_NPC_FLAGS)
                {
                    ...
                    {
                        // A HOUSE CRITTER IS A GOSSIP NPC TO ITS OWNER, AND TO
                        // NOBODY ELSE -- so the flag is TAKEN OFF for everyone
                        // else, rather than added for the owner.
                        //
                        // THAT DIRECTION IS FORCED, and getting it backwards
                        // cost a build. Object::_SetCreateBits only sets a
                        // field's bit when its value is non-zero, so a flag
                        // that lives only in here is never in the creation
                        // block and this branch never runs. HouseMgr::
                        // DressCritter puts it on the object for real; every
                        // other rule below masks off for the same reason.
                        //
                        // No mode and so no blink -- unlike the gameobject type
                        // swap, ownership does not change while somebody is
                        // standing there, so this is simply correct from the
                        // creation block onwards and the client never has to
                        // re-read it.
                        //
                        // The guid compare comes first and settles it for every
                        // creature in the world that is not one of ours.
                        if ((appendValue & UNIT_NPC_FLAG_GOSSIP)
                            && GetGUIDLow() >= HOUSE_CRITTER_GUID_MIN
                            && GetGUIDLow() <= HOUSE_CRITTER_GUID_MAX
                            && !sHouseMgr.CanOwnerTalkTo(target, GetGUIDLow()))
                ...
                /*
                 * ONE BRANCH, THREE RULES, and it was two branches until a
                 * third was needed. `else if` chains on the same index are a
                 * trap: the second rule could only ever apply when the first
                 * did not, so a viewer in GM mode never got the video-mode
                 * treatment and a new rule added at the end would have been
                 * shadowed by both. Computing the value and writing it once
                 * makes the rules additive, which is what they always meant.
                 */
                else if (index == UNIT_FIELD_FLAGS)
                {
                    uint32 unitFlags = m_uint32Values[index];

                    // Video maker - hide unit name, etc ...
                    if (target->HasOption(PLAYER_VIDEO_MODE) && target != this)
                        unitFlags |= UNIT_FLAG_NOT_SELECTABLE;

                    // Gamemasters should be always able to select units and view auras
                    if (target->IsGameMaster())
                        unitFlags = (unitFlags | UNIT_FLAG_AURAS_VISIBLE) & ~UNIT_FLAG_NOT_SELECTABLE;

                    /*
                     * A BOT NPC IS NOT TOLD TO BE PLAYER-CONTROLLED.
                     *
                     * The client paints a player's name in the player colour
                     * rather than the reaction colour, which is why a bot NPC
                     * standing among real NPCs reads as blue where its
                     * neighbours are green. Faction is not the lever: the
                     * factionless rewrite below already hands every viewer their
                     * own faction template, so the bot is ALREADY friendly and
                     * still blue.
                     *
                     * UNIT_FLAG_PLAYER_CONTROLLED is the field-level expression
                     * of "this is a player", and the client's own
                     * UnitPlayerControlled() reads it. Masking it off for
                     * viewers is the same shape as the house critter's gossip
                     * flag and the faction template next door -- the server's
                     * copy is untouched, so everything that keys off the flag
                     * server-side still sees the truth.
                     *
                     * Whether the client actually takes the hint is the thing
                     * being measured; it may branch on OBJECT_FIELD_TYPE
                     * instead, which cannot be touched without breaking how a
                     * player model renders at all.
                     */
                    if (GetTypeId() == TYPEID_PLAYER && target != this && ToPlayer()->IsBotNpc())
                ...
                else if (index == UNIT_FIELD_FACTIONTEMPLATE)
                {
                    ...
                    if (owner && owner->IsPlayer())
                    {
                        ...
                        if (ft1 && ft2 && !ft1->IsFriendlyTo(*ft2))
                        {
                            ...
                            // ComfyCraft: every player is sent the viewer's own faction
                            // template, so the client resolves friendly out of its own
                            // unmodified FactionTemplate.dbc and paints a green name. No
                            // client patch needed. Attackability does not come from here
                            // - it comes from the FFA flag below, which the client honours
                            // over a friendly faction (this is how Gurubashi Arena works).
                            // Battlegrounds are exempt for the same reason as in
                            // GetReactionTo: they keep the stock faction path throughout.
                            // See docs/features/factionless.md piece 2.
                            if (sWorld.getConfig(CONFIG_BOOL_FACTIONLESS_ENABLE)
                                && !ownerPlayer->InBattleGround() && !target->InBattleGround())
                                forceFriendly = true;
                            else if (ownerPlayer->IsInSameRaidWith(target))
                                if (ownerPlayer->IsInInterFactionMode() && target->IsInInterFactionMode())
                                    forceFriendly = true;
                        }
                    }
                ...
                {
                    uint32 playerFlags = m_uint32Values[index];
                    Player* owner = ((Unit*)this)->GetCharmerOrOwnerPlayerOrPlayerItself();

                    // ComfyCraft: a PvP-flagged player is advertised to everyone as FFA.
                    // That is what lets the 1.12 client offer an attack on someone its own
                    // DBC calls friendly - the Gurubashi Arena mechanic - now that every
                    // player is sent a friendly faction template above. Keyed on this
                    // player's own flag only, never the viewer's, which would go stale the
                    // moment the viewer's own flag changed.
                    //
                    // This rides on PLAYER_FLAGS being sent at all, and flagging for PvP
                    // writes UNIT_FIELD_FLAGS instead - so Unit::SetPvP forces this index
                    // out. Without that the bit had to wait for something unrelated to
                    // dirty PLAYER_FLAGS (going contested on the first swing, or the
                    // sanctuary flag clearing on leaving a town), and the opening attack
                    // of a fight was one-sided.
                    //
                    // Server-side hostility does not read this at all; GetReactionTo
                    // uses IsPvP() directly. See docs/features/factionless.md piece 1.
                    if (sWorld.getConfig(CONFIG_BOOL_FACTIONLESS_ENABLE) && owner && owner->IsPvP()
                        && !owner->InBattleGround() && !target->InBattleGround())
                        playerFlags |= PLAYER_FLAGS_FFA_PVP;

                    // RAID ally-horde : pas de flag FFA. Load-bearing once every flagged
                    // player carries FFA - it is what stops your own raid looking attackable.
                    if ((playerFlags & PLAYER_FLAGS_FFA_PVP) && owner && owner != target && owner->IsInSameRaidWith(target))
                        playerFlags &= ~PLAYER_FLAGS_FFA_PVP;
    ...
    else if (isType(TYPEMASK_GAMEOBJECT))                   // gameobject case
    {
        for (uint16 index = 0; index < m_valuesCount; ++index)
        {
            ...
            {
                // send in current format (float as float, uint32 as uint32)
                // EDIT MODE MAKES YOUR OWN FURNITURE A GOOBER, FOR YOU ALONE.
                //
                // The client will not highlight or click a GENERIC (5), CHAIR
                // (7) or MAP_OBJECT (14) -- GameObject::Use only raises gossip
                // for QUESTGIVER and GOOBER -- which is the whole reason edit
                // mode used to summon a gear to click instead. Told the object
                // is a GOOBER, the client lights it under the cursor and sends
                // the click, and the furniture becomes its own handle.
                //
                // PER VIEWER, exactly like GAMEOBJECT_DYN_FLAGS below, and that
                // is what makes it safe. SetGoType() would have been the
                // obvious route and is a trap: the server would then see a
                // GOOBER whose TEMPLATE is a chair, so GameObject::Update's
                // GO_JUST_DEACTIVATED case reads GetGOInfo()->goober.spellId --
                // the wrong member of the template union -- down a path that
                // ends in the object being destroyed. Here GetGoType() still
                // answers CHAIR everywhere on the server, so Update(), Use()'s
                // switch and every union read stay correct.
                //
                // It also means nothing has to be put back. Outside the edit
                // session the true type is sent, so a chair is a chair and can
                // be sat on again; a crash mid-edit leaves nothing behind,
                // because nothing was ever changed.
                //
                // NOT BUILDINGS. THE CLIENT PICKS ITS RENDERER FROM THIS
                // FIELD, which is the one thing the type is used for besides
                // interaction. MAP_OBJECT (14) is the WMO path; GOOBER is the
                // M2 path. Tell it a WMO is a GOOBER and the display id goes to
                // a renderer that cannot read it, so the building vanishes and
                // leaves a blue placeholder box standing in the room -- and,
                // having no model, it cannot be moused over either, so the
                // change costs the highlight it was made for.
                //
                // Found in play 2026-09-03. Housing's other two types, GENERIC
                // (5) and CHAIR (7), are both M2 doodads and swap cleanly;
                // buildings keep their real type and are moved with the
                // commands and the addon, as they were before any of this.
                //
                // The guid compare comes first and settles it for every
                // gameobject in the world that is not housing's, which is all
                // but a few hundred.
                if (index == GAMEOBJECT_TYPE_ID
                    && GetGUIDLow() >= HOUSE_GO_GUID_MIN
                    && ((GameObject*)this)->GetGoType() != GAMEOBJECT_TYPE_MAP_OBJECT
                    && sHouseMgr.InEditSession(target, GetGUIDLow()))
                {
                    *data << uint32(GAMEOBJECT_TYPE_GOOBER);
                }
    ...
    if (IsUnit() && (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) || b_IsPossessed))
    {
        if (target->IsUnit() && (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) || b_IsPossessed))
        {
            if (selfPlayerOwner && targetPlayerOwner)
            {
                ...
                // ComfyCraft: race does not decide who you may hit, the PvP flag does.
                // Both flagged is a fight; anyone else is a friendly stranger whatever
                // their race. Self, duel, group and sanctuary all returned above, so
                // this never overrides them.
                //
                // Battlegrounds are left out entirely. Everyone inside one is flagged
                // (UpdateZone, AREATEAM_NONE), so without this the rule would make
                // team-mates hostile to each other and would only be saved by them
                // sharing a BG raid group. That happens to hold, but a battleground
                // staying on the stock faction path is far easier to reason about than
                // one that is correct because three mechanisms line up.
                // See docs/features/factionless.md piece 1.
                if (sWorld.getConfig(CONFIG_BOOL_FACTIONLESS_ENABLE)
                    && !selfPlayerOwner->InBattleGround() && !targetPlayerOwner->InBattleGround())
                    return (selfPlayerOwner->IsPvP() && targetPlayerOwner->IsPvP())
                        ? REP_HOSTILE : REP_FRIENDLY;
            }
        }
