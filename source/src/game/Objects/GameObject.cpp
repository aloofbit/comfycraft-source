...
void GameObject::Use(Unit* user)
{
    ...
    if (user->IsPlayer())
    {
        ...
        // HOUSING FURNITURE HAS NO SCRIPT TO FIND. It uses stock
        // gameobject_template rows we do not own, so script_name is not ours to
        // set and OnGameObjectUse above can never match one. This is the same
        // seam one line lower down, keyed on the housing guid range instead of
        // a template.
        //
        // It has to be BEFORE the type switch for the same reason pGOHello does:
        // past here a click runs the real type's branch, which for a chair means
        // sitting on it in the middle of moving it.
        if (GetGUIDLow() >= HOUSE_GO_GUID_MIN && HouseGameObjectUse((Player*)user, GetGUIDLow()))
            return;
    }
