...
void ReputationMgr::LoadFromDB(QueryResult *result)
{
    ...
    ApplyFactionlessPeace();
}

void ReputationMgr::ApplyFactionlessPeace()
{
    if (!sWorld.getConfig(CONFIG_BOOL_FACTIONLESS_ENABLE))
        return;

    // The total standing every listed faction is lifted to. 0 is Neutral, which is
    // all Player::CanInteractWithNPC needs -- it refuses at REP_UNFRIENDLY or below
    // -- and it buys no reputation discount. 3000 is Friendly, and the difference
    // in game is a yellow name against a green one. Config, not a constant, because
    // that is a taste question and not worth a rebuild.
    int32 const target = int32(sWorld.getConfig(CONFIG_UINT32_FACTIONLESS_PEACE_STANDING));

    for (auto& itr : m_factions)
    {
        FactionState& state = itr.second;

        if (!sWorld.IsFactionlessPeaceFaction(state.ID))
            continue;

        FactionEntry const* factionEntry = sObjectMgr.GetFactionEntry(state.ID);
        if (!factionEntry)
            continue;

        // Only touch a faction this character's race is actually born at war with.
        // A Human is at war with Orgrimmar and at peace with Stormwind, and both
        // are in the list, so without this an Alliance character would have their
        // own capital's standing flattened to Neutral.
        if (!(GetDefaultStateFlags(factionEntry) & FACTION_FLAG_AT_WAR))
            continue;

        // Move the STANDING, never the base.
        //
        // The client is never sent the base. SendInitialReputations writes the
        // standing alone and the client adds a base out of its own Faction.dbc,
        // which still says -42000 for the other team's cities. So a base lowered
        // server-side leaves the two disagreeing: the server sees Neutral and
        // stops the guards attacking, while the client still sees Hated, paints
        // every NPC red, and never sends CMSG_GOSSIP_HELLO at all -- the server
        // would have allowed the conversation, it was simply never asked for one.
        // That exact split cost a debugging round on 2026-09-09; sql/custom/074
        // has the long version.
        int32 const base = GetBaseReputation(factionEntry);
        if (base + state.Standing < target)
        {
            state.Standing = target - base;
            state.needSend = true;
            state.needSave = true;
        }

        // The stored flag is re-applied verbatim by LoadFromDB above, so clearing
        // it in the database alone does not hold. Clear it here, after the load.
