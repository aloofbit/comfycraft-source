...
Transport::Transport() : GameObject(),
    _transportInfo(nullptr), _isMoving(true), _pendingStop(false), _frozen(false),
...
void Transport::Update(uint32 update_diff, uint32 /*time_diff*/)
{
    ...
    // `.gobject stop`. Everything below this line is what moves the server's
    // copy of the ship, so holding here holds all of it at once. Deliberately
    // placed after the AI update so a frozen transport still runs its script.
    if (_frozen)
        return;
