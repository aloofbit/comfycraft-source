...
class Transport : public GameObject
{
    ...
    public:
        ...
        // A PROBE SWITCH, not a feature. `.gobject stop` sets it, and Update
        // returns immediately while it is set, so the server's copy of the ship
        // holds still: no path progress, no spline evaluation, no relocation.
        // It exists to answer one question -- whether a 1.12 client follows the
        // server's idea of where a transport is, or free-runs along its own copy
        // of the path after the create packet. `GetPathProgress()` is sent in
        // exactly one place (Object.cpp, the create block) and never again,
        // which SUGGESTS free-running, but nobody here has watched it happen.
        // docs/notes/airships.md.
        bool IsFrozen() const { return _frozen; }
        void SetFrozen(bool val) { _frozen = val; }
