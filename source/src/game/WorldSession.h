...
class WorldSession
{
    ...
    public:
        ...
        // NPC
        //
        // onlyItems, when given, is a whitelist of item entries: anything not
        // in it is left out of the window. That is how the housing furniture
        // shop draws one page per category out of a single npc_vendor list.
        //
        // SAFE BECAUSE THE CLIENT BUYS BY ITEM ENTRY, NOT BY LIST INDEX.
        // CMSG_BUY_ITEM carries the entry (HandleBuyItemOpcode), and
        // BuyItemFromVendor looks it up in the creature's FULL vendor list, so
        // a filtered window never sends the wrong thing back. Nothing about
        // the vendor row changes -- this is presentation only.
        //
        // `vendorTemplate` NAMES A SHOP OUTRIGHT, instead of taking whatever the
        // NPC is linked to. That is what lets ONE NPC offer several shops from a
        // gossip menu: the option carries the id and this sends that list.
        //
        // THE SESSION HAS TO REMEMBER IT, and that is the whole cost of the
        // feature. CMSG_BUY_ITEM carries the vendor guid and an item entry and
        // nothing else -- the client cannot tell us which of the NPC's shops the
        // window came from -- so BuyItemFromVendor would check the purchase
        // against the NPC's own list and refuse an item it had just displayed.
        // See GetOpenVendorTemplate below.
        void SendListInventory(ObjectGuid guid, uint8 menu_type = VENDOR_MENU_ALL,
                               std::set<uint32> const* onlyItems = nullptr,
                               uint32 vendorTemplate = 0);

        /*
         * WHICH SHOP THIS PLAYER CURRENTLY HAS OPEN AT `vendorGuid`, or 0 for the
         * ordinary case of "whatever that NPC sells".
         *
         * GUARDED ON THE GUID, and that guard is the security of the thing. The
         * answer is only given for the vendor the list was actually sent for, so
         * opening a shop at one NPC cannot be used to buy from it while standing
         * at another. Every SendListInventory sets both fields, so walking up to
         * a second vendor clears the first by ordinary use.
         */
        uint32 GetOpenVendorTemplate(ObjectGuid vendorGuid) const
        {
            return m_openVendorGUID == vendorGuid ? m_openVendorTemplate : 0;
        }

        /*
         * THE SAME PROBLEM AGAIN, for the other frame.
         *
         * `custom_merchant` is keyed by creature entry, and the addon's reply
         * carries only a row id. The server worked out which merchant was meant
         * by looking up the gossip guid and reading its creature entry -- which
         * a BOT NPC can never satisfy, because it has no creature entry at all.
         *
         * So the shop is remembered here when it is opened, exactly like the
         * vendor template above and guarded on the guid for the same reason: the
         * answer is only given for the unit the list was actually sent for.
         */
        void SetOpenMerchant(ObjectGuid vendorGuid, uint32 entry)
        {
            m_openMerchantGUID = vendorGuid;
            m_openMerchantEntry = entry;
        }
        uint32 GetOpenMerchantEntry(ObjectGuid vendorGuid) const
        {
            return m_openMerchantGUID == vendorGuid ? m_openMerchantEntry : 0;
        }
        ObjectGuid GetOpenMerchantGuid() const { return m_openMerchantGUID; }
        ...
        // Pushes a bot NPC's quest mark after the quest log has changed under
        // it. Does nothing for anything that is not a player. QuestHandler.cpp
        // has the measurement and the reason.
        void SendQuestGiverMarkIfPlayer(Object* questgiver);
    ...
    private:
        ...
        // The last vendor window this session was sent, and the shop it showed.
        // 0 means the NPC's own stock, which is every case but a gossip option
        // that named a shop. Set together, always, by SendListInventory.
        ObjectGuid m_openVendorGUID;
        uint32     m_openVendorTemplate;
        ObjectGuid m_openMerchantGUID;
        uint32     m_openMerchantEntry;
