...
// tw_world.rented_mount -- which stable NPCs will hire you a mount, and on what
// terms. This used to be a switch statement over twelve creature entries in
// src/scripts/miscellaneous/random_scripts_1.cpp, which meant a core rebuild per
// mount. sql/custom/079_rented_mounts.sql documents every column.
//
// spellId is the mount's OWN spell, not a rental spell, because Spell.dbc holds
// only twelve Rented-something spells and the client ships the same twelve -- a
// thirteenth would mean a patched Spell.dbc on every client. So the cast gives
// the right model, name and icon, and speed/duration clamp it to rental terms
// afterwards. Either at 0 means "leave the spell as it is".
struct RentedMount
{
    uint32 spellId  = 0;
    uint32 cost     = 0;                                    // copper
    uint32 speed    = 0;                                    // mounted speed %
    uint32 duration = 0;                                    // milliseconds
    uint32 textId   = 0;                                    // npc_text greeting
    std::string noun;                                       // "horse", in "Hire this horse for 50 copper."
};

typedef robin_hood::unordered_map<uint32 /*creature entry*/, RentedMount> RentedMountMap;

// display id -> the mount spell that produces it. Built from Spell.dbc rather
// than from a table, because it exists for one narrow job: a renter whose
// creature_template carries several display ids shows a RANDOM one of them
// (Creature::ChooseDisplayId), re-rolled every time it spawns, so its row's
// single spell can only ever match one of its faces.
typedef robin_hood::unordered_map<uint32 /*display id*/, uint32 /*spell id*/> RentedMountDisplayMap;
...
// vendor_cost.id -> what that row costs. sql/custom/094.
typedef robin_hood::unordered_map<uint32, VendorCost> CacheVendorCostMap;
...
// Raised from 15 to 24 on 2026-09-14, to match characters.name after
// sql/custom/089. This is the SERVER'S internal cap, deliberately larger than
// MAX_PLAYER_NAME, and normalizePlayerName returns FALSE above it -- which is
// not a soft failure anywhere it matters:
//
//   ObjectMgr::LoadPlayerCacheData normalises every character's name and only
//   caches the ones that pass. A bot NPC called "Innkeeper Allison" was
//   therefore absent from the player cache ENTIRELY -- no guid-to-account, no
//   guid-to-name, nothing -- and the only symptom was "is placed on character
//   4651, which has no account". Nothing logged the name as the reason.
//
// MAX_PLAYER_NAME stays 12: a real player still goes through CheckPlayerName,
// which caps there and refuses spaces. Only bot NPCs are exempt from that, and
// only the server can give them such a name.
#define MAX_INTERNAL_PLAYER_NAME 24                         // max server internal player name length ( > MAX_PLAYER_NAME for support declined names )
...
class ObjectMgr
{
    ...
    public:
        ...
        // `reload` keeps the existing entries in place instead of clearing them.
        // Every live GameObject caches a GameObjectInfo const* into this map
        // (GameObject::m_goInfo, the only long-lived pointer into it), and the
        // map is a std::map, so overwriting a node leaves those pointers valid
        // while clearing would dangle every one of them. See the note on
        // HandleReloadGameObjectTemplateCommand.
        void LoadGameobjectInfo(bool reload = false);
        ...
        /*
         * WHAT VENDOR ROWS COST, keyed by vendor_cost.id.
         *
         * MUST BE LOADED BEFORE THE VENDOR TABLES, and `reload npc_vendor` runs
         * it first for that reason: a cost_id the cache does not know reads as
         * "no cost" and the row quietly sells at the item's own price, which is
         * a wrong price rather than a missing one.
         */
        void LoadVendorCosts();

        VendorCost const* GetVendorCost(uint32 id) const
        {
            if (!id)
                return nullptr;
            auto iter = m_CacheVendorCostMap.find(id);
            return iter == m_CacheVendorCostMap.end() ? nullptr : &iter->second;
        }
        void LoadTrainerTemplates();
        void LoadTrainers() { LoadTrainers("npc_trainer", false); }

        void LoadRentedMounts();
        void BuildRentedMountDisplayIndex();
        RentedMount const* GetRentedMount(uint32 creatureEntry) const
        {
            auto itr = m_RentedMountMap.find(creatureEntry);
            return itr != m_RentedMountMap.end() ? &itr->second : nullptr;
        }

        // For a renter wearing one of several random models: the mount spell
        // that produces the model actually standing there. 0 when no mount
        // spell uses that display, which is a real case -- two of the Riding
        // Tiger's four cat skins belong to no creature and have no spell.
        uint32 GetRentedMountSpellForDisplay(uint32 displayId) const
        {
            auto itr = m_RentedMountByDisplay.find(displayId);
            return itr != m_RentedMountByDisplay.end() ? itr->second : 0;
        }
    ...
    private:
        ...
        RentedMountMap m_RentedMountMap;
        RentedMountDisplayMap m_RentedMountByDisplay;
        ...
        CacheVendorCostMap m_CacheVendorCostMap;
