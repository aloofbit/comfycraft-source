...
#include "Chat.h"
#include "ScriptMgr.h"                                      // Script_IsAIControlled
...
static void GetDataForXPAtKill_helper(Player* player, Unit const* victim, uint32& sum_level, uint32& real_count, uint32& bot_count, Player*& member_with_max_level, Unit*& not_gray_member_with_max_level)
{
    // Script_IsAIControlled is the core's own bot seam - the same one the login
    // and movement handlers use - so this needs no playerbot header.
...
void Group::GetDataForXPAtKill(Unit const* victim, uint32& count, uint32& sum_level, uint32& real_count, uint32& bot_count, Player*& member_with_max_level, Unit*& not_gray_member_with_max_level, Player* additional)
{
    for (GroupReference *itr = GetFirstMember(); itr != nullptr; itr = itr->next())
    {
        ...
        ++count;
        GetDataForXPAtKill_helper(member, victim, sum_level, real_count, bot_count, member_with_max_level, not_gray_member_with_max_level);
    }
    ...
    if (additional)
    {
        ...
        {
            ++count;
            GetDataForXPAtKill_helper(additional, victim, sum_level, real_count, bot_count, member_with_max_level, not_gray_member_with_max_level);
        }
    }
}
...
static void RewardGroupAtKill_helper(Player* pGroupGuy, Unit* pVictim, uint32 count, bool PvP, uint32 real_count, uint32 bot_count, bool is_dungeon, Unit* not_gray_member_with_max_level, Player* member_with_max_level, uint32 /*xp*/)
{
    ...
    if (!PvP)
    {
        // The party no longer divides the kill. Each member's base is what they
        // would have earned killing it ALONE, against their own level, and the
        // party applies a multiplier on top. withChallenges is false here for the
        // same reason the old shared `xp` passed false: the challenge modifiers
        // are applied per member a few lines below, and would double up.
        const uint32 base = MaNGOS::XP::Gain(pGroupGuy, static_cast<Creature*>(pVictim), false);

        // Everyone real except me. A bot being rewarded has no "other real
        // players minus myself" to subtract, so it takes the count whole.
        const uint32 realOthers = Script_IsAIControlled(pGroupGuy)
                                ? real_count
                                : (real_count ? real_count - 1 : 0);

        const uint32 topLevel = member_with_max_level ? member_with_max_level->GetLevel()
                                                      : pGroupGuy->GetLevel();

        float rate = MaNGOS::XP::group_xp_rate(realOthers, bot_count)
                   * MaNGOS::XP::carry_xp_factor(topLevel, pGroupGuy->GetLevel());
        ...
        {
            // No grey-member halving any more: base is computed against this
            // member's own level, and XP::Gain already returns 0 when the victim
            // is grey to them, so the case that halving approximated is now exact.
            uint32 itr_xp = uint32(base * rate);
...
void Group::RewardGroupAtKill(Unit* pVictim, Player* pPlayerTap)
{
    ...
    GetDataForXPAtKill(pVictim, count, sum_level, real_count, bot_count, member_with_max_level, not_gray_member_with_max_level, pPlayerTap);
    ...
    if (member_with_max_level)
    {
        ...
        // The split is gone, so there is no single party rate any more: each
        // member's share is worked out from real_count / bot_count inside the
        // helper. The raid flag went with it, having only ever fed a function
        // that ignored it.
        //
        // `xp` above is still computed because the member_with_max_level guard
        // reads it, but it is no longer what anybody is paid.
        ...
        for (GroupReference *itr = GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            ...
            RewardGroupAtKill_helper(pGroupGuy, pVictim, count, PvP, real_count, bot_count, is_dungeon, not_gray_member_with_max_level, member_with_max_level, xp);
        }
        ...
        if (pPlayerTap)
        {
            ...
            if (pPlayerTap->IsAtGroupRewardDistance(pVictim))
                RewardGroupAtKill_helper(pPlayerTap, pVictim, count, PvP, real_count, bot_count, is_dungeon, not_gray_member_with_max_level, member_with_max_level, xp);
        }
    }
}
