...
void WorldSession::SendPetNameQuery(ObjectGuid petguid, uint32 petnumber)
{
    ...
    // A house critter's given name. Creature::GetName() is hardcoded to the
    // template -- return GetCreatureInfo()->name.c_str() -- so without this an
    // animal would always answer with its species. False for anything that is
    // not ours, and for one that has never been named.
    HouseCritterPetName(pet->GetGUIDLow(), name);
