...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket & recv_data)
{
    ...
    Object* questgiver = _player->GetObjectByTypeMask(guid, TYPEMASK_CREATURE_GAMEOBJECT_OR_PLAYER);
    if (!questgiver)
    {
    ...
    switch (questgiver->GetTypeId())
    {
        ...
        /*
         * A BOT NPC. No sScriptMgr.GetDialogStatus step, because its two
         * overloads take a Creature and a GameObject and a script attached to a
         * bot NPC is not a thing that exists -- what a bot does lives in its
         * gossip menu and in dialog_lua.
         *
         * WHETHER THE CLIENT DRAWS THE MARK OVER A PLAYER'S HEAD AT ALL is the
         * open question this arm exists to answer. The server answering
         * honestly is the half we control; if the 1.12 client only draws the
         * icon over units it considers NPCs, this is a correct reply nobody
         * sees, and the quest is still reachable by talking.
         */
        case TYPEID_PLAYER:
...
/*
 * PUSH A BOT NPC'S QUEST MARK, because the client will not come back and ask.
 *
 * MEASURED IN GAME 2026-09-15, with a temporary log on the handler above. The
 * 1.12 client sends CMSG_QUESTGIVER_STATUS_QUERY for a bot NPC exactly ONCE,
 * when the unit comes into view. For a CREATURE it asks again every time the
 * quest log changes, which is what keeps a "!" honest; it does not do that for
 * a player. So accepting a quest from a bot left the "!" sitting over its head
 * until you walked out of range and back, while the server held the right
 * answer the whole time and was never asked for it.
 *
 * THE CLIENT DRAWS A MARK IT DID NOT ASK FOR PERFECTLY WELL, which is the same
 * narrow refusal the merchant window turned out to have: it would not ASK for
 * a bot NPC's stock and cheerfully drew the batch we pushed unasked. Worth
 * remembering as a pattern rather than as two coincidences -- when this client
 * refuses something for a player guid, test the neighbouring question before
 * concluding it cannot be done.
 *
 * SENDING ONE IS ALL THIS DOES. Working out WHICH ones need it belongs to
 * Player::UpdateForQuestWorldObjects, which already means "the quest log
 * changed, refresh what they can see" and already holds the visibility set.
 *
 * This was called from the accept and reward handlers at first, on the
 * questgiver whose guid was in hand. That is wrong for the ordinary case of a
 * quest given by one NPC and handed in to another: the second one never heard
 * about it, so it showed the quest as complete inside its gossip window and
 * carried no mark at all. Nothing calls it that way any more.
 */
void WorldSession::SendQuestGiverMarkIfPlayer(Object* questgiver)
{
    if (!questgiver || questgiver->GetTypeId() != TYPEID_PLAYER)
        return;

    _player->PlayerTalkClass->SendQuestGiverStatus(
        getDialogStatus(_player, questgiver, DIALOG_STATUS_NONE), questgiver->GetObjectGuid());
}
...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket & recv_data)
{
    ...
    /*
     * A BOT NPC, WHICH REACHES THIS HANDLER ONLY IF THE CLIENT DECIDES IT
     * SHOULD. Right-clicking a bot NPC has always sent CMSG_GOSSIP_HELLO; what
     * the client does once UNIT_NPC_FLAG_QUESTGIVER is also set is not
     * something reading the server can answer, and if it switches opcodes
     * without this arm the NPC simply goes mute -- which would look like a
     * broken bot rather than like a client preference, and would have been an
     * expensive thing to misread.
     *
     * So both doors lead to the same room. GetNPCIfCanInteractWith below cannot
     * serve this one: it resolves the guid with GetAnyTypeCreature, which
     * answers nullptr for a player.
     */
    if (guid.IsPlayer())
    {
        Player* pBot = ObjectAccessor::FindPlayer(guid);
        if (!pBot || !pBot->GetBotNpcQuestGiverId() || pBot == _player)
            return;

        if (!_player->IsWithinDistInMap(pBot, INTERACTION_DISTANCE))
            return;

        if (GetPlayer()->HasUnitState(UNIT_STAT_FEIGN_DEATH))
            GetPlayer()->RemoveSpellsCausingAura(SPELL_AURA_FEIGN_DEATH);

        _player->PrepareGossipMenu(pBot, pBot->GetDefaultGossipMenuId());
        _player->SendPreparedGossip(pBot);
        return;
    }
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket & recv_data)
{
     ...
     *
     * TWO WAYS A PLAYER CAN PUT A QUEST IN FRONT OF YOU, and this knew about
     * one. CanShareQuest is the party-share case: the offerer has the quest in
     * their own log and it carries QUEST_FLAGS_SHARABLE. A BOT NPC has neither
     * -- it is a questgiver, not a sharer -- so the whole condition collapsed
     * to false and the accept was dropped.
     *
     * Measured in game 2026-09-15, and worth recording because of HOW it
     * failed: the mark drew, the gossip line drew, the offer scroll drew with
     * its Accept button, and pressing Accept did nothing at all. Every visible
     * step of the sequence had already succeeded, so the one broken line was
     * the last place anybody would look.
     *
     * Player::HasQuest answers the questgiver case and answers false for every
     * ordinary player, so asking both widens this for bot NPCs and for nothing
     * else.
     */
    bool const offeredByGiver = pObject && pObject->HasQuest(quest);
    bool const offeredBySharer = pObject && pObject->GetTypeId() == TYPEID_PLAYER &&
                                 static_cast<Player*>(pObject)->CanShareQuest(quest);

    if (!pObject || (!offeredByGiver && !offeredBySharer))
    {
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket & recv_data)
{
    ...
    // PLAYER is in the mask for bot NPCs. Player::HasQuest and HasInvolvedQuest
    // below answer for one; every other player answers false, so widening the
    // mask opens nothing that was closed on purpose.
    Object* pObject = _player->GetObjectByTypeMask(guid, TYPEMASK_CREATURE_GAMEOBJECT_PLAYER_OR_ITEM);
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket & recv_data)
{
    ...
    WorldObject* pObject = (WorldObject*)_player->GetObjectByTypeMask(guid, TYPEMASK_CREATURE_GAMEOBJECT_OR_PLAYER);
    if (!pObject)
        return;
...
void WorldSession::HandleQuestgiverRequestRewardOpcode(WorldPacket & recv_data)
{
    ...
    Object* pObject = _player->GetObjectByTypeMask(guid, TYPEMASK_CREATURE_GAMEOBJECT_OR_PLAYER);
...
uint32 WorldSession::getDialogStatus(Player *pPlayer, Object* questgiver, uint32 defstatus)
{
    ...
    switch (questgiver->GetTypeId())
    {
        ...
        // A bot NPC. GetEntry() is 0 on a player, which is why this cannot
        // share the creature arm: the id comes off the design instead.
        case TYPEID_PLAYER:
        {
            uint32 const giver = static_cast<Player*>(questgiver)->GetBotNpcQuestGiverId();
            if (!giver)
