...
void WorldSession::HandleGossipHelloOpcode(WorldPacket & recv_data)
{
    ...
    /*
     * A BOT NPC -- a character acting as an NPC. Measured 2026-09-13: the 1.12
     * client draws the talk cursor on a PLAYER unit carrying UNIT_NPC_FLAGS,
     * sends this opcode with the player's guid, and renders whatever menu comes
     * back keyed to that guid. docs/ai/content/playerbot-npcs.md.
     *
     * This does NOT go through GetNPCIfCanInteractWith, and cannot: that ends in
     * GetMap()->GetAnyTypeCreature(guid), which is nullptr for a player guid.
     * The checks it would have run are spelled out here instead, and are the
     * subset that means anything for a player -- alive, flagged, in reach, not
     * hostile. There is no charm, no invisibility-for-alive and no
     * stablemaster-is-a-hunter case to answer.
     *
     * Everything downstream is the stock pipeline, unchanged: PrepareGossipMenu
     * and SendPreparedGossip take a WorldObject*, and a bot NPC answers
     * GetDefaultGossipMenuId with its bound menu. So the DIALOG is ordinary
     * `gossip_menu` / `gossip_menu_option` data and `.reload gossip_menu` edits
     * it, with nothing new to reload.
     */
    if (guid.IsPlayer())
    {
        Player* pNpc = GetPlayer()->GetMap()->GetPlayer(guid);

        if (!pNpc || pNpc == GetPlayer() || !pNpc->IsAlive() ||
            !pNpc->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP) ||
            !GetPlayer()->IsWithinDistInMap(pNpc, INTERACTION_DISTANCE) ||
            pNpc->IsHostileTo(GetPlayer()))
        {
            DEBUG_LOG("WORLD: HandleGossipHelloOpcode - %s is not a reachable bot NPC.", guid.GetString().c_str());
            return;
        }
        ...
        /*
         * TELL THE IDLE DRIVER SOMEBODY IS TALKING TO IT, so it does not choose
         * this moment to wander off and tend the fire. The same note is what
         * later fires the "after somebody talks to it" beat, because there is
         * no close event to hang that on -- BotNpcIdle::NoteTalk has the
         * measurement.
         *
         * Find, not create: an NPC that does nothing unprompted has no driver
         * and no reason to grow one over a conversation.
         */
        if (BotNpcIdle* idle = pNpc->GetMap()->FindBotNpcIdle())
            idle->NoteTalk(guid, GetPlayer()->GetObjectGuid());

        GetPlayer()->PrepareGossipMenu(pNpc, pNpc->GetDefaultGossipMenuId());
        GetPlayer()->SendPreparedGossip(pNpc);
        return;
    }
    ...
    // A house critter's menu, which has no script for OnGossipHello to find --
    // see the note beside the matching hook in HandleGossipSelectOptionOpcode.
    // Answers false for anything that is not this player's animal.
    if (pCreature->GetGUIDLow() >= HOUSE_CRITTER_GUID_MIN
        && HouseCritterGossipHello(_player, pCreature->GetGUIDLow()))
        return;
...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket & recv_data)
{
    ...
    if (guid.IsAnyTypeCreature())
    {
        ...
        // Housing critters, which have no script for OnGossipSelect to find --
        // they use stock creature_template rows we do not own, so script_name
        // is not ours to set.
        if (pCreature->GetGUIDLow() >= HOUSE_CRITTER_GUID_MIN
            && HouseCritterGossipSelect(_player, pCreature->GetGUIDLow(), sender, action,
    ...
    {
        // A bot NPC. Same shape as the creature arm next door, minus the parts
        // that have no meaning for a player: no movement pause (a bot NPC is
        // told to stay, not scripted), and no script hook, since script_name
        // lives on creature_template and a character has no template at all.
        Player* pNpc = GetPlayer()->GetMap()->GetPlayer(guid);
        ...
        if (!pNpc || pNpc == GetPlayer() || !pNpc->IsAlive() ||
            ...
            !GetPlayer()->IsWithinDistInMap(pNpc, INTERACTION_DISTANCE))
        {
            DEBUG_LOG("WORLD: HandleGossipSelectOptionOpcode - %s is not a reachable bot NPC.", guid.GetString().c_str());
            return;
        }

        // Every click counts as still talking. See the matching hook in
        // HandleGossipHelloOpcode.
        if (BotNpcIdle* idle = pNpc->GetMap()->FindBotNpcIdle())
            idle->NoteTalk(guid, GetPlayer()->GetObjectGuid());

        GetPlayer()->OnGossipSelect(pNpc, gossipListId);
    }
    else if (guid.IsGameObject())
    {
        ...
        // Housing furniture, which has no script for OnGossipSelect to find --
        // see the note beside the matching hook in GameObject::Use.
        if (pGo->GetGUIDLow() >= HOUSE_GO_GUID_MIN
            && HouseGameObjectGossipSelect(_player, pGo->GetGUIDLow(), sender, action,
                                           code.empty() ? nullptr : code.c_str()))
            return;
