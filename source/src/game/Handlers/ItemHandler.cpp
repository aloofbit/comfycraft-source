...
void WorldSession::SendListInventory(ObjectGuid vendorguid, uint8 menu_type,
                                     std::set<uint32> const* onlyItems,
                                     uint32 vendorTemplate)
{
    DEBUG_LOG("WORLD: Sent SMSG_LIST_INVENTORY");

    /*
     * RECORDED BEFORE ANY EARLY RETURN, and deliberately so. Every refusal below
     * is also a window that did not open, and leaving a stale shop remembered
     * across one would let the next purchase be checked against a list this
     * player was never shown.
     *
     * WorldSession::GetOpenVendorTemplate has the reasoning; the short version
     * is that CMSG_BUY_ITEM does not say which of an NPC's shops it came from,
     * so the server has to remember.
     */
    m_openVendorGUID = vendorguid;
    m_openVendorTemplate = vendorTemplate;

    Creature *pCreature = nullptr;

    /*
     * A BOT NPC -- a character acting as an NPC, see BotNpc.h. Everything below
     * this branch works on `pVendor`, which is the creature in every ordinary
     * case and the bot's Player unit here.
     *
     * IT CANNOT GO THROUGH GetNPCIfCanInteractWith, for the same reason
     * HandleGossipHelloOpcode cannot: that ends in GetAnyTypeCreature(guid),
     * which is nullptr for a player guid. The checks it would have run are
     * spelled out here instead, and are the subset that means anything for a
     * player -- alive, flagged, in reach, not hostile.
     */
    ...
    {
        Player* pNpc = GetPlayer()->GetMap()->GetPlayer(vendorguid);
        ...
        {
            DEBUG_LOG("WORLD: SendListInventory - %s is not a reachable bot NPC vendor.", vendorguid.GetString().c_str());
            _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, nullptr, ObjectGuid(), 0);
            return;
        }
    ...
    // Stop the npc if moving. Creatures only: a bot NPC stands where
    // `.npcbot place` put it and has no out-of-combat movement to pause.
    if (pCreature && !pCreature->HasExtraFlag(CREATURE_FLAG_EXTRA_NO_MOVEMENT_PAUSE))
        pCreature->PauseOutOfCombatMovement();

    /*
     * A BOT NPC HAS ONLY THE TEMPLATE HALF, and that is not a gap: npc_vendor is
     * keyed by creature entry, so the "own list" half cannot exist for it. It
     * answers VENDOR_MENU_TEMPLATE, which is what VENDOR_MENU_ALL includes, so a
     * categorised shop (HouseVendor.cpp) filters a bot's stock the same way.
     */
    /*
     * A NAMED SHOP REPLACES BOTH HALVES. The option said which list to show, so
     * the NPC's own stock is not part of the answer -- showing it as well would
     * make "Reagents" a window with the blacksmith's hammers in it.
     */
    ...
    {
        vItems = nullptr;
        tItems = sObjectMgr.GetNpcVendorTemplateItemList(vendorTemplate);
    }
    else
    {
        vItems = pCreature && (menu_type & VENDOR_MENU_NORMAL)
            ? pCreature->GetVendorItems() : nullptr;
        tItems = menu_type & VENDOR_MENU_TEMPLATE
            ? (pCreature ? pCreature->GetVendorTemplateItems() : botItems) : nullptr;
    }
    ...
    // Not onto a category page -- a joke item filed under "Seating" reads as a
    // bug in the filter rather than as a joke.
    if (!onlyItems && sWorld.IsAprilFools() && urand(0, 10) == 0)
    {
        ++count;
    ...
    float discountMod = _player->GetReputationPriceDiscount(pVendor);
    ...
    for (int i = 0; i < numitems; ++i)
    {
        ...
        if (crItem)
        {
            // One page of a categorised vendor: everything outside the page is
            // simply not sent. The row is untouched, so buying still works --
            // the client answers with the item entry, not with a list index.
            if (onlyItems && onlyItems->find(crItem->item) == onlyItems->end())
                continue;
            ...
            if (ItemPrototype const *pProto = sObjectMgr.GetItemPrototype(crItem->item))
            {
                if (!_player->IsGameMaster())
                {
                    ...
                    if (!pProto->RequiredReputationFaction && pProto->RequiredReputationRank > 0 &&
                            ReputationRank(pProto->RequiredReputationRank) > _player->GetReputationRank(pVendor->GetFactionId()))
                        continue;

                    if (crItem->conditionId && !sObjectMgr.IsConditionSatisfied(crItem->conditionId, _player, pVendor->GetMap(), pVendor, CONDITION_FROM_VENDOR))
                        continue;
                }
                ...
                /*
                 * A COST ROW REPLACES THE PRICE OUTRIGHT, and is NOT discounted.
                 *
                 * The reputation discount is a rule about what an item is worth
                 * to this player at this faction's vendor. A price a shop set by
                 * hand is not a valuation, it is a decision, and quietly taking
                 * 10% off it would make the editor's number a lie.
                 *
                 * The ITEM half of a cost cannot be sent at all -- seven uint32
                 * per row and none of them is an item cost, see sql/custom/094.
                 * The window shows this money and the server refuses the rest.
                 */
                VendorCost const* cost = sObjectMgr.GetVendorCost(crItem->costId);
                uint32 price = cost ? cost->money
                                    : uint32(floor(pProto->BuyPrice * discountMod));

                data << uint32(count);
                data << uint32(crItem->item);
                data << uint32(pProto->DisplayInfoID);
                data << uint32(crItem->maxcount <= 0 ? 0xFFFFFFFF : pVendor->GetVendorItemCurrentCount(crItem));
                data << uint32(price);
                data << uint32(pProto->MaxDurability);
