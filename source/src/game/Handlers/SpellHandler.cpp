...
void WorldSession::HandleGameObjectUseOpcode(WorldPacket & recv_data)
{
    ...
    // Never expect this opcode for some type GO's -- EXCEPT housing furniture,
    // which is the one case where the client is right and this check is wrong.
    //
    // In edit mode the client is told the object is a GOOBER, per viewer, in
    // Object::BuildValuesUpdate. So it sends the click. But GetGoType() here
    // reads the SERVER's copy, which is still GENERIC and deliberately so --
    // mutating it would leave a GOOBER whose template is a chair, and
    // GameObject::Update's GO_JUST_DEACTIVATED would then read
    // GetGOInfo()->goober.spellId out of the wrong union member, down a path
    // that destroys the object.
    //
    // That split is the price of the per-viewer trick and this is where it
    // showed: a chair (type 7) passed this guard and opened its menu, while a
    // table (type 5) was dropped one line before Use() and did nothing at all,
    // with no error anywhere. Same edit session, same highlight, same click.
    //
    // Letting housing through costs nothing for anyone else: a client that was
    // never told GOOBER does not send this opcode, and HouseGameObjectUse
    // inside Use() answers false for anybody not editing that object -- so a
    // chair in a house you are visiting is still just a chair to sit on.
    if (obj->GetGoType() == GAMEOBJECT_TYPE_GENERIC && obj->GetGUIDLow() < HOUSE_GO_GUID_MIN)
        return;
