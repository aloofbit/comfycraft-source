...
void WorldSession::HandleGameObjectQueryOpcode(WorldPacket & recv_data)
{
    ...
    if (info)
    {
        // THE NAME THE CLIENT PUTS UNDER THE CURSOR, and in the title bar of a
        // gossip window. 9,577 of the 21,200 gameobject_template rows hold
        // their model PATH in the name column with backslashes turned into
        // spaces -- "World GENERIC DWARF PASSIVE DOODADS Beds DwarvenBed01.mdx"
        // where "Dwarven Bed 01" was meant -- so hovering half the catalogue
        // reads as garbage. HouseMgr::ObjectDisplayName cuts the leaf out and
        // spaces it; housing's menus and the addon's cards have used it all
        // along, and this is the same answer given to the client.
        //
        // UNCONDITIONAL, AND SCOPING IT TO HOUSING WOULD BE WORSE. The query
        // carries a guid as well as an entry, so answering differently for
        // furniture is possible -- but the client caches this by ENTRY, in
        // gameobjectcache.wdb. Scoped, entry 2413 would be "Wooden Chair" or a
        // model path depending on which one you happened to meet first, and
        // that answer would then persist on disk. One name per entry is the
        // only coherent choice.
        //
        // It is also a no-op on anything authored: ObjectDisplayName tests for
        // a path first and returns the raw name untouched when it is not one,
        // which is the other half of the table.
        std::string const display = HouseMgr::ObjectDisplayName(entryID);
        char const* name = display.c_str();

        // A real localisation still wins. These paths only ever appear in the
        // default name, so a locale row means somebody wrote a name on purpose.
        int loc_idx = GetSessionDbLocaleIndex();
        if (loc_idx >= 0)
        {
