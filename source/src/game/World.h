...
enum eConfigUInt32Values
{
    ...
    CONFIG_UINT32_FACTIONLESS_PEACE_STANDING,
    CONFIG_UINT32_INVASION_MIN_LEVEL,
    CONFIG_UINT32_INVASION_LEVEL_RANGE,
    CONFIG_UINT32_INVASION_ARRIVE_DISTANCE,
    CONFIG_UINT32_INVASION_WARN_SECONDS,
    CONFIG_UINT32_INVASION_DURATION_SECONDS,
    CONFIG_UINT32_INVASION_RETURN_SECONDS,
    CONFIG_UINT32_INVASION_DEATH_LINGER_SECONDS,
    CONFIG_UINT32_INVASION_REVIVE_SECONDS,
    CONFIG_UINT32_INVASION_RESPITE_SECONDS,
...
enum eConfigBoolValues
{
    ...
    CONFIG_BOOL_FACTIONLESS_ENABLE,
    CONFIG_BOOL_FACTIONLESS_IGNORE_QUEST_RACES,
    CONFIG_BOOL_FACTIONLESS_SANCTUARY_CAPITALS,
    CONFIG_BOOL_INVASION_ENABLE,
    CONFIG_BOOL_INSTANCE_IGNORE_LEVEL,
    CONFIG_BOOL_INSTANCE_IGNORE_RAID,
    CONFIG_BOOL_INSTANCE_PACK_IDS,
...
class World
{
    public:
        ...
        // Factionless: is this one of the reputations a race is born at war with that
        // we want lifted to peace? Deliberately a list rather than "anything flagged
        // AT_WAR by default" -- that would also catch Bloodsail, the Syndicate, the
        // Timbermaw and the battleground factions, whose hostility is real content.
        bool IsFactionlessPeaceFaction(uint32 factionId) const
        { return m_factionlessPeaceFactions.find(factionId) != m_factionlessPeaceFactions.end(); }

        // Factionless: is this area one where players cannot fight each other? Towns
        // have to be a list because the DBC has no flag for one -- all ten area flags
        // are snow, duel, arena, capital and unknowns, and none says "settlement".
        // Capitals are not in here; they come off AREA_FLAG_CAPITAL instead, since a
        // city sprawls across sub-areas that would each need listing.
        bool IsFactionlessSanctuaryArea(uint32 areaId) const
        { return m_factionlessSanctuaryAreas.find(areaId) != m_factionlessSanctuaryAreas.end(); }
    ...
    private:
        ...
        // Factionless: the reputations a race is born at war with that should be
        // lifted to peace. Read from Factionless.PeaceFactions; see World.cpp.
        std::set<uint32> m_factionlessPeaceFactions;

        // Factionless: areas where players are sanctuary to each other. Read from
        // Factionless.SanctuaryAreas; see World.cpp.
        std::set<uint32> m_factionlessSanctuaryAreas;

        // Parse a comma-separated id list out of the config into ids. Shared by the
        // two Factionless lists; `key` is used to name the offending token in the log.
        static void LoadIdList(char const* key, char const* defaultValue, std::set<uint32>& ids);
