...
/*
 * WHY THIS EXISTS. ScriptMgr::LoadScripts opens with `if (IsScriptScheduled())
 * return;` and means it: a scheduled ScriptAction holds a `ScriptInfo const*`
 * INTO the map that LoadScripts is about to clear, so reloading underneath one
 * is a use-after-free rather than merely untidy.
 *
 * The trouble is that the gate is a count, and on this server it does not fall
 * back to zero -- measured 2026-09-14, `reload gossip_scripts` was a silent
 * no-op four times running, including on a server twenty seconds old. So the
 * table could not be reloaded at all, and a gossip script change meant a full
 * restart.
 *
 * Draining first turns the count into something true rather than something
 * hoped for: with every schedule empty, no pointer is outstanding, and the
 * reload is safe BY CONSTRUCTION instead of by inspection. Pending delayed
 * actions are lost, which is the right trade for a command an administrator
 * typed.
 *
 * The iteration deliberately copies InitializeVisibilityDistanceInfo above and
 * takes no MapManager lock. The narrow race that leaves -- a map scheduling a
 * script between the drain and the load -- is exactly the race the bare count
 * already had, and this runs on the world thread from a console command.
 */
uint32 MapManager::ClearAllScheduledScripts()
{
    uint32 dropped = 0;
    for (const auto& itr : i_maps)
        dropped += itr.second->ClearScheduledScripts();
    return dropped;
}
