...
Map::~Map()
{
    ...
    // Before anything else it could still be holding a guid for. Closing the
    // state drops every parked coroutine with it; a scene half way through when
    // its map goes away simply stops, which is the only sane outcome.
    delete m_luaScene;
    m_luaScene = nullptr;

    delete m_botNpcIdle;
    m_botNpcIdle = nullptr;
...
void Map::Update(uint32 t_diff)
{
    ...
    // Resume any Lua scene whose wait() is up. Costs a null check on the
    // overwhelming majority of maps, which never make a state at all.
    if (m_luaScene)
        m_luaScene->Update();

    // What bot NPCs do when nobody has told them to do anything.
    UpdateBotNpcIdle(t_diff);

    // Give back the flags of anything whose busy time is up. Empty on nearly
    // every map, so this is a size check and nothing else.
    if (!m_busy.empty())
    {
        time_t const now = sWorld.GetGameTime();
        for (auto it = m_busy.begin(); it != m_busy.end(); )
        {
            ...
            // Gone is fine and common. A creature that despawned and came back
            // is holding whatever its template says, which is where it started.
            if (Unit* u = GetUnit(it->first))
                u->SetUInt32Value(UNIT_NPC_FLAGS, it->second.flags);
            it = m_busy.erase(it);
        }
    }
...
bool DungeonMap::Add(Player *player)
{
    ...
    // The counterpart to the ClearEdit / ClearSelection / ClearMark trio in
    // Remove below, and self-guarding on the map id the same way -- so this
    // file carries one unconditional line and none of housing's rules. Every
    // way into a house passes through here, including a relog inside one,
    // which is the arrival no teleport hook would have seen.
    sHouseMgr.OnPlayerArrive(player);
...
{
    // A guest, put here by an instance override rather than by a bind of their
    // own. Their binds are none of this map's business, and touching them would
    // trip the assert below: a permanently bound player is standing in an
    // instance that is not the one they are bound to, which is exactly the state
    // this function exists to declare impossible.
    if (player->GetForcedInstanceId(GetId()))
        return;
...
void DungeonMap::Remove(Player *player, bool remove)
{
    ...
    // The visit ends when you leave, or the override would keep dragging the
    // player back into somebody else's instance on every later entry here.
    //
    // Not when the destination is this same map, though. A far teleport removes
    // the player from the old map BEFORE the destination instance is resolved,
    // so going from one house straight to another -- or from your own house to a
    // friend's -- would clear the override that was just set for where you are
    // going, and silently land you back where you started.
    //
    // THE FAR SEMAPHORE CANNOT BE PART OF THIS TEST. ExecuteTeleportFar calls
    // this Remove and only sets SetSemaphoreTeleportFar(true) a few lines
    // LATER, so IsBeingTeleportedFar() is always false right here and a guard
    // built on it cleared the override on every same-map hop -- .house
    // template goto from inside a house landed you in your own bind. The
    // recorded teleport destination is written at TeleportTo time and is the
    // one truthful signal at this moment. A STALE destination equal to this
    // map only survives a non-teleport removal (logout inside the house),
    // where the kept override dies with the player object anyway.
    if (player->GetForcedInstanceId(GetId()) &&
        player->GetTeleportDest().mapId != GetId())
        player->ClearForcedInstance();

    // Handles are temporary summons in THIS map, so they have to be despawned
    // while the player is still standing in it. Leaving them to the 30 minute
    // timer would work, but it would also leave a room full of wisps behind for
    // anybody who walked in after you.
    sHouseMgr.ClearEdit(player);

    // The selection goes with them, glow and all. The glow is a summon in this
    // map and is NOT part of the edit session -- it stands whether or not the
    // gears are out, so ClearEdit deliberately does not touch it and it has to
    // come down here.
    //
    // CLEARING THE SELECTION ITSELF, not just despawning the marker, and the
    // difference is a real one. GetSelectedObject validates on read and answers
    // 0 anywhere outside the house, so walking out was already harmless -- but
    // walk back IN and the object is in that house again, so the selection came
    // back to life with no glow on it. An invisible live selection is the exact
    // state the marker exists to prevent, and it was reachable by a round trip
    // through the door.
    //
    // Safe to call here only since ClearSelection stopped flashing the object
    // on the way out: a blink is Remove(go, false) + Add on the map the player
    // is in the middle of leaving, which is churn for every other viewer and
    // says nothing to the one person it was meant for.
    sHouseMgr.ClearSelection(player);

    // The chalk mark goes the same way, and for the same reason rather than a
    // merely similar one: GetMark validates against the house you are standing
    // in, so walking out already answered "no mark" -- but the ROW stayed, and
    // walking back IN would revive it with no glow on it, the marker having
    // been despawned on the way past. Same round trip that made an invisible
    // live selection reachable.
...
uint32 Map::ClearScheduledScripts()
{
    std::unique_lock<MapMutexType> lock(m_scriptSchedule_lock);

    uint32 const n = uint32(m_scriptSchedule.size());
    if (!n)
        return 0;

    // Decrease by exactly what is being dropped, the way ~Map does. The count is
    // global across every map, so a blanket reset here would wipe out the other
    // maps' outstanding work as far as the gate is concerned.
    m_scriptSchedule.clear();
    sScriptMgr.DecreaseScheduledScriptCount(n);
    return n;
}
...
/*
 * BUSY: unclickable now, exactly as it was in `seconds`.
 *
 * THE WHOLE UNIT_NPC_FLAGS WORD, not the gossip bit. Every bit in that field is
 * another way in, and clearing only gossip leaves a vendor or a questgiver
 * cursor lit over an NPC that is supposed to be in the middle of something. We
 * learned that on Orcy, who is gossip plus vendor.
 *
 * ALREADY ZERO MEANS SOMEBODY ELSE IS HOLDING IT -- most likely a Lua scene
 * that called HideGossip, which does its own save and restore. Recording a zero
 * here would mean whichever of us ran second handed back nothing at all, so the
 * second one does nothing and the first one's restore stands.
 *
 * An overlapping call EXTENDS the deadline rather than re-saving, which is what
 * makes two scenes on one NPC behave instead of the shorter one ending the
 * longer one's quiet.
 */
void Map::SetBusy(ObjectGuid guid, uint32 seconds)
{
    Unit* u = GetUnit(guid);
    if (!u)
        return;

    time_t const until = time_t(sWorld.GetGameTime() + seconds);

    auto it = m_busy.find(guid);
    if (it != m_busy.end())
    {
        if (until > it->second.until)
            it->second.until = until;
        return;
    }
    ...
    m_busy[guid] = BusyUnit{ was, until };
    u->SetUInt32Value(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_NONE);
}

LuaScene* Map::GetLuaScene()
{
    // Made on first use. Most maps in this world never run a scripted
    // conversation and should not carry a Lua state for the privilege.
    if (!m_luaScene)
        m_luaScene = new LuaScene(this);
    return m_luaScene;
}

/*
 * TWO BOOLS BEFORE ANYTHING IS ALLOCATED, and on nearly every map both are the
 * cheap answer.
 *
 * `AnyIdleRules` is false on a server where nobody has written one, which is
 * every server until somebody does. `HavePlayers` stands in for "is there a
 * bot NPC here": a bot NPC IS a player, so a map with nobody on it cannot have
 * one, and the map list is mostly empty instances.
 *
 * Made on first use past those, like the Lua state next door, so a map that
 * never sees an NPC with something to do never carries a driver for it.
 */
void Map::UpdateBotNpcIdle(uint32 diff)
{
    if (!sBotNpcMgr.AnyIdleRules() || !HavePlayers())
        return;

    if (!m_botNpcIdle)
        m_botNpcIdle = new BotNpcIdle();

    m_botNpcIdle->Update(this, diff);
}
