...
{
    /*
     * A UNIT, NOT A CREATURE, SINCE 2026-09-15. This was ToCreature and nothing
     * below ever needed one: IsAlive, HasUnitState, GetMotionMaster and
     * MonsterMoveWithSpeed are all declared on Unit, and GetRandomPoint,
     * GetDistance and GetAngle on WorldObject. The narrower cast was upstream
     * being careful rather than a requirement, and it cost us the one thing a
     * gossip script most wants to move: a BOT NPC, which is a Player.
     *
     * Relaxing it can only turn an error into a move. Every row that worked
     * before is on a Creature and takes exactly the same path it always did.
     */
    Unit* pSource = ToUnit(source);

    if (!pSource)
    {
        sLog.outError("SCRIPT_COMMAND_MOVE_TO (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_RecruitBot(const ScriptInfo& script, WorldObject* source, WorldObject* target)
{
    Player* pPlayer;

    if (!((pPlayer = ToPlayer(target)) || (pPlayer = ToPlayer(source))))
    {
        sLog.outError("SCRIPT_COMMAND_RECRUIT_BOT (script id %u) call for a nullptr or non-player source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }

    // Lives in the playerbots module; src/game/PlayerbotStubs.cpp supplies a
    // do-nothing version for BUILD_PLAYERBOTS=OFF builds. Declared here at the
    // call site because no core file includes a playerbot header - the same
    // seam BotActionLog_* uses from Unit.cpp.
    extern bool Playerbot_RecruitBotForPlayer(Player* player, uint32 role, uint32 classId);

    Playerbot_RecruitBotForPlayer(pPlayer, script.recruitBot.role, script.recruitBot.classId);

    // Finding nobody free is a normal outcome, not a script failure - the
    // player was already told. Never abort the rest of the gossip script.
    return false;
}

/*
 * SCRIPT_COMMAND_PLAY_SPELL_VISUAL (94)
 *
 * The hearts a critter gets when you pat it, on a gossip button. One packet,
 * SMSG_PLAY_SPELL_VISUAL, carrying a guid and a SpellVisualKit index; the
 * client draws the kit on that unit and everybody in range sees it.
 *
 * A KIT ID, NOT A SpellVisual ID -- the chain runs SpellVisualEffectName ->
 * SpellVisualKit -> SpellVisual and this packet stops at the MIDDLE table.
 * Passing a SpellVisual id draws nothing whatsoever, with no error and no log
 * line, which is the failure mode to expect from a wrong number here.
 * HouseMgr.h has the long note; HOUSE_CRITTER_HEART_VISUAL is 6512.
 *
 * NOTHING IS VALIDATED, because nothing can be: the core loads SpellVisual.dbc
 * and not SpellVisualKit.dbc, so there is no store to look an id up in. The
 * editor picks from a list of kits somebody has actually looked at, which is
 * the only check that means anything for a visual anyway.
 *
 * ToUnit, NEVER ToCreature. A bot NPC is a Player, so the source of a gossip
 * script is routinely not a Creature -- that is the gap that made action
 * scripts unreachable from a bot NPC until 2026-09-14, and reaching for
 * Creature members here would reintroduce half of it.
 */
bool Map::ScriptCommand_PlaySpellVisual(const ScriptInfo& script, WorldObject* source, WorldObject* target)
{
    // Target-first, the convention every command in this file that wants the
    // player follows. The gossip arm passes (source = the NPC, target = the
    // viewer), so the other order would play the effect on the NPC whenever the
    // author asked for the player -- and it would not error, it would work on
    // the wrong person.
    Unit* pUnit = script.playSpellVisual.on == SO_PLAY_VISUAL_ON_SOURCE
        ? (ToUnit(source) ? ToUnit(source) : ToUnit(target))
        : (ToUnit(target) ? ToUnit(target) : ToUnit(source));

    if (!pUnit)
    {
        sLog.outError("SCRIPT_COMMAND_PLAY_SPELL_VISUAL (script id %u) call for a nullptr or non-unit source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }

    pUnit->SendPlaySpellVisual(script.playSpellVisual.kitId);
    return false;
}

/*
 * SCRIPT_COMMAND_EMOTE_TARGET (95)
 *
 * The mirror of SCRIPT_COMMAND_EMOTE (1), which emotes its source and has no
 * field left over to say otherwise. This one emotes the TARGET, which for a
 * gossip script is the player who clicked.
 *
 * It does NOT copy 1's targeted-emote branch. That one demands a Creature
 * source so it can pause its movement and turn it to face somebody, and the
 * thing being turned here is a player -- who is already facing the NPC,
 * having just clicked it, and whose camera is not ours to move.
 */
bool Map::ScriptCommand_EmoteTarget(const ScriptInfo& script, WorldObject* source, WorldObject* target)
{
    // Target first, then source. Same convention and same reason as the
    // command above: asking for the player and silently getting the NPC is
    // worse than an error, because it works.
    Unit* pUnit = ToUnit(target) ? ToUnit(target) : ToUnit(source);

    if (!pUnit)
    {
        sLog.outError("SCRIPT_COMMAND_EMOTE_TARGET (script id %u) call for a nullptr or non-unit source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }

    uint32 emoteCount = 1;
    for (; emoteCount < MAX_EMOTE_ID && script.emoteTarget.emoteId[emoteCount]; ++emoteCount);

    pUnit->HandleEmote(script.emoteTarget.emoteId[urand(0, emoteCount - 1)]);
    return false;
}

/*
 * SCRIPT_COMMAND_RUN_LUA (96)
 *
 * Hands the two units this script was started with to a sandboxed Lua scene.
 * `datalong` is a dialog_lua row; the source itself lives there rather than in
 * gossip_scripts, whose widest text column is a 255 byte comment.
 *
 * NOTHING IS RESOLVED HERE BUT THE GUIDS. A scene can wait, and everything it
 * touches after the first wait() has to be looked up again anyway, so passing
 * pointers in would only invite somebody to keep one. LuaScene.h has the long
 * version.
 */
bool Map::ScriptCommand_RunLua(const ScriptInfo& script, WorldObject* source, WorldObject* target)
{
    Unit* pNpc = ToUnit(source);
    Unit* pPlayer = ToUnit(target);

    if (!pNpc && !pPlayer)
    {
        sLog.outError("SCRIPT_COMMAND_RUN_LUA (script id %u) call for a nullptr or non-unit source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }

    auto data = LuaScene::Data();
    auto it = data->scripts.find(script.runLua.sceneId);
    if (it == data->scripts.end())
    {
        sLog.outError("SCRIPT_COMMAND_RUN_LUA (script id %u) wants dialog_lua %u, which is not loaded.", script.id, script.runLua.sceneId);
        return ShouldAbortScript(script);
    }

    char name[32];
    snprintf(name, sizeof(name), "scene %u", script.runLua.sceneId);

    if (LuaScene* scene = GetLuaScene())
    {
        scene->Run(script.runLua.sceneId, it->second, name,
                   pNpc ? pNpc->GetObjectGuid() : ObjectGuid(),
                   pPlayer ? pPlayer->GetObjectGuid() : ObjectGuid());
    }

    // A scene that fails has already said so, in the log and to the player.
    // Never abort the rest of the gossip script for it.
    return false;
}

/*
 * SCRIPT_COMMAND_SET_BUSY (97)
 *
 * Makes the NPC unclickable for a while, which is the only thing that stops a
 * scene being started twice: there is no lock on a gossip script and no
 * "already running" check anywhere, so two players can press one button at the
 * same moment and the NPC plays the whole thing twice, overlapping itself.
 *
 * A DURATION, NOT A PAIR. Estelle Gendry's shipped script does this with two
 * MODIFY_FLAGS rows, one at each end, and that works because her script is
 * fixed and tested. Written by hand in an editor it is a trap: the second row
 * is one delete away from leaving an NPC permanently unclickable, and it never
 * runs at all if the script aborts between them. A deadline cannot be orphaned.
 *
 * MODIFY_FLAGS could not do this anyway. It adds and removes KNOWN bits, and
 * "put back whatever it had" needs somewhere to remember the word. Map::SetBusy
 * is that somewhere.
 */
bool Map::ScriptCommand_SetBusy(const ScriptInfo& script, WorldObject* source, WorldObject* target)
{
    // Source first here, unlike most of this file: the thing being made
    // unclickable is the NPC, and the NPC is the source. Falling back to the
    // target would make the PLAYER unclickable, which is not a thing worth
    // having and would be baffling to debug.
    Unit* pUnit = ToUnit(source);

    if (!pUnit)
    {
        sLog.outError("SCRIPT_COMMAND_SET_BUSY (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }

    SetBusy(pUnit->GetObjectGuid(), script.setBusy.seconds);
    return false;
}

/*
 * SCRIPT_COMMAND_WALK_FROM_POST (98)
 *
 * OURS, ADDED 2026-09-16. Walks the source to a point so many yards ahead of
 * and to the right of its POST, worked out at the moment the row runs.
 *
 * WHY NOT MOVE_TO'S RELATIVE MODE. SO_MOVETO_COORDINATES_RELATIVE_TO_TARGET
 * adds the offset to the TARGET's position -- the player who set a script off
 * -- ignores every facing, and in an idle loop has no target at all and logs.
 * "Three yards in front of her" cannot be said with it.
 *
 * WHY THE POST AND NOT WHERE IT STANDS. An offset from its current position
 * drifts a little further on every pass of a loop. From the post it is the
 * same place every time, and `.npcbot move` carries the whole routine along.
 * BotNpc_PointFromPost has the rest, including why the height is searched from
 * a few yards above.
 *
 * ENDS FACING THE WAY ITS POST FACES, so an NPC sent home at 0, 0 is actually
 * home rather than standing on the spot looking wherever it last walked.
 */
bool Map::ScriptCommand_WalkFromPost(const ScriptInfo& script, WorldObject* source, WorldObject* target)
{
    Unit* pSource = ToUnit(source);

    if (!pSource)
    {
        sLog.outError("SCRIPT_COMMAND_WALK_FROM_POST (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }

    if (!pSource->IsAlive() || pSource->HasUnitState(UNIT_STAT_NOT_MOVE))
        return ShouldAbortScript(script);

    float x, y, z, o;
    if (!BotNpc_PointFromPost(pSource, script.x, script.y, x, y, z, o))
    {
        sLog.outError("SCRIPT_COMMAND_WALK_FROM_POST (script id %u): %s has no post on map %u to walk from, skipping.", script.id, pSource->GetName(), pSource->GetMapId());
        return ShouldAbortScript(script);
    }

    pSource->MonsterMoveWithSpeed(x, y, z, o, 0.0f, script.walkFromPost.movementOptions);
    return false;
}
