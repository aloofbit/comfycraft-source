...
class Map : public GridRefManager<NGridType>
{
    ...
    public:
        ...
        // Drops every script action still waiting on this map and corrects the
        // global count by exactly what it dropped. Returns how many. Only the
        // script-table reloads call it -- see ChatHandler::HandleReloadGossipScriptsCommand.
        uint32 ClearScheduledScripts();
        ...
        // Creates it if this map has not needed one before.
        LuaScene* GetLuaScene();

        /*
         * The bot NPC idle driver, or nullptr on a map that has never needed
         * one. NOT a "get or create": the only caller outside Map::Update is
         * the gossip handler telling it somebody is talking, and a conversation
         * with an NPC that does nothing unprompted is nothing to record.
         */
        BotNpcIdle* FindBotNpcIdle() { return m_botNpcIdle; }

        // Unclickable for this many seconds, then exactly as it was.
        void SetBusy(ObjectGuid guid, uint32 seconds);
    ...
    private:
        ...
        /*
         * The map's Lua, made on first use and never for a map that has no
         * scripted conversation on it -- which is nearly all of them. See
         * LuaScene.h on why it is per map rather than one global state.
         */
        LuaScene* m_luaScene = nullptr;

        /*
         * WHAT THE BOT NPCs ON THIS MAP DO UNPROMPTED, and what each of them is
         * in the middle of. Made on first use exactly like the Lua state above,
         * and for the same reason: most maps have no bot NPC on them at all.
         * BotNpcIdle.h has the design.
         */
        BotNpcIdle* m_botNpcIdle = nullptr;
        void UpdateBotNpcIdle(uint32 diff);

        /*
         * NPCs THAT ARE BUSY, and the flags word each one is owed back.
         *
         * SCRIPT_COMMAND_SET_BUSY takes a DURATION rather than pairing with a
         * "free again" command, and that is the whole design. A pair can be
         * orphaned -- the second row never runs because the first aborted the
         * script, or the author deleted it, or the NPC despawned between them --
         * and an orphaned restore leaves an NPC permanently unclickable, which
         * needs a GM to unstick. A deadline cannot be orphaned by anything.
         *
         * Keyed by guid rather than by pointer for the same reason the Lua
         * handles are: the unit can die, log out or change map before the
         * deadline, and a stale pointer would be a crash where a failed lookup
         * is a shrug.
         */
        struct BusyUnit
        {
        ...
        std::map<ObjectGuid, BusyUnit> m_busy;
        ...
        bool ScriptCommand_RecruitBot(const ScriptInfo& script, WorldObject* source, WorldObject* target);
        bool ScriptCommand_PlaySpellVisual(const ScriptInfo& script, WorldObject* source, WorldObject* target);
        bool ScriptCommand_EmoteTarget(const ScriptInfo& script, WorldObject* source, WorldObject* target);
        bool ScriptCommand_RunLua(const ScriptInfo& script, WorldObject* source, WorldObject* target);
        bool ScriptCommand_SetBusy(const ScriptInfo& script, WorldObject* source, WorldObject* target);
        bool ScriptCommand_WalkFromPost(const ScriptInfo& script, WorldObject* source, WorldObject* target);
        ...
        const ScriptCommandFunction m_ScriptCommands[SCRIPT_COMMAND_MAX] =
        {
            ...
            &Map::ScriptCommand_RecruitBot,             // 93
            &Map::ScriptCommand_PlaySpellVisual,        // 94
            &Map::ScriptCommand_EmoteTarget,            // 95
            &Map::ScriptCommand_RunLua,                 // 96
            &Map::ScriptCommand_SetBusy,                // 97
            &Map::ScriptCommand_WalkFromPost,           // 98
            &Map::ScriptCommand_TakeMoney,              // 99
        };
