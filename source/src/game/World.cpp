...
// Read a comma-separated id list out of the config. Follows vmap.ignoreSpellIds,
// which is the precedent in this core for a config that holds a list of ids: a bad
// token is named in the log and skipped rather than taking the rest of the list
// down with it.
void World::LoadIdList(char const* key, char const* defaultValue, std::set<uint32>& ids)
{
    ids.clear();

    std::string token;
    std::istringstream stream(sConfig.GetStringDefault(key, defaultValue));
    while (std::getline(stream, token, ','))
    {
        ...
        try
        {
            ids.insert(uint32(std::stoul(token)));
        }
        catch (std::exception const&)
        {
            sLog.outError("%s: '%s' is not an id, ignoring it.", key, token.c_str());
        }
    }
}
...
void World::LoadConfigSettingsFromFile(bool reload)
{
    ...
    setConfig(CONFIG_BOOL_FACTIONLESS_ENABLE,             "Factionless.Enable", false);
    setConfig(CONFIG_BOOL_FACTIONLESS_IGNORE_QUEST_RACES, "Factionless.IgnoreQuestRaces", false);
    setConfig(CONFIG_UINT32_FACTIONLESS_PEACE_STANDING,   "Factionless.PeaceStanding", 0);

    // Factionless: which reputations to lift out of the war a race is born into.
    // A list rather than a rule, because "starts at war" also describes Bloodsail,
    // the Syndicate, the Timbermaw and every battleground faction, and none of
    // those should be pacified. Default is the eight capitals, the two team
    // umbrellas, and the world factions that gate a whole zone behind them.
    LoadIdList("Factionless.PeaceFactions",
        "47,54,61,67,68,69,72,76,81,108,269,469,471,509,510,530,729,730,889,890,893,1001,1011",
        m_factionlessPeaceFactions);

    // Factionless: a city at peace should not be somewhere two flagged players can
    // brawl, and guards cannot break it up now that nobody is hostile to them. So the
    // fight is prevented instead: PLAYER_FLAGS_SANCTUARY makes two players friendly
    // to each other in GetReactionTo, above the flag rule and above FFA. The check
    // was already in the core and had never fired -- vanilla shipped no sanctuary
    // zones, they arrived with Shattrath.
    //
    // Capitals come off AREA_FLAG_CAPITAL. Towns cannot: the DBC has ten area flags
    // and none of them means "settlement", so they are a list. The default is every
    // area that a tavern trigger or a known town name resolves to in area_template,
    // Turtle's own towns included. See docs/features/factionless.md piece 8.
    setConfig(CONFIG_BOOL_FACTIONLESS_SANCTUARY_CAPITALS, "Factionless.SanctuaryCapitals", true);

    LoadIdList("Factionless.SanctuaryAreas",
        "35,42,69,75,87,108,117,131,144,150,159,186,222,251,271,272,280,320,321,340,348,349,"
        "362,367,378,380,392,415,431,442,460,467,484,513,541,608,702,976,977,1099,1116,1237,"
        "1438,1446,1997,1998,2255,2268,2361,2408,2418,2479,2897,3137,3197,3317,3425,4010,4018,"
        "4021,5028,5040,5051,5065,5093,5118,5120,5123,5214,5533,5583,5598,5603,5657,5659",
        m_factionlessSanctuaryAreas);

    // Invasions. The Invader's Sigil finds a flagged player near your level and
    // teleports YOU to them; nobody else is ever moved. It leans on factionless
    // for one thing -- the PvP flag rather than the race deciding hostility --
    // without which it could only ever pair Alliance against Horde.
    // See docs/features/invasions.md.
    setConfig(CONFIG_BOOL_INVASION_ENABLE,                "Invasion.Enable", true);
    setConfig(CONFIG_UINT32_INVASION_MIN_LEVEL,           "Invasion.MinLevel", 10);

    // Plus or minus, so a 5 means a 10-level spread at the widest. The
    // battleground brackets are 10 wide and reusing them would have been free,
    // but 20-against-29 is not a fight when it is one on one.
    setConfig(CONFIG_UINT32_INVASION_LEVEL_RANGE,         "Invasion.LevelRange", 5);
    setConfig(CONFIG_UINT32_INVASION_ARRIVE_DISTANCE,     "Invasion.ArriveDistance", 10);

    // The warning window. The defender is told something is coming and gets
    // this long to decide where to be standing when it does.
    setConfig(CONFIG_UINT32_INVASION_WARN_SECONDS,        "Invasion.WarnSeconds", 5);
    setConfig(CONFIG_UINT32_INVASION_DURATION_SECONDS,    "Invasion.DurationSeconds", 300);
    setConfig(CONFIG_UINT32_INVASION_RETURN_SECONDS,      "Invasion.ReturnSeconds", 5);

    // A fallen invader is not snatched back to life. They lie dead where they
    // fell for the linger, are carried home still dead, and wake a moment after
    // arriving -- so arriving and standing up are two things you see rather
    // than one thing you miss.
    setConfig(CONFIG_UINT32_INVASION_DEATH_LINGER_SECONDS,"Invasion.DeathLingerSeconds", 10);
    setConfig(CONFIG_UINT32_INVASION_REVIVE_SECONDS,      "Invasion.ReviveSeconds", 3);
    // THE RESPITE IS THE DEFENDER'S, NOT THE INVADER'S. Nothing stops somebody
    // hunting again the moment they are home; what is limited is how often the
    // same person can be hunted. See InvasionMgr's m_respite for why the sides
    // are not symmetrical.
    setConfig(CONFIG_UINT32_INVASION_RESPITE_SECONDS,     "Invasion.RespiteSeconds", 300);
    ...
    setConfig(CONFIG_BOOL_INSTANCE_PACK_IDS,     "Instance.PackIdsOnStartup", true);
...
///- Initialize the random number generator
    ...
    sSpellMgr.LoadSpellScriptNames();                       // must be after LoadScriptNames()
	...
	// Off here. Packing renumbers every instance id on every startup, which is
	// fine for the core -- it rewrites character_instance in the same pass -- but
	// it breaks anything holding an instance id outside those tables, and player
	// housing does. Compact ids buy nothing; GenerateInstanceId is a 32 bit
	// counter. Defaults to 1, so upstream behaviour is unchanged elsewhere.
	if (getConfig(CONFIG_BOOL_INSTANCE_PACK_IDS))
    ...
    sLog.outString("Loading dialog scenes and spots...");
    LuaScene::LoadAll();                                    // dialog_lua + dialog_spot
    sObjectMgr.LoadGossipMenus();
    sLog.outString("Loading vendor templates...");
    sLog.outString("Loading Vendor Costs...");
    sObjectMgr.LoadVendorCosts();                           // must be before both vendor tables
    ...
    sLog.outString("Loading rented mounts...");
    sObjectMgr.LoadRentedMounts();                          // must be after load CreatureTemplate, NPCText and Spell.dbc
    ...
    sMapMgr.Initialize();
    sLog.outString("Loading player houses...");
    sHouseMgr.LoadFromDB();                                 // after SetHighestGuids (guid reserve) and sMapMgr.Initialize (instance ids)
    ...
    sLog.outString("Loading bot NPCs...");
    sBotNpcMgr.Load();
