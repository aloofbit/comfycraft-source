...
WorldSession::WorldSession(uint32 id, WorldSocket *sock, AccountTypes sec, time_t mute_time, LocaleConstant locale, const std::string& remote_ip, uint32 binaryIp) :
    ...
    m_openVendorTemplate(0),
    m_openMerchantEntry(0)
{
    if (sock)
    {
        m_Address = remote_ip;
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    if (_player)
    {
        ...
        // An invader logging out is put back where they set out from, so the
        // position that reaches the database is the origin rather than whatever
        // corner of the world they were hunting in. It belongs HERE and not up
        // beside sLFTMgr: TeleportTo on its own is deferred, and
        // ExecuteSingleDelayedTeleport is what makes it land ahead of SaveToDB
        // -- which is the same reason the dungeon check below is where it is.
        // The two never both fire; an invasion cannot start or survive inside an
        // instance.
        if (sInvasionMgr.OnPlayerLogout(_player))
        {
            removedFromMap = true;
            sMapMgr.ExecuteSingleDelayedTeleport(_player);
        }
