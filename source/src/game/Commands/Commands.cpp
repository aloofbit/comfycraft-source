...
bool ChatHandler::HandleLevelUpCommand(char* args)
{
    ...
    else
    {
        ...
        // Same fill-in as .setlevel: the level alone leaves a husk, and a delta is no
        // reason to get less of a character than an absolute would.
        if (target)
        {
            target->LearnClassSpellsForLevel();
            target->MaxLevelDependentSkills();
        }
        ...
        if (!m_session || m_session->GetPlayer() != target)     // including chr==nullptr
        {
            ...
            PSendSysMessage("Level %i. Spellbook and weapon skills to match.", newlevel);
    }
...
//
// What .levelup should have been: an absolute level rather than a delta, and a character
// that is actually that level afterwards. GiveLevel now walks the weapon skills, the
// spellbook and the talents down when the level drops, but it can only hand back what an
// earlier drop took away - so this fills in the rest, which is what makes ".setlevel 40"
// on a level 12 character produce a working level 40 instead of a level 40 husk.
bool ChatHandler::HandleSetLevelCommand(char* args)
{
    int32 newLevel;
    ...
    {
        // Never a creature. With a mob targeted this still means you, which is the trap
        // .levelup sets every time it is run without deselecting first.
        ...
        target_guid = target->GetObjectGuid();
    ...
    if (newLevel > int32(PLAYER_STRONG_MAX_LEVEL))
        newLevel = PLAYER_STRONG_MAX_LEVEL;

    int32 oldLevel = target ? int32(target->GetLevel()) : int32(Player::GetLevelFromDB(target_guid));

    HandleCharacterLevel(target, target_guid, oldLevel, newLevel);

    if (!target)
    {
        PSendSysMessage("%s is offline: level set to %i, spells and skills untouched.", target_name.c_str(), newLevel);
        return true;
    }
    ...
    if (m_session && m_session->GetPlayer() == target)
        ...
        PSendSysMessage("%s is level %i. Spellbook and weapon skills to match.", GetNameLink(target).c_str(), newLevel);
...
/*
 * .reload bot_npc -- re-read tw_char.bot_npc and re-apply it to everyone in world.
 *
 * Only the BINDING lives in that table -- which character is an NPC, with which
 * flags and which menu. What a bot NPC SAYS is ordinary `gossip_menu` and
 * `gossip_menu_option` data, so iterating on dialog wants `.reload gossip_menu`
 * and never this. This is for adding an NPC or pointing one at a different menu.
 *
 * SEC_ADMINISTRATOR like every other reload, which on this core means it is out
 * of reach of an in-game GM (rank 3 is the highest that works in game). Run it
 * from the mangosd console, which is SEC_CONSOLE, or through .mangos-command --
 * the same rule CLAUDE.md states for the whole reload family.
 */
bool ChatHandler::HandleReloadBotNpcCommand(char* /*args*/)
{
    sLog.outString("Re-Loading bot NPCs...");
    sBotNpcMgr.Reload();
    SendSysMessage(">> Table `bot_npc` reloaded.");
    return true;
}

/*
 * .modify npcflag [value] -- UNIT_NPC_FLAGS on the SELECTED UNIT, player or creature.
 *
 * The probe behind "can a playerbot be an NPC". .modify faction next door can
 * already write this field, but it is GetSelectedCreature() only, so there was
 * no way to put a flag on a player and find out what the client does with it.
 *
 * Deliberately a UNIT command: the interesting target is a Player. With no
 * argument it prints the current value, which is how you check the field
 * actually took -- setting it and seeing nothing in game is ambiguous between
 * "the server did not store it" and "the client ignored it", and those want
 * very different next moves.
 *
 * 1 is UNIT_NPC_FLAG_GOSSIP. 0 puts a bot back to being a bot.
 */
bool ChatHandler::HandleModifyNpcFlagCommand(char* args)
{
    Unit* target = GetSelectedUnit();
    if (!target)
    {
    ...
    char const* kind = target->GetTypeId() == TYPEID_PLAYER ? "player" : "creature";

    if (!*args)
    {
        PSendSysMessage("npcflag of %s %s is %u.", kind, target->GetName(),
                        target->GetUInt32Value(UNIT_NPC_FLAGS));
        return true;
    }

    uint32 npcflag;
    if (!ExtractUInt32(&args, npcflag))
        return false;

    target->SetUInt32Value(UNIT_NPC_FLAGS, npcflag);

    PSendSysMessage("npcflag of %s %s set to %u.", kind, target->GetName(), npcflag);

    // The field is now dirty and goes out in the next values update -- but
    // whether the CLIENT re-reads UNIT_NPC_FLAGS on an update, rather than only
    // from a creation block, is exactly one of the things this probe does not
    // know. If nothing changes in game, walk out of visibility range and back
    // before concluding anything: that forces a fresh creation block, which the
    // client certainly does read.
    sLog.outString("[NPCFLAG PROBE] %s %s (%s) npcflag -> %u",
                   kind, target->GetName(), target->GetObjectGuid().GetString().c_str(), npcflag);
...
/*
 * .gobject nudge <guid> <dx> <dy> <dz>
 *
 * A PROBE, NOT A FEATURE. It asks one question: does the 1.12 client redraw a
 * gameobject when GAMEOBJECT_POS_X/Y/Z change in an ordinary values update?
 *
 * Everything else in this core that moves a gameobject takes it out of the map
 * and puts it back -- `.gobject move` above, HouseMgr::ApplyPosition for
 * furniture, and Transport::Update at a map-change frame, which does
 * SendOutOfRangeUpdateToMap + SendCreateUpdateToMap. That is destroy and
 * recreate: the model pops out and back in, and any client-side transport
 * attachment goes with it. Nobody here has checked whether the cheap path works,
 * and the answer decides whether a server-steered airship is possible at all.
 * If the client honours a values update, the server can fly a WMO along any
 * route it likes and no client patch is needed for the route.
 *
 * So this deliberately does the LEAST possible: set the three fields, keep the
 * server's own position in step, and let the normal update mask carry the change
 * out with the next tick. No Remove, no Add, no Refresh, no SaveToDB -- the
 * spawn row is left alone so the world is unchanged after a restart.
 */
bool ChatHandler::HandleGameObjectNudgeCommand(char* args)
{
    uint32 lowguid;
    if (!ExtractUint32KeyFromLink(&args, "Hgameobject", lowguid))
        return false;
    ...
    // A TRANSPORT IS NOT IN `gameobject`, so the lookup above cannot find one.
    // Transports live in `transports` and carry HIGHGUID_MO_TRANSPORT, which has
    // no entry in the guid. Falling through to the map's own transport set is
    // the whole reason this command can answer anything interesting: the type
    // picks the client's code path, and a MAP_OBJECT ignoring a position update
    // says nothing about what a MO_TRANSPORT does with one.
    if (!obj)
        if (Player* me = m_session->GetPlayer())
            obj = me->GetMap()->GetTransport(ObjectGuid(HIGHGUID_MO_TRANSPORT, lowguid));
    ...
    {
        SendSysMessage("Usage: .gobject nudge <guid> <dx> <dy> <dz>");
        SetSentErrorMessage(true);
        return false;
    }

    float const x = obj->GetPositionX() + dx;
    float const y = obj->GetPositionY() + dy;
    float const z = obj->GetPositionZ() + dz;
    ...
    // Keep the server's collision copy honest, so `.gobject info` and any
    // height query agree with where the server now thinks it is. This is the
    // one thing beyond the three fields, and it sends nothing to anybody.
    obj->UpdateModelPosition();

    PSendSysMessage("Nudged %u to %.2f %.2f %.2f -- fields only, no respawn. If the model did not move, the client ignores position in a values update.",
                    obj->GetGUIDLow(), x, y, z);

    if (obj->GetGoType() == GAMEOBJECT_TYPE_MO_TRANSPORT &&
        !static_cast<Transport*>(obj)->IsFrozen())
        SendSysMessage("That one is a moving transport -- Update rewrites its position from the spline every 50ms, so freeze it first with .gobject stop or you are reading a value that is already gone.");
 ...
 * .gobject stop <guid>
 *
 * The other half of the nudge probe. Holds the SERVER's copy of a transport
 * still, and asks whether the client cares.
 *
 * Why this is the interesting question: the server drives a type 15 itself --
 * Transport::Update advances _pathProgress, evaluates the spline, relocates and
 * drags the passengers and the collision model along. But the only time the
 * client is ever told that progress is the create block in Object.cpp. If the
 * client free-runs from its own copy of the path after that, freezing the server
 * copy will do nothing visible and a server-steered route is impossible. If the
 * client follows, a great deal becomes possible.
 *
 * `_pendingStop` is not used for this. It already exists and Update honours it,
 * but it only engages at the next stop frame, and a gryphon path has none -- so
 * it can park a zeppelin at a tower and cannot hold one over open ground. The
 * probe needs the stronger thing.
 */
bool ChatHandler::HandleGameObjectStopCommand(char* args)
{
    uint32 lowguid;
    if (!ExtractUint32KeyFromLink(&args, "Hgameobject", lowguid))
        return false;
    ...
    Transport* t = me->GetMap()->GetTransport(ObjectGuid(HIGHGUID_MO_TRANSPORT, lowguid));
    if (!t)
    {
        PSendSysMessage("No transport with guid %u on this map. `transports` guids, not `gameobject` ones.", lowguid);
        SetSentErrorMessage(true);
        return false;
    }
    ...
    PSendSysMessage("%s (%u) is now %s at %.1f %.1f %.1f, path progress %u of %u.",
                    t->GetName(), lowguid, t->IsFrozen() ? "FROZEN" : "running again",
                    t->GetPositionX(), t->GetPositionY(), t->GetPositionZ(),
                    t->GetPathProgress(), t->GetPeriod());
    return true;
}
...
bool ChatHandler::HandleGameObjectAddCommand(char* args)
{
    ...
    /*
     * ABSENT MEANS FIVE MINUTES; AN EXPLICIT 0 MEANS NEVER DESPAWNS.
     *
     * The default used to be 0 here and then a `SetRespawnTime(300)` below
     * overwrote the parsed value UNCONDITIONALLY, so `.gobject add 1234 600`
     * silently saved 300 and there was no way to ask for anything else. The
     * default moves into the parse instead, which keeps the old behaviour when
     * nothing is passed and makes the argument mean what it says.
     *
     * 0 IS THE INTERESTING ONE and the reason this matters for quest objects.
     * GameObject::Update returns early at `if (!m_respawnDelayTime) return;`
     * after a goober is used, so a delay of zero is what keeps the thing
     * standing there for everybody instead of vanishing for five minutes the
     * first time one player clicks it.
     */
    int32 spawntimeSecs;
    if (!ExtractOptInt32(&args, spawntimeSecs, 300))
        return false;
    ...
    pGameObj->SetRespawnTime(spawntimeSecs > 0 ? spawntimeSecs : 0);
...
/*
 * AUDITIONING AN EFFECT, which is the only way to know whether one is worth
 * offering. `.house critter visual` does this for a critter and was written for
 * exactly this reason; this is the same trade with nothing to stand next to.
 *
 * WHICH KIT LOOKS RIGHT IS THE CLIENT'S BUSINESS, NOT THE DBC'S. A kit that
 * needs an animation the model has not got, or a state slot where an impact was
 * wanted, draws nothing at all -- no error, no log line, nothing to debug. So
 * the effects the dialog editor offers are a list somebody has walked with this
 * command, and adding one later means walking it again.
 *
 * Console-safe, hence the optional name: `.effect 6512 Bob` works with nobody
 * at the keyboard, which is what `.mangos-command` needs.
 */
bool ChatHandler::HandleEffectCommand(char* args)
{
    uint32 kitId = 0;
    if (!ExtractUInt32(&args, kitId) || !kitId)
    {
        SendSysMessage("Usage: .effect <SpellVisualKit id> [player]");
        SendSysMessage("Plays on your target, or on you, or on the player you name. 6512 is the critter hearts.");
        SetSentErrorMessage(true);
        return false;
    }
    ...
    if (args && *args)
    {
        ...
        if (!ExtractPlayerTarget(&args, &named))
            return false;
        if (!named)
        {
            SendSysMessage(LANG_PLAYER_NOT_FOUND);
    ...
    {
        // Your selection if you have one -- the point of this command is holding
        // an effect up against the thing that will carry it.
    ...
    if (!pUnit)
    {
        SendSysMessage("From the console, name a player: .effect <kit> <player>");
        SetSentErrorMessage(true);
        return false;
    }

    pUnit->SendPlaySpellVisual(kitId);
    PSendSysMessage("Effect %u on %s.", kitId, pUnit->GetName());
    return true;
}

/*
 * .emote <Emotes.dbc id> [player]
 *
 * The same audition tool as .effect above and for the same reason: whether a
 * model actually has an animation is not written down anywhere the server can
 * read, and one it has not got plays as nothing at all. The only way to know
 * is to stand somebody up and look.
 *
 * Emote 0 is allowed and is the useful half of this command, since it is what
 * puts a model that has been left dancing back on its feet.
 */
/*
 * .spot <name>        remember where you are standing, under that name
 * .spot del <name>    forget it
 *
 * The capture end of the dialog editor's "walks to" action. MOVE_TO takes raw
 * x/y/z and a browser has no way to pick a point in the world, so the point is
 * picked HERE, by standing on it, and the editor offers the name.
 *
 * Deliberately not `.gps`-and-paste. Three floats typed by hand into a web form
 * are three chances to send an NPC into a wall, and the number is not what
 * anybody writing a scene is thinking about -- the fire is.
 *
 * In game only. There is nowhere for a console session to be standing.
 */
/*
 * .scene <dialog_lua id>
 *
 * Run a scene by hand, with your TARGET as npc and YOU as player. The same two
 * arguments a gossip button would pass, which is the point: the only other way
 * to see a scene run is to wire it to a button and press it, and a scene you
 * are still writing is exactly when you do not want to do that.
 *
 * Errors come back as a whisper, the same as they would from a button.
 */
bool ChatHandler::HandleSceneCommand(char* args)
{
    uint32 id = 0;
    if (!ExtractUInt32(&args, id) || !id)
    {
        SendSysMessage("Usage: .scene <id>");
        SendSysMessage("Runs a dialog_lua scene with your target as npc and you as player.");
        SetSentErrorMessage(true);
        return false;
    }
    ...
    if (!m_session)
    {
        SendSysMessage("A scene needs somebody to run it on, so this one needs a character.");
        SetSentErrorMessage(true);
        return false;
    }
    ...
    {
        PSendSysMessage("There is no scene %u loaded. Try: reload dialog_scenes", id);
        SetSentErrorMessage(true);
        return false;
    }
    ...
    // Your selection if you have one, otherwise you are both halves. Running a
    // scene on yourself is a real way to test one.
    Unit* npc = GetSelectedUnit();
    if (!npc)
        npc = me;
    ...
    char name[32];
    snprintf(name, sizeof(name), "scene %u", id);

    if (LuaScene* scene = map->GetLuaScene())
        scene->Run(id, it->second, name, npc->GetObjectGuid(), me->GetObjectGuid());

    PSendSysMessage("Scene %u started on %s.", id, npc->GetName());
    return true;
}

bool ChatHandler::HandleSpotCommand(char* args)
{
    if (!m_session)
    {
        SendSysMessage("A spot is where you are standing, so this one needs a character.");
        SetSentErrorMessage(true);
        return false;
    }

    bool forget = !!ExtractLiteralArg(&args, "del");

    // The rest of the line, spaces and all: "the fire by the inn" is a name.
    char* rest = args ? args : (char*)"";
    while (*rest == ' ') ++rest;
    ...
    if (name.empty())
    {
        SendSysMessage("Usage: .spot <name>   or   .spot del <name>, with the NPC it is for selected");
        SendSysMessage("Stand where the NPC should end up, then name it. It is saved as \"<NPC>: <name>\".");
        // The other way to say "walk there", and the one to reach for when the
        // walk should move with the NPC rather than stay put in the world.
        SendSysMessage("Or .spotfrom: how far ahead and right of its post you are, for \"Walks from its post\".");
        SetSentErrorMessage(true);
        return false;
    }

    /*
     * A SPOT BELONGS TO THE NPC YOU HAVE SELECTED, BY NAME. Required since
     * 2026-09-16.
     *
     * Spots are one server-wide list with names unique across all of it, so
     * with a dozen NPCs the picker was going to be a long column of clashing
     * names -- there can only be one "the fire". Saving "Mother: the fire"
     * keeps the list sorted by NPC for free, lets every NPC have its own fire,
     * and needed no new column: it is a naming rule the command enforces rather
     * than a habit somebody has to remember.
     *
     * AN NPC, NOT A PLAYER. A creature or a bot NPC. GetSelectedUnit hands back
     * the caller when nothing is selected, which is exactly the case this has
     * to refuse, and a spot named after the GM who typed it would be the habit
     * failing silently.
     *
     * The position is still YOURS, not the NPC's. A spot is where it should
     * end up, and it is standing somewhere else.
     */
    Player* me = m_session->GetPlayer();
    Unit* target = GetSelectedUnit();
    bool const isNpc = target && target != me &&
        (target->GetTypeId() == TYPEID_UNIT ||
         (target->GetTypeId() == TYPEID_PLAYER && static_cast<Player*>(target)->IsBotNpc()));
    if (!isNpc)
    {
        SendSysMessage("Select the NPC this spot is for first. It is saved as \"<NPC>: <name>\".");
        SetSentErrorMessage(true);
        return false;
    }

    name = std::string(target->GetName()) + ": " + name;

    // REFUSED, NOT CUT. The old resize was harmless with nothing in front of
    // the name; with an NPC's name in front, cutting two long ones to the same
    // 48 characters would quietly make the second overwrite the first.
    if (name.size() > 48)
    {
        PSendSysMessage("\"%s\" is %u characters and a spot name is at most 48. Pick a shorter name.",
    ...
    if (forget)
    {
        WorldDatabase.PExecute("DELETE FROM `dialog_spot` WHERE `name` = '%s'", safe.c_str());
        PSendSysMessage("Forgot the spot \"%s\".", name.c_str());
        return true;
    }

    Player* pl = me;
    std::string author = m_session->GetUsername();
    WorldDatabase.escape_string(author);

    /*
     * REPLACE, not INSERT: naming a spot that already exists is how you MOVE
     * it, and standing somewhere better and typing the same name again is the
     * obvious way to want to. `name` is the unique key, so the old row goes.
     *
     * The id changes when that happens and nothing cares, because nothing
     * outside this table ever holds one -- a script stores the COORDINATES it
     * was written with, not the spot. Which also means moving a spot does NOT
     * move the scenes already written against it; they keep walking to where it
     * used to be until they are saved again.
     */
    WorldDatabase.PExecute(
        "REPLACE INTO `dialog_spot` (`name`, `map`, `x`, `y`, `z`, `o`, `author`) "
        "VALUES ('%s', %u, %f, %f, %f, %f, '%s')",
        safe.c_str(), pl->GetMapId(),
        pl->GetPositionX(), pl->GetPositionY(), pl->GetPositionZ(), pl->GetOrientation(),
        author.c_str());

    PSendSysMessage("Spot \"%s\" is here, on map %u.", name.c_str(), pl->GetMapId());
    return true;
}

/*
 * .spotfrom -- where you are standing, as yards ahead of and to the right of the
 * selected NPC's post. Added 2026-09-16.
 *
 * "Walks from its post" takes two numbers, and typing "3 ahead, 2 left" from
 * memory is guesswork -- which was the weak half of post offsets next to a spot,
 * made by standing somewhere and naming it. This makes the numbers the same
 * gesture: stand where the NPC should go, select it, read them off.
 *
 * IN THE FORM THEY ARE TYPED, not in the editor's sentence. "ahead 4.2, right
 * -1.8" goes straight into the two boxes, and the WalkOffset call beside it goes
 * straight into a scene. Rounded to a tenth of a yard, which is finer than
 * anyone can stand.
 *
 * ONE LINE, because this is typed while walking about and read in a narrow chat
 * frame. The 40 yard limit is said only when it bites: the core clamps a longer
 * offset, so the NPC would stop short of where you are standing and nothing
 * else would tell you why.
 */
bool ChatHandler::HandleSpotFromCommand(char* /*args*/)
{
    if (!m_session)
    {
        SendSysMessage("This measures from where you are standing, so it needs a character.");
        SetSentErrorMessage(true);
        return false;
    }
    ...
    if (!isNpc)
    {
        SendSysMessage("Select the NPC first, then stand where it should walk to.");
        SetSentErrorMessage(true);
        return false;
    }
    ...
    {
        SendSysMessage("That NPC is on another map.");
        SetSentErrorMessage(true);
        return false;
    }

    float forward, right;
    if (!BotNpc_OffsetFromPost(target, me->GetPositionX(), me->GetPositionY(), forward, right))
    {
        PSendSysMessage("%s has no post on this map to measure from.", target->GetName());
        SetSentErrorMessage(true);
        return false;
    }

    // A tenth of a yard, and never "-0.0" -- which is what rounding a tiny
    // negative gives, and which reads like a sign that matters.
    float const f = std::round(forward * 10.0f) / 10.0f + 0.0f;
    float const r = std::round(right * 10.0f) / 10.0f + 0.0f;

    bool const tooFar = std::fabs(f) > BOT_NPC_POST_MAX_OFFSET || std::fabs(r) > BOT_NPC_POST_MAX_OFFSET;

    PSendSysMessage("%s: ahead %.1f, right %.1f   npc:WalkOffset(%.1f, %.1f)%s",
                    target->GetName(), f, r, f, r,
                    tooFar ? "   past the 40 yard limit, use a spot" : "");
    return true;
}

bool ChatHandler::HandleEmoteCommand(char* args)
{
    uint32 emoteId = 0;
    if (!ExtractUInt32(&args, emoteId))
    {
        SendSysMessage("Usage: .emote <Emotes.dbc id> [player]");
        SendSysMessage("Plays on your target, or on you, or on the player you name. 0 stands a model back up.");
        SetSentErrorMessage(true);
        return false;
    }

    EmotesEntry const* pEmote = sEmotesStore.LookupEntry(emoteId);
    if (!pEmote && emoteId)
    {
        PSendSysMessage("Emote %u does not exist.", emoteId);
        SetSentErrorMessage(true);
        return false;
    }
    ...
    if (!pUnit)
    {
        SendSysMessage("From the console, name a player: .emote <id> <player>");
        SetSentErrorMessage(true);
        return false;
    }

    pUnit->HandleEmote(emoteId);

    // Say which of the two kinds it was. It is the thing you most need to know
    // while walking a list, and a lasting one that nobody warned you about is
    // how a model ends up stuck in a pose two auditions later.
    if (!emoteId)
        PSendSysMessage("Cleared the emote state on %s.", pUnit->GetName());
    else if (pEmote->EmoteType)
        PSendSysMessage("Emote %u (%s) on %s. This one LASTS: .emote 0 to stop it.", emoteId, pEmote->Name ? pEmote->Name : "", pUnit->GetName());
    else
        PSendSysMessage("Emote %u (%s) on %s.", emoteId, pEmote->Name ? pEmote->Name : "", pUnit->GetName());
    return true;
}
...
/*
 * `gameobject_template`, WITHOUT CLEARING THE MAP FIRST.
 *
 * This is what makes a gameobject somebody just wrote usable without a
 * restart, which is what /admin/objects needs: it writes a row and sends this,
 * and `.gobject add <entry>` works a second later.
 *
 * WHY IT IS SAFE, AND WHY IT IS NOT JUST A LoadGameobjectInfo() CALL. Every
 * GameObject in the world caches a GameObjectInfo const* at Create time and
 * never looks it up again. m_GameObjectInfoMap is a std::map, so inserting a
 * new entry moves nothing and overwriting an existing one happens in place --
 * every cached pointer stays valid and starts reading the new values. The
 * startup path clears first, which is right when nothing is spawned yet and a
 * use-after-free on all 66,056 spawns when something is.
 *
 * WHAT IT WILL NOT DO, said here because the silence would be the bug:
 *   - a row DELETED from the table stays in memory until a restart
 *   - a live object keeps the picture and the size it was created with, since
 *     the client was told those once. Re-place it, or restart, to see those.
 * Neither matters for adding a new one, which is what this exists for.
 */
bool ChatHandler::HandleReloadGameObjectTemplateCommand(char* /*args*/)
{
    sObjectMgr.LoadGameobjectInfo(true);
    SendSysMessage("DB table `gameobject_template` reloaded. New and changed entries only; a deleted one needs a restart.");
    return true;
}
...
// Adding a rentable mount is two statements and this command. It does NOT pick
// up a creature that is new to `rented_mount` and still unflagged -- that NPC
// needs `reload creature_template` first, for its npc_flags and script_name.
bool ChatHandler::HandleReloadRentedMountCommand(char* /*args*/)
{
    sObjectMgr.LoadRentedMounts();
    SendSysMessage("DB table `rented_mount` reloaded.");
    return true;
}
...
{
    /*
     * DRAIN, THEN LOAD -- the same treatment `reload gossip_scripts` got below,
     * and the reasoning there applies here in full. What is different is that
     * this table is now the one MOST certain to have something scheduled when
     * somebody types the command: bot NPC idle behaviour lives in
     * `generic_scripts`, and an idle loop is by definition a script that is
     * running or about to be, on every map that has an NPC with one.
     *
     * So the old "DB scripts used currently, please attempt reload later." was
     * not merely unreliable here, it was an instruction that could never be
     * followed. There is no later.
     */
    if (uint32 dropped = sMapMgr.ClearAllScheduledScripts())
        PSendSysMessage("Dropped %u pending script action(s) first.", dropped);

    if (sScriptMgr.IsScriptScheduled())
    {
        SendSysMessage("Script actions are still outstanding after draining every map -- "
                       "the scheduled-script count is leaking. Not reloading.");
        SetSentErrorMessage(true);
        return false;
    }
...
{
    /*
     * DRAIN, THEN LOAD. This used to refuse outright when anything was
     * scheduled, and on this server that meant it refused always: measured
     * 2026-09-14, four attempts across two sessions -- one of them on a server
     * twenty seconds old -- and every one was a silent no-op, because
     * ScriptMgr::LoadScripts returns early on the same condition. The result was
     * that `reload gossip_scripts` did not work at all and a gossip script
     * change needed a full restart.
     *
     * The gate is not paranoia: a scheduled ScriptAction holds a ScriptInfo
     * const* into the very map LoadScripts clears. But refusing is only one way
     * to honour that, and the weaker one -- it depends on a count falling back
     * to zero, which here it does not. Dropping the pending actions first makes
     * the precondition TRUE rather than merely tested, and pending delayed
     * actions are a reasonable thing to lose when an administrator has asked for
     * a reload.
     *
     * The count is reported because it is the only view anyone has of it, and if
     * it is still non-zero on the line after a full drain then something
     * increments it without ever scheduling anything -- which is worth knowing
     * and is not knowable any other way right now.
     */
    if (uint32 dropped = sMapMgr.ClearAllScheduledScripts())
        PSendSysMessage("Dropped %u pending script action(s) first.", dropped);
...
/*
 * `broadcast_text` IS RELOADABLE AND ALWAYS WAS -- nobody had wired the command.
 * ObjectMgr::LoadBroadcastTexts opens with `m_BroadcastTextLocaleMap.clear();
 * // for reload case`, so the loader was written for this; the reload table in
 * Chat.cpp simply had no row for it. Until 2026-09-14 CLAUDE.md listed it as one
 * of the two tables that need a full restart, which made every wording change in
 * a custom gossip menu a ~25s restart.
 *
 * BOTH CALLS ARE REQUIRED, IN THIS ORDER. LoadBroadcastTextLocales does not own a
 * map of its own -- it looks each entry up in m_BroadcastTextLocaleMap and fills
 * the locale columns into the record that is already there. So reloading the
 * texts alone leaves every non-English string blank until the next restart, and
 * the only sign of it is a player on a localised client reading English.
 */
bool ChatHandler::HandleReloadBroadcastTextCommand(char* /*args*/)
{
    sLog.outInfo("Re-Loading `broadcast_text` Table!");
    sObjectMgr.LoadBroadcastTexts();
    sObjectMgr.LoadBroadcastTextLocales();
    SendSysMessage("DB table `broadcast_text` reloaded.");
    return true;
}

/*
 * reload dialog_scenes -- dialog_lua AND dialog_spot, together.
 *
 * One command for two tables because they are one idea: a scene and the places
 * it can walk to. Reloading a spot without the scenes that name it would leave
 * a script erroring on a name that is right there in the database.
 *
 * Every map's compiled chunks go with it, lazily: LuaScene::Compile notices the
 * generation has moved and throws its cache away on the next press. Scenes
 * already PARKED mid-wait keep running on the old source, which is the right
 * answer -- the alternative is stopping them halfway through.
 */
bool ChatHandler::HandleReloadDialogScenesCommand(char* /*args*/)
{
    sLog.outInfo("Re-Loading `dialog_lua` and `dialog_spot`!");
    LuaScene::LoadAll();
    SendSysMessage("Dialog scenes and spots reloaded.");
    return true;
}
...
/*
 * THE CUSTOM MERCHANT TABLES, which had no reload at all.
 *
 * Turtle's four quartermasters were content somebody wrote once, so a restart
 * per edit was nobody's problem. It becomes one the moment a shop editor writes
 * these tables: `custom_merchant` is the only path on this client that can SHOW
 * an item as part of a price, and a ~14 second restart per saved row is not an
 * editing loop.
 *
 * Load() re-reads `custom_merchant` joined to `itemextendedcost`, so one command
 * covers both -- there is no separate cache for the costs to get out of step
 * with.
 */
bool ChatHandler::HandleReloadCustomMerchantCommand(char* /*args*/)
{
    sLog.outInfo("Re-Loading `custom_merchant` Table!");
    sCustomMerchantMgr.Load();
    SendSysMessage("DB table `custom_merchant` reloaded.");
    return true;
}
...
bool ChatHandler::HandleReloadNpcVendorCommand(char* /*args*/)
{
    ...
    //
    // `vendor_cost` FIRST, and this is the same rule the tables themselves
    // follow: a cost_id the cache does not know reads as "no cost" and the row
    // sells at the item's own price. That is a WRONG price rather than a missing
    // one, and nothing in game says so. sql/custom/094.
    sLog.outInfo("Re-Loading `vendor_cost` Table!");
    sObjectMgr.LoadVendorCosts();
    SendSysMessage("DB table `vendor_cost` reloaded.");
...
// ===========================================================================
//  .vendor -- a directory of the NPCs this server adds itself.
// ===========================================================================
//  Creature entries 100000-109999 are the custom block, so this list is
//  DERIVED and never hardcoded: a vendor added by a new sql/custom/ file turns
//  up here with no code change, and one that is removed stops turning up.
//
//  It reads the world DB and then compares what it read against what the world
//  is actually holding, and THAT COMPARISON IS THE POINT. Creature templates
//  and vendor stock are cached at startup, and `reload npc_vendor` validates
//  its rows against the CACHED creature templates -- so running the reloads in
//  the wrong order drops every row of a new vendor with a log-only error, and
//  in game the NPC greets you and opens an empty window. That is the quietest
//  failure mode in this repo, and here it is one line of output.
//
//  The second thing it surfaces is the window cap. MAX_VENDOR_ITEMS is 128,
//  and going over it is silent in a different way: the load-time check is
//  against UINT8_MAX (255), while 128 is applied when the vendor window is
//  built, with nothing logged on that path at all. A vendor with more rows than
//  that serves its first 128 forever and never says so.
namespace
{
    uint32 const CUSTOM_ENTRY_MIN = 100000;
    uint32 const CUSTOM_ENTRY_MAX = 109999;
    ...
    void LoadCustomNpcs(std::vector<CustomNpcRow>& out)
    {
        std::unique_ptr<QueryResult> result(WorldDatabase.PQuery(
            "SELECT `ct`.`entry`, `ct`.`name`, `ct`.`subname`, `ct`.`gossip_menu_id`, `ct`.`vendor_id`, "
            "(SELECT COUNT(*) FROM `npc_vendor` `v` WHERE `v`.`entry` = `ct`.`entry`) "
            "FROM `creature_template` `ct` "
            "WHERE `ct`.`entry` BETWEEN %u AND %u ORDER BY `ct`.`entry`",
            CUSTOM_ENTRY_MIN, CUSTOM_ENTRY_MAX));
        ...
        do
        {
            ...
            CustomNpcRow row;
            row.entry            = fields[0].GetUInt32();
            row.name             = fields[1].GetCppString();
            row.subname          = fields[2].GetCppString();
            row.gossipMenuId     = fields[3].GetUInt32();
            row.vendorTemplateId = fields[4].GetUInt32();
            row.dbItems          = fields[5].GetUInt32();

            // It has to sell something or have something to say. That is what
            // keeps the retired furniture-handle creature (100010) out of the
            // list: it is neither, having been superseded by a gameobject.
            if (!row.dbItems && !row.gossipMenuId)
                continue;
        ...
        // Spawns in one more query rather than one per NPC. Some of these are
        // spawned more than once; the first answers "where do I find one".
        std::unique_ptr<QueryResult> spawns(WorldDatabase.PQuery(
            "SELECT `id`, `map`, `position_x`, `position_y`, `position_z` FROM `creature` "
            "WHERE `id` BETWEEN %u AND %u", CUSTOM_ENTRY_MIN, CUSTOM_ENTRY_MAX));
        ...
        do
        {
            ...
            for (CustomNpcRow& row : out)
            {
                if (row.entry != id || row.spawned)
                    continue;

                row.spawned = true;
                row.map = fields[1].GetUInt32();
    ...
    // The zone name, which is the only part of a location a player can act on.
    // GetZoneId goes through getAreaFlag, which pulls a VMap tile off disk --
    // fine for the dozen-odd spawns here, and the reason this is not the shape
    // to copy for anything larger.
    std::string CustomNpcPlace(CustomNpcRow const& row)
    {
        if (!row.spawned)
            return "not spawned";

        uint32 const zoneId = sTerrainMgr.GetZoneId(row.map, row.x, row.y, row.z);
        if (AreaEntry const* zone = AreaEntry::GetById(zoneId))
            if (zone->Name && *zone->Name)
                return zone->Name;

        std::ostringstream fallback;
        fallback << "map " << row.map;
        return fallback.str();
    }

    // Whatever is wrong with one row, in the order it would bite. Empty when
    // the row is healthy, so a clean list stays quiet.
    std::string CustomNpcWarning(CustomNpcRow const& row)
    {
        if (!sObjectMgr.GetCreatureTemplate(row.entry))
            return "   << not loaded: reload creature_template";

        // A vendor whose stock comes from npc_vendor_template counts rows we
        // did not query, so the comparison below would cry wolf. None of ours
        // uses one today; this is here so the first that does is not reported
        // as broken.
        if (!row.vendorTemplateId)
        {
            VendorItemData const* live = sObjectMgr.GetNpcVendorItemList(row.entry);
            uint32 const liveItems = live ? live->GetItemCount() : 0;

            if (liveItems != row.dbItems)
            {
                std::ostringstream s;
                s << "   << serving " << liveItems << " of " << row.dbItems
                  << ": reload npc_vendor";
                return s.str();
            }
        }

        if (row.dbItems > MAX_VENDOR_ITEMS)
        {
            std::ostringstream s;
            s << "   << " << (row.dbItems - MAX_VENDOR_ITEMS) << " hidden: the window shows "
              << uint32(MAX_VENDOR_ITEMS);
            return s.str();
        }
    ...
    std::string CustomNpcLabel(CustomNpcRow const& row)
    {
...
bool ChatHandler::HandleVendorCommand(char* args)
{
    std::vector<CustomNpcRow> npcs;
    LoadCustomNpcs(npcs);

    if (npcs.empty())
    {
        SendSysMessage("No custom NPCs found. The custom block is creature 100000-109999.");
        SetSentErrorMessage(true);
        return false;
    }

    // Guard the pointer as well as the contents: every handler in this file
    // assumes a non-null args, and that assumption holds today, but this one is
    // reachable from the console as well as from chat.
    std::string const filter = ToLowerCopy((args && *args) ? args : "");

    std::vector<CustomNpcRow const*> matches;
    for (CustomNpcRow const& row : npcs)
    {
        if (!filter.empty())
        {
            std::ostringstream haystack;
            haystack << row.entry << ' ' << row.name << ' ' << row.subname;
            if (ToLowerCopy(haystack.str()).find(filter) == std::string::npos)
                continue;
        }
    ...
    if (matches.empty())
    {
        PSendSysMessage("No custom NPC matches \"%s\". Type .vendor for all %u.",
                        args, uint32(npcs.size()));
        SetSentErrorMessage(true);
        return false;
    }

    if (filter.empty())
        PSendSysMessage("The %u NPCs this server adds. `.vendor <name>` to narrow it.",
                        uint32(npcs.size()));
    else
        PSendSysMessage("%u of %u custom NPCs match \"%s\":",
                        uint32(matches.size()), uint32(npcs.size()), args);

    uint32 unspawned = 0;
    for (CustomNpcRow const* row : matches)
    {
        if (!row->spawned)
            ++unspawned;

        std::string const label = CustomNpcLabel(*row);
        std::string const place = CustomNpcPlace(*row);
        std::string const warn  = CustomNpcWarning(*row);

        if (row->dbItems)
            PSendSysMessage("  %u  %s - %u items - %s%s", row->entry, label.c_str(),
                            row->dbItems, place.c_str(), warn.c_str());
        else
            PSendSysMessage("  %u  %s - talk to it - %s%s", row->entry, label.c_str(),
                            place.c_str(), warn.c_str());
    }

    // Coordinates are noise to a player and the whole point to whoever is
    // placing them, so they follow the access level rather than the argument.
    if (matches.size() == 1 && GetAccessLevel() >= SEC_DEVELOPER)
    {
        CustomNpcRow const* row = matches[0];
        if (row->spawned)
            PSendSysMessage("     .go xyz %.1f %.1f %.1f %u", row->x, row->y, row->z, row->map);
        else
            PSendSysMessage("     stand where you want it, then:  .npc add %u", row->entry);
    }
    else if (unspawned && GetAccessLevel() >= SEC_DEVELOPER)
        PSendSysMessage("  %u not spawned. Stand where you want one and .npc add <entry>.",
                        unspawned);
