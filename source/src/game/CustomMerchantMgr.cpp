...
void CustomMerchantMgr::Load()
{
    ...
    do
    {
        ...
        /*
         * `entry` IS NOT ALWAYS A CREATURE ENTRY ANY MORE.
         *
         * Turtle's four quartermasters key their stock by their own creature
         * entry, and that still has to be validated -- a typo there is a shop
         * that silently never appears. But the shop editor writes rows keyed by
         * SHOP ID (200000-200999, the same block `npc_vendor_template` uses),
         * because a shop is not owned by one creature: several NPCs can offer
         * it, and a bot NPC has no creature entry to be keyed by at all.
         *
         * The two cannot collide. No creature in this database is numbered in
         * the shop block, and the editor allocates nothing outside it.
         */
        if (!IsShopId(merchantItem.entry) && !sObjectMgr.GetCreatureTemplate(merchantItem.entry))
        {
...
bool CustomMerchantMgr::HandleAddonMessage(WorldSession* session, Player* player, uint32 type, std::string const& msg)
{
    ...
    /*
     * WHICH MERCHANT DOES THIS REPLY BELONG TO?
     *
     * Two answers, and TURTLE'S GOES FIRST. A creature whose own entry keys
     * rows in `custom_merchant` is one of the four stock quartermasters, and
     * the client asked about it unprompted -- so that reading is always the
     * right one, and putting it first means a stale shop remembered from
     * somewhere else can never answer in its place.
     *
     * Otherwise the shop the server pushed, recorded on the session. That is
     * the only answer a bot NPC can ever have, since the client will not ask
     * on its behalf at all (measured 2026-09-14, see OpenShop), and it is also
     * what lets several NPCs share one shop.
     *
     * The guid is taken from the SESSION rather than from the gossip window,
     * which may well have been closed on the way into the frame.
     */
    Unit* vendor = nullptr;
    uint32 entry = 0;

    if (Creature* creature = player->GetNPCIfCanInteractWith(session->GetCurrentGossipGUID(), UNIT_NPC_FLAG_NONE))
    {
        if (GetItems(creature->GetEntry()))
        {
            vendor = creature;
    ...
    if (!vendor)
    {
        ObjectGuid vendorGuid = session->GetOpenMerchantGuid();
        entry = session->GetOpenMerchantEntry(vendorGuid);
        if (!entry)
            return true;

        // Re-checked on every message rather than only when the window opened.
        // The guid guard says WHICH unit; this says the player is still at it.
        vendor = player->GetMap()->GetUnit(vendorGuid);
        if (!vendor || !vendor->IsAlive() || !player->IsWithinDistInMap(vendor, INTERACTION_DISTANCE))
            return true;
    }

    if (!vendor || !GetItems(entry))
        return true;

    std::string command = document["command"].GetString();
    if (command == "show")
    {
        SendItemList(player, vendor, entry);
        ...
        BuyItem(player, vendor, entry, document["idx"].GetUint());

    return true;
}

bool CustomMerchantMgr::OpenShop(Player* player, Unit* vendor, uint32 entry)
{
    if (!player || !vendor || !GetItems(entry))
        return false;

    // Remembered BEFORE the batches go out, so a fast click cannot arrive
    // against a session that does not know what it is showing yet.
    player->GetSession()->SetOpenMerchant(vendor->GetObjectGuid(), entry);
...
bool CustomMerchantMgr::CanUseItem(Player* player, Unit* vendor, CustomMerchantItem const& merchantItem) const
{
    ...
    if (!reqFaction && proto->RequiredReputationRank > 0)
        reqFaction = vendor->GetFactionId();
    ...
    if (merchantItem.conditionId && !player->IsGameMaster() && !sObjectMgr.IsConditionSatisfied(merchantItem.conditionId, player, vendor->GetMap(), vendor, CONDITION_FROM_VENDOR))
        return false;
...
void CustomMerchantMgr::SendItemList(Player* player, Unit* vendor, uint32 entry) const
{
    CustomMerchantItemList const* items = GetItems(entry);
    if (!items)
        return;
    ...
    for (size_t begin = 0; begin < visibleItems.size(); begin += ItemsPerBatch)
    {
        ...
        for (size_t i = begin; i < end; ++i)
        {
            ...
            writer.Key("can_use");
            writer.Bool(CanUseItem(player, vendor, item));
            writer.EndObject();
        }
...
bool CustomMerchantMgr::BuyItem(Player* player, Unit* vendor, uint32 entry, uint32 id) const
{
    CustomMerchantItem const* merchantItem = GetItem(id);
    // Against the SHOP that is open, not against the unit's own entry -- those
    // stopped being the same thing when a bot NPC could sell.
    if (!merchantItem || merchantItem->entry != entry)
    {
        player->SendBuyError(BUY_ERR_CANT_FIND_ITEM, vendor, 0, 0);
        return false;
    }
    ...
    if (!proto)
    {
        player->SendBuyError(BUY_ERR_CANT_FIND_ITEM, vendor, merchantItem->item, 0);
        return false;
    }
    ...
    if (!CanUseItem(player, vendor, *merchantItem))
    {
        player->SendBuyError(BUY_ERR_RANK_REQUIRE, vendor, merchantItem->item, 0);
        return false;
    }
    ...
    {
        player->SendBuyError(BUY_ERR_NOT_ENOUGHT_MONEY, vendor, merchantItem->item, 0);
        return false;
    }
