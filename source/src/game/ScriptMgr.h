...
enum eScriptCommand
{
    ...
    SCRIPT_COMMAND_FIELD_SET                = 2,            // source = Object
                                                            // datalong = field_id
                                                            // datalong2 = value
    SCRIPT_COMMAND_MOVE_TO                  = 3,            // source = Unit (was Creature until 2026-09-15)
    ...
    SCRIPT_COMMAND_START_SCRIPT_ON_ZONE     = 92,           // source = Map
                                                            // datalong = generic_script_id
                                                            // datalong2 = zone_id
    SCRIPT_COMMAND_RECRUIT_BOT              = 93,           // source = Player
                                                            // datalong = eRecruitBotRole
                                                            // datalong2 = level tolerance, 0 = any level
    SCRIPT_COMMAND_PLAY_SPELL_VISUAL        = 94,           // source = Unit (the NPC), target = Unit (the viewer)
                                                            // datalong = SpellVisualKit.dbc index
                                                            // datalong2 = ePlaySpellVisualOn
    SCRIPT_COMMAND_EMOTE_TARGET             = 95,           // source = Unit, target = Unit (the one who emotes)
                                                            // datalong1-4 = emote_id
    SCRIPT_COMMAND_RUN_LUA                  = 96,           // source = Unit (the NPC), target = Unit (the player)
                                                            // datalong = dialog_lua.id
    SCRIPT_COMMAND_SET_BUSY                 = 97,           // source = Unit (the NPC)
                                                            // datalong = seconds
    SCRIPT_COMMAND_WALK_FROM_POST           = 98,           // source = Unit (the NPC)
                                                            // datalong3 = movement_options (see enum MoveOptions)
                                                            // x = yards ahead of its post, y = yards to the right
    SCRIPT_COMMAND_TAKE_MONEY               = 99,           // source = Player (from provided source or target)
                                                            // datalong = copper amount
                                                            // Upstream numbers this 93, which is RECRUIT_BOT here.
...
// Used for SCRIPT_COMMAND_EMOTE and SCRIPT_COMMAND_EMOTE_TARGET. More than one
// id means pick one of them at random, and the count is however many lead
// entries are non-zero -- so a gap stops the count, and datalong3 set with
// datalong2 clear is one emote, not two.
static constexpr uint32 MAX_EMOTE_ID = 4;

// WHY THERE ARE TWO EMOTE COMMANDS. SCRIPT_COMMAND_EMOTE (1) emotes its SOURCE
// and there is no field that can say otherwise: all four datalongs are emote
// ids and dataint is the targeted flag. For a gossip script the source is the
// NPC, which is the common case and the reason 1 is left exactly as it was.
// SCRIPT_COMMAND_EMOTE_TARGET (95) is the other half -- the player who clicked,
// who command 1 cannot reach at all.
//
// It is NOT a "who" flag on 1, the way SCRIPT_COMMAND_PLAY_SPELL_VISUAL has
// one, because 1 is upstream's and has no spare field. That does mean the two
// halves of one idea are stored under different commands, which is worth
// knowing before writing SQL by hand.
...
// Possible datalong values for SCRIPT_COMMAND_RECRUIT_BOT
enum eRecruitBotRole
{
    SO_RECRUIT_TANK    = 0,
    SO_RECRUIT_HEALER  = 1,
    SO_RECRUIT_DPS     = 2,

    // Not a role: dismisses every companion the player currently has. Sharing
    // the command keeps the whole companion interaction on one gossip page and
    // one script id, instead of a second command that would need its own
    // validation, its own stub and its own module entry point.
    SO_RECRUIT_DISMISS = 3,

    SO_RECRUIT_MAX
};

// Possible datalong2 values for SCRIPT_COMMAND_PLAY_SPELL_VISUAL.
//
// WHICH UNIT THE PACKET NAMES IS THE WHOLE EFFECT. SMSG_PLAY_SPELL_VISUAL
// carries a guid and a kit, and the client draws the kit ON that unit -- so
// "hearts over the player" and "hearts over the NPC" are one field apart and
// there is no third answer. Everyone in range sees either.
enum ePlaySpellVisualOn
{
    SO_PLAY_VISUAL_ON_TARGET = 0,                           // the player who clicked
    SO_PLAY_VISUAL_ON_SOURCE = 1,                           // the NPC he clicked

    SO_PLAY_VISUAL_ON_MAX
};
...
struct ScriptInfo
{
    ...
    union
    {
        ...
        struct                                              // SCRIPT_COMMAND_RECRUIT_BOT (93)
        {
            uint32 role;                                    // datalong
            uint32 classId;                                 // datalong2: 0 = any class that fits the role
        } recruitBot;

        struct                                              // SCRIPT_COMMAND_PLAY_SPELL_VISUAL (94)
        {
            uint32 kitId;                                   // datalong
            uint32 on;                                      // datalong2: ePlaySpellVisualOn
        } playSpellVisual;

        struct                                              // SCRIPT_COMMAND_EMOTE_TARGET (95)
        {
            uint32 emoteId[MAX_EMOTE_ID];                   // datalong to datalong4
        } emoteTarget;

        struct                                              // SCRIPT_COMMAND_RUN_LUA (96)
        {
            uint32 sceneId;                                 // datalong: a row in dialog_lua
        } runLua;

        struct                                              // SCRIPT_COMMAND_SET_BUSY (97)
        {
            uint32 seconds;                                 // datalong
        } setBusy;

        struct                                              // SCRIPT_COMMAND_WALK_FROM_POST (98)
        {
            uint32 unused1;                                 // datalong
            uint32 unused2;                                 // datalong2
            uint32 movementOptions;                         // datalong3, laid out as MOVE_TO's is
        } walkFromPost;

        struct                                              // SCRIPT_COMMAND_TAKE_MONEY (99)
        {
