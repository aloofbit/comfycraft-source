...
// == rented mounts ==========================================================
//
// The stables. Every NPC with script_name 'rented_mount' hires you the mount it
// is standing there being, for the cost, speed and duration in its tw_world.
// rented_mount row. sql/custom/079_rented_mounts.sql is the whole story; the two
// things worth knowing while reading this file:
//
//   THE SPELL IS THE MOUNT'S OWN, NOT A RENTAL SPELL. Spell.dbc has twelve
//   Rented-something spells and the client ships the same twelve, so a
//   thirteenth would mean a patched Spell.dbc on every client. Casting the real
//   mount spell gets the right model, name and icon for free; the clamp below
//   then makes it a rental. The twelve originals still point at their own 40700-
//   40711 rows, where the clamp is a no-op because they are already 40% / 5 min.
//
//   THE CLAMP HAS TO SURVIVE A LOGOUT, and it does, because character_aura saves
//   the modifier's CURRENT amount (Player::SaveAura) and restores it
//   (Player::LoadAura). Writing m_amount is therefore the whole fix -- without
//   it, relogging on a rented epic mount would hand back its real speed.
//
// This was a switch statement over twelve hardcoded creature entries until
// 2026-09-10, which meant a core rebuild per mount. One of those twelve, the
// Thalassian Unicorn, had a case here and none in the gossip half, so spell
// 40711 was unreachable for as long as it has existed.

// "50 copper", "1 gold 20 silver". Only ever built from a `cost` column, so it
// stays a local helper rather than earning a place in the core.
static std::string RentedMountPrice(uint32 copper)
{
    uint32 const gold   = copper / 10000;
    uint32 const silver = (copper % 10000) / 100;
    uint32 const rest   = copper % 100;

    std::ostringstream out;
    if (gold)
        out << gold << " gold";
    if (silver)
        out << (gold ? " " : "") << silver << " silver";
    if (rest || (!gold && !silver))
        out << ((gold || silver) ? " " : "") << rest << " copper";
...
{
    RentedMount const* rented = sObjectMgr.GetRentedMount(mount->GetEntry());
    if (!rented)
        return false;

    // Already mounted. Returning false rather than an empty menu, so the click
    // does nothing at all instead of opening a window with no options in it.
    if (player->GetMountID())
        return false;

    std::string const offer = rented->cost
        ? "Hire this " + rented->noun + " for " + RentedMountPrice(rented->cost) + "."
        : "Ride this " + rented->noun + ".";

    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, offer.c_str(), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
    player->SEND_GOSSIP_MENU(rented->textId, mount->GetGUID());
    return true;
}
...
bool GossipSelect_rented_mount(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    ...
    RentedMount const* rented = sObjectMgr.GetRentedMount(pCreature->GetEntry());
    if (!rented || pPlayer->GetMountID())
    {
        pPlayer->CLOSE_GOSSIP_MENU();
        return true;
    }

    if (pPlayer->GetMoney() < rented->cost)
    {
    ...
    // RENT THE ANIMAL THAT IS STANDING THERE, not the one in the row.
    //
    // Four renters wear several models -- Gadgetzan's Riding Tiger has four cat
    // skins, and Riding Horse, Riding Raptor and Riding Wolf four each of
    // theirs -- and Creature::ChooseDisplayId rolls which one at spawn, afresh
    // on every restart. A row names one spell, so it can only ever match one
    // face; the display index supplies the rest.
    //
    // The row wins whenever the NPC is showing its FIRST display, which covers
    // every single-model renter and costs nothing to check. That matters
    // because the row was chosen by name where several spells share a model,
    // and the index only knows how to break that tie by speed.
    uint32 spellId = rented->spellId;
    if (CreatureInfo const* ci = pCreature->GetCreatureInfo())
        if (pCreature->GetDisplayId() != ci->display_id[0])
            if (uint32 forThisModel = sObjectMgr.GetRentedMountSpellForDisplay(pCreature->GetDisplayId()))
                spellId = forThisModel;

    pPlayer->CastSpell(pPlayer, spellId, true);

    // THE MONEY COMES OUT AFTER THE CAST LANDS, not before. A mount spell has
    // conditions of its own -- indoors, in combat, shapeshifted -- and the old
    // code took the 50 copper whether or not the player ended up on a horse.
    // spellId, NOT rented->spellId: miss that and the clamp silently finds no
    // holder whenever the display index picked a different spell, which hands
    // out a permanent 60% mount for 50 copper.
    SpellAuraHolder* holder = pPlayer->GetSpellAuraHolder(spellId);
    if (!holder)
    {
        pPlayer->CLOSE_GOSSIP_MENU();
        return true;
    ...
    if (rented->duration)
    {
        // SetAuraMaxDuration clears the permanent flag for us, which is what
        // turns a real mount spell (duration index 21, -1) into a rental.
        holder->SetAuraMaxDuration(rented->duration);
        holder->SetAuraDuration(rented->duration);
        holder->UpdateAuraDuration();
    }

    if (rented->speed)
    {
        // Which effect index carries the speed is not fixed across mount spells,
        // so look rather than assume.
        for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
            if (Aura* aura = holder->GetAuraByEffectIndex(SpellEffectIndex(i)))
                if (aura->GetModifier()->m_auraname == SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED)
                    aura->GetModifier()->m_amount = int32(rented->speed);

        // UpdateSpeed recomputes from GetMaxPositiveAuraModifier, so rewriting
        // the amount and asking for a recompute is enough -- no reapply needed.
        pPlayer->UpdateSpeed(MOVE_RUN, true);
    }

    pPlayer->ModifyMoney(-int32(rented->cost));
    pPlayer->CLOSE_GOSSIP_MENU();
    return true;
}
